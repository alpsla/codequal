export interface PricingTier {
  ...
  // Stripe IDs (will be added later)
...
export class PricingConfig {
  ...
  static recommendTier(monthlyApiCalls: number): string {
    if (monthlyApiCalls <= 100) return 'free';
    if (monthlyApiCalls <= 1000) return 'starter';
    if (monthlyApiCalls <= 5000) return 'growth';
    if (monthlyApiCalls <= 20000) return 'scale';
    return 'enterprise';
  }
  ...
  static calculateOverageCharge(
    ...
    const tier = this.TIERS[tierId];
    if (!tier || tier.apiCallsPerMonth === -1) return 0;
