...
class ResultProcessor {
     ...
     */
    async processAgentResults(agentResults) {
        try {
            ...
            // Calculate metrics
            const metrics = this.calculateProcessingMetrics(allFindings, deduplicatedFindings, resolvedFindings);
            return {
     ...
     */
    async deduplicateFindings(findings) {
     ...
     */
    async groupSimilarFindings(findings) {
        const groups = [];
        ...
        for (const finding of findings) {
            ...
            for (const otherFinding of findings) {
                if (processed.has(otherFinding.id))
                    continue;
     ...
     */
    async calculateSimilarity(finding1, finding2) {
     ...
     */
    async mergeFindings(findings) {
        // Use the finding with highest confidence as base
        const baseFinding = findings.reduce((highest, current) => current.confidence > highest.confidence ? current : highest);
     ...
     */
    async resolveConflicts(findings) {
     ...
     */
    async detectConflicts(findings) {
        const conflicts = [];
     ...
     */
    resolveContradictoryFindings(findings) {
        ...
        return findings.reduce((highest, current) => current.confidence > highest.confidence ? current : highest);
    }
    /**
     * Organize findings by category
     */
    organizeByCategory(findings) {
        const organized = {
            security: [],
            architecture: [],
            performance: [],
     ...
     */
    calculateProcessingMetrics(original, deduplicated, resolved) {
    ...
    calculateSubstringSimilarity(str1, str2) {
        ...
        for (let len = shorter.length; len > 0; len--) {
            ...
            if (longestMatch > 0)
                break; // Found a match, no need to check shorter lengths
        }
    ...
    calculateSeveritySimilarity(severity1, severity2) {
        const severityOrder = { critical: 4, high: 3, medium: 2, low: 1 };
        const score1 = severityOrder[severity1] || 2;
        const score2 = severityOrder[severity2] || 2;
    ...
    calculateMergedConfidence(findings) {
    ...
    combineRecommendations(findings) {
