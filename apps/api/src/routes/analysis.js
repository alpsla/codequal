...
exports.storeAnalysisInHistory = storeAnalysisInHistory;
 ...
 */
exports.analysisRoutes.get('/history', async (req, res) => {
    try {
        const user = req.user;
        ...
        if (repositoryUrl && typeof repositoryUrl === 'string') {
            filteredHistory = userHistory.filter(analysis => analysis.repository.url === repositoryUrl);
        }
        // Apply pagination
 ...
 */
exports.analysisRoutes.get('/:id/results', async (req, res) => {
    try {
 ...
 */
exports.analysisRoutes.get('/:id/report', async (req, res) => {
    try {
        const { id: analysisId } = req.params;
 ...
 */
exports.analysisRoutes.post('/:id/feedback', async (req, res) => {
    try {
        const { id: analysisId } = req.params;
 ...
 */
exports.analysisRoutes.get('/stats', async (req, res) => {
    try {
        const user = req.user;
        ...
        const recentAnalyses = userHistory.filter(analysis => new Date(analysis.metadata.timestamp) >= cutoffDate);
...
function generateFindingsMarkdown(findings) {
    let markdown = '';
...
function calculateLanguageStats(analyses) {
...
function storeAnalysisInHistory(userId, analysis) {
