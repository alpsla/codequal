...
function exportToPDF() {
    ...
    // Add print mode class to body for better control
...
function copyShareLink() {
    ...
    // For now, we'll create a mock shareable URL
    const baseUrl = 'https://app.codequal.com/reports/';
    const reportId = document.querySelector('[data-analysis-id]')?.getAttribute('data-analysis-id') || 'abc-123-def-456';
    const shareableUrl = `${baseUrl}${reportId}`;
...
function exportToMarkdown() {
    ...
    // Add metadata
    const prNumber = document.querySelector('.subtitle')?.textContent || 'Pull Request Analysis';
    ...
    // Add PR Decision
    const decisionTitle = document.querySelector('.decision-title')?.textContent || '';
    const decisionMessage = document.querySelector('.decision-message')?.textContent || '';
    const confidence = document.querySelector('.confidence-percentage')?.textContent || '';
    
    if (decisionTitle) {
        markdown += `## PR Decision: ${decisionTitle}\n\n`;
        markdown += `**Confidence:** ${confidence}%\n\n`;
        markdown += `${decisionMessage}\n\n`;
        
        // Add blocking issues
        const blockingIssues = document.querySelectorAll('.blocking-issues li');
        if (blockingIssues.length > 0) {
            markdown += '### Blocking Issues\n\n';
            blockingIssues.forEach(issue => {
                markdown += `- ${issue.textContent.trim()}\n`;
            });
            markdown += '\n';
        }
        
        // Add positive findings
        const positiveFindings = document.querySelectorAll('.positive-findings li');
        if (positiveFindings.length > 0) {
            markdown += '### Positive Findings\n\n';
            positiveFindings.forEach(finding => {
                markdown += `- ${finding.textContent.trim()}\n`;
            });
            markdown += '\n';
        }
    ...
    // Add PR Issues
    markdown += '## Current PR Issues\n\n';
    document.querySelectorAll('#pr-issues .finding').forEach(finding => {
        ...
        const codeSnippet = finding.querySelector('.code-snippet')?.textContent || '';
        const recommendation = finding.querySelector('.recommendation-box')?.textContent || '';
    ...
    document.querySelectorAll('#repo-issues .finding').forEach(finding => {
        const severity = finding.querySelector('.badge')?.textContent || '';
        const title = finding.querySelector('h3')?.textContent || '';
        const description = finding.querySelector('p')?.textContent || '';
        const codeSnippet = finding.querySelector('.code-snippet')?.textContent || '';
    ...
    if (skillCards.length > 0) {
        markdown += '## Skills Assessment\n\n';
        skillCards.forEach(card => {
            const skillName = card.querySelector('.skill-header span:first-child')?.textContent || '';
            const skillScore = card.querySelector('.skill-header span:last-child')?.textContent || '';
            if (skillName && skillScore) {
                markdown += `- **${skillName}**: ${skillScore}\n`;
            }
        });
        markdown += '\n';
        
        // Add skill recommendations
        const skillRecs = document.querySelectorAll('.skill-recommendations .recommendation-card');
        if (skillRecs.length > 0) {
            markdown += '### Improvement Suggestions\n\n';
            skillRecs.forEach(rec => {
                const title = rec.querySelector('h4')?.textContent || '';
                const desc = rec.querySelector('p')?.textContent || '';
                markdown += `- **${title.trim()}**: ${desc}\n`;
            });
            markdown += '\n';
        }
    ...
    const eduModules = document.querySelectorAll('.edu-module');
    if (eduModules.length > 0) {
        markdown += '## Educational Resources\n\n';
        eduModules.forEach(module => {
            const title = module.querySelector('h3')?.textContent || '';
            const desc = module.querySelector('p')?.textContent || '';
            const link = module.querySelector('a')?.href || '';
            
            markdown += `### ${title}\n\n`;
            markdown += `${desc}\n`;
            if (link && link !== '#') {
                markdown += `[Learn More](${link})\n`;
            }
            markdown += '\n';
        });
    }
    ...
    // Add overall score
    const overallScore = document.querySelector('.score-number')?.textContent || '';
    if (overallScore) {
        markdown += '---\n\n';
        markdown += `## Overall Code Quality Score: ${overallScore}/100\n`;
    }
...
function toggleLowerPriorityIssues() {
    ...
    if (section && section.style.display === 'none') {
        ...
        if (toggleText) toggleText.textContent = toggleText.textContent.replace('Show All', 'Hide');
    } else if (section) {
        section.style.display = 'none';
        if (toggleArrow) toggleArrow.textContent = '▼';
        if (toggleText) toggleText.textContent = toggleText.textContent.replace('Hide', 'Show All');
    }
}

window.toggleLowerPriorityIssues = toggleLowerPriorityIssues;
...
window.copyPRComment = copyPRComment;
...
`;
document.head.appendChild(style);// Chart.js default options to ensure proper sizing
Chart.defaults.responsive = true;
Chart.defaults.maintainAspectRatio = true;
Chart.defaults.aspectRatio = 1;

// Override chart initialization to add size constraints
const originalInitCharts = window.initCharts;
window.initCharts = function() {
    // Set canvas dimensions before initialization
...
// Fix for doughnut chart specifically
document.addEventListener('DOMContentLoaded', function() {
    const issuesChart = document.getElementById('issuesChart');
    if (issuesChart) {
        issuesChart.parentElement.style.maxWidth = '400px';
        issuesChart.parentElement.style.margin = '0 auto';
    }
});