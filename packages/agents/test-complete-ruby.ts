...
/**
 * Complete Ruby Flow Test
 * Testing with sinatra repository (popular Ruby web framework)
 */
...
async function testCompleteRubyFlow() {
  console.log('💎 Complete Ruby Flow Test\n');
  console.log('=' .repeat(70) + '\n');
  
  loadEnvironment();
  
  // Using sinatra web framework - lightweight Ruby web framework
  const repositoryUrl = 'https://github.com/sinatra/sinatra';
  const prNumber = 1900; // Recent PR with changes
  
  console.log(`📦 Repository: ${repositoryUrl} (Ruby)`);
  ...
  try {
    ...
    const rubyPatterns = mainIssues.filter(i => 
      i.location?.file && (
        i.location.file.endsWith('.rb') ||
        i.location.file.endsWith('.rake') ||
        i.location.file.includes('lib/') ||
        i.location.file.includes('spec/')
      )
    );
    console.log(`   💎 Ruby files detected: ${rubyPatterns.length}/${mainIssues.length}`);
    ...
    const reportPath = `./test-reports/ruby-complete-${timestamp}.md`;
    ...
    // Analyze report quality and Ruby-specific checks
    ...
    const checks = {
      ...
      'Ruby-specific patterns': report.includes('def ') || report.includes('class ') || report.includes('module ') || report.includes('require '),
      'No placeholders': !report.includes('example.com') && !report.includes('def example')
    };
    ...
    if (qualityScore >= 90) {
      console.log('   🎉 EXCELLENT: Ruby support is working perfectly!');
    } else if (qualityScore >= 70) {
      console.log('   ⚠️  GOOD: Ruby support is functional but needs improvement');
    } else {
      console.log('   ❌ NEEDS WORK: Ruby support has issues');
    }
    
    // Final summary
    console.log('\n' + '='.repeat(70));
    console.log('🎯 RUBY LANGUAGE SUMMARY:');
    console.log('='.repeat(70));
    ...
    console.log(`   Ruby files detected: ${rubyPatterns.length}`);
    ...
    const hasRubySupport = rubyPatterns.length > 0;
    
    if (hasRealSnippets && hasGoodReport && hasRubySupport) {
      console.log('\n🎉 Ruby language support is working correctly!');
      console.log('   Multi-language support is ready for production.');
    } else {
      console.log('\n⚠️  Some issues with Ruby support:');
      if (!hasRealSnippets) console.log('   - Code snippet extraction needs improvement for Ruby');
      if (!hasGoodReport) console.log('   - Report generation needs Ruby-specific fixes');
      if (!hasRubySupport) console.log('   - Ruby file detection not working');
    }
...
// Run the test
testCompleteRubyFlow().catch(console.error);