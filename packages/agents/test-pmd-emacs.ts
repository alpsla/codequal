...
async function testPMDEmacs() {
  const namespace = 'codequal-dev';
  const workspaceId = `test-pmd-emacs-${Date.now()}`;
...
`;
    ...
    logger.info('🔧 Testing PMD with EMACS Format');

    const jobName = `test-pmd-emacs-${workspaceId}`;

    // The updated command from kubernetes-repository-manager.ts
    const pmdCommand = `echo 'Running PMD...' && cd /workspace/repo && pmd check -d . -f emacs -R category/java/bestpractices.xml,category/java/codestyle.xml,category/java/errorprone.xml,category/java/design.xml 2>&1 || echo 'PMD analysis complete'`;
  ...
  template:
    spec:
      ...
      - name: pmd-test
        ...
        args:
        - |
          echo "=== Testing PMD with EMACS Format ==="
          ...
          echo "Running PMD with EMACS format:"
          echo "${pmdCommand}"
          echo "---"
...
`;
    ...
    logger.info('✅ PMD EMACS Test Results:');
    console.log(output);
    ...
    if (result && result.parsedIssues && result.parsedIssues.length > 0) {
      logger.info(`\n🎉 PMD EMACS FORMAT WORKING: Found ${result.parsedIssues.length} issues`);
      logger.info('📊 First 15 issues:');
      result.parsedIssues.slice(0, 15).forEach((issue, idx) => {
        const msg = issue.message || issue.raw || 'Unknown';
        logger.info(`   ${idx + 1}. ${issue.file}:${issue.line} - ${msg.substring(0, 100)}`);
      });
    } else {
      logger.warn(`⚠️ PMD: Parser still not working properly`);
      logger.info('Raw output for debugging:');
...
testPMDEmacs().catch(console.error);