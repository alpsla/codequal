...
exports.ModelTokenTracker = void 0;
exports.getModelTokenTracker = getModelTokenTracker;
const utils_1 = require("@codequal/core/utils");
/**
 * Service for tracking model token usage and costs
 */
class ModelTokenTracker {
    constructor(modelVersionSync, vectorStorage) {
        this.modelVersionSync = modelVersionSync;
        this.vectorStorage = vectorStorage;
        this.records = new Map();
        this.modelPricingCache = new Map();
        this.logger = (0, utils_1.createLogger)('ModelTokenTracker');
    }
    /**
    ...
    async trackUsage(params) {
        try {
            ...
            // Calculate costs
            const cost = {
                input: (params.tokenUsage.input / 1000000) * pricing.input,
                output: (params.tokenUsage.output / 1000000) * pricing.output,
                total: 0
            };
    ...
    async getModelPricing(model) {
        ...
        try {
            ...
            const canonicalVersion = await this.modelVersionSync.getCanonicalVersion(model.includes('/') ? model.split('/')[0] : 'openrouter', model.includes('/') ? model.split('/').slice(1).join('/') : model);
     ...
     */
    async exportUsageReport(analysisId) {
...
exports.ModelTokenTracker = ModelTokenTracker;
// Singleton instance
let trackerInstance = null;
/**
 * Get or create the model token tracker instance
 */
function getModelTokenTracker(modelVersionSync, vectorStorage) {
