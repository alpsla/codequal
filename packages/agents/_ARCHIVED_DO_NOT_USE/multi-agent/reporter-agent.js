 ...
 */
class ReporterAgent {
    constructor(vectorDB, // Optional Vector DB for content search
    reportingService // Optional reporting service for visualizations
    ) {
        this.vectorDB = vectorDB;
        this.reportingService = reportingService;
        this.logger = (0, utils_1.createLogger)('ReporterAgent');
        this.reportFormatter = new report_formatter_service_1.ReportFormatterService();
    }
    /**
     ...
     */
    async generateStandardReport(analysisResult, compiledEducationalData, recommendationModule, reportFormat) {
        ...
        const standardReport = await this.reportFormatter.formatReport(analysisResult, compiledEducationalData, recommendationModule, reportFormat);
     ...
     */
    async generateReport(analysisResults, educationalContent, format) {
        ...
        const educationalSections = await this.createEducationalSections(educationalContent, searchPrompts, format.educationalDepth);
        // Format the report based on output type
        const report = await this.formatReport(analysisResults, educationalSections, format);
        return report;
    }
    /**
     * Enrich standard report with Vector DB search results
     */
    async enrichReportWithSearchResults(report, compiledEducationalData) {
        try {
     ...
     */
    async enhanceVisualizationsWithService(report) {
        try {
            if (!this.reportingService)
                return;
            // Generate additional visualization data
            const dependencyGraph = await this.reportingService.generateDependencyGraph?.(report.modules.findings.categories.dependencies?.findings || []);
            if (dependencyGraph) {
            ...
            const trendData = await this.reportingService.generateTrendAnalysis?.(report.repositoryUrl);
            if (trendData) {
     ...
     */
    async enhanceWithMCPTools(report) {
        try {
     ...
     */
    async generatePDFExports(report) {
        const pdfExports = [];
        try {
            const exportFormats = [
                {
     ...
     */
    async updateGrafanaDashboards(report) {
        const dashboardUrls = [];
        try {
     ...
     */
    generateDependencyMermaid(dependencies) {
     ...
     */
    generateFindingsFlowMermaid(findings) {
     ...
     */
    generateLearningPathMermaid(learningPath) {
        let mermaidCode = 'flowchart TD\n';
     ...
     */
    estimatePDFPages(report, format) {
     ...
     */
    sanitizeDashboardId(input) {
        return input.replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase();
    }
    /**
     * Generate specific search prompts for educational content
     */
    generateEducationalSearchPrompts(educationalContent) {
        const prompts = [];
     ...
     */
    async createEducationalSections(educationalContent, searchPrompts, depth) {
        const sections = [];
        ...
        if (educationalContent.learningPath.steps.length > 0) {
            sections.push({
                ...
                keyResources: await this.searchKeyResources(searchPrompts.filter(p => p.context.includes('Learning path')), depth),
        ...
        if (educationalContent.skillGaps.length > 0 && depth !== 'summary') {
            sections.push({
                ...
                keyResources: await this.searchKeyResources(searchPrompts.filter(p => p.context === 'Identified skill gap'), depth),
                searchPrompts: depth === 'comprehensive' ?
     ...
     */
    async searchKeyResources(prompts, depth) {
        const resources = [];
     ...
     */
    formatLearningPathSummary(learningPath, depth) {
        if (depth === 'summary') {
            ...
            const topTopics = learningPath.steps.slice(0, 3).map((s) => {
     ...
     */
    formatBestPracticesSummary(bestPractices, depth) {
        if (depth === 'summary') {
     ...
     */
    extractBestPracticeResources(bestPractices) {
     ...
     */
    async formatReport(analysisResults, educationalSections, format) {
        const report = {
     ...
     */
    generateExecutiveSummary(analysisResults, educationalSections) {
     ...
     */
    generateRecommendations(analysisResults, educationalSections) {
     ...
     */
    async formatForDashboard(report) {
