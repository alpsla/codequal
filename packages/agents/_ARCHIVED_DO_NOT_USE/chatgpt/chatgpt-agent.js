 ...
 */
class ChatGPTAgent extends base_agent_1.BaseAgent {
    /**
    ...
    constructor(promptTemplate, config = {}) {
        ...
        this.model = cleanConfig.model || model_versions_1.DEFAULT_MODELS_BY_PROVIDER['openai'];
        // Log model configuration for debugging
        this.log('ChatGPTAgent initialized with model:', {
            configModel: cleanConfig.model,
            defaultModel: model_versions_1.DEFAULT_MODELS_BY_PROVIDER['openai'],
    ...
    initOpenAIClient() {
        ...
        const logger = (0, utils_1.createLogger)(useOpenRouter ? 'OpenRouterAPI' : 'OpenAIAPI');
     ...
     */
    createClientWrapper(openai, logger) {
        return {
            chat: {
                completions: {
                    create: async (params) => {
    ...
    async analyze(data) {
        try {
            ...
            const systemPrompt = (0, prompt_loader_1.loadPromptTemplate)(`${this.promptTemplate}_system`) ||
