 ...
 */
class DeepSeekAgent extends base_agent_1.BaseAgent {
    /**
    ...
    constructor(promptTemplate, config = {}) {
        ...
        if (config.premium) {
            ...
            this.model = model_versions_1.DEFAULT_MODELS_BY_PROVIDER['deepseek']; // Default model
        }
    ...
    initDeepSeekClient() {
        ...
        const logger = (0, utils_1.createLogger)('DeepSeekAPI');
        // Ensure this.model is set
        if (!this.model) {
            this.model = model_versions_1.DEFAULT_MODELS_BY_PROVIDER['deepseek'];
