...
exports.ComparisonOrchestrator = void 0;
const comparison_agent_1 = require("../comparison/comparison-agent");
const location_enhancer_1 = require("../services/location-enhancer");
...
class ComparisonOrchestrator {
    constructor(configProvider, skillProvider, dataStore, researcherAgent, educatorAgent, logger, comparisonAgentInstance) {
        this.configProvider = configProvider;
        this.skillProvider = skillProvider;
        this.dataStore = dataStore;
        this.researcherAgent = researcherAgent;
        this.educatorAgent = educatorAgent;
        this.logger = logger;
        this.comparisonAgentInstance = comparisonAgentInstance;
        this.modelConfigId = '';
        this.primaryModel = '';
        this.fallbackModel = '';
        this.language = 'typescript';
        this.repositorySize = 'medium';
        // Use injected comparison agent or create default with skill provider (BUG-012 fix)
        this.comparisonAgent = comparisonAgentInstance || new comparison_agent_1.ComparisonAgent(logger, undefined, // modelService
        this.skillProvider // Pass skill provider for score persistence
        );
        this.batchLocationEnhancer = new location_enhancer_1.BatchLocationEnhancer();
        this.locationEnhancer = new location_enhancer_1.LocationEnhancer();
        this.languageRouter = new language_router_1.LanguageRouter();
    }
    /**
     ...
     */
    async initialize(language, repoSize) {
        this.language = language || 'typescript';
        this.repositorySize = repoSize || 'medium';
        try {
            const supabaseConfig = await (0, monitoring_1.getDynamicModelConfig)('orchestrator', this.language, this.repositorySize);
            if (supabaseConfig) {
    ...
    async executeComparison(request) {
        ...
        const deepWikiScanDuration = request.deepWikiScanDuration;
        try {
                    ...
                    try {
                        ...
                        const [mainEnhancement, prEnhancement] = await Promise.all([
                            request.mainBranchAnalysis?.issues ?
                                this.locationEnhancer.enhanceIssuesWithLocations(request.mainBranchAnalysis.issues, repoUrl || '', undefined // Use undefined for main branch
                                ) : Promise.resolve(null),
                            request.featureBranchAnalysis?.issues ?
                                this.locationEnhancer.enhanceIssuesWithLocations(request.featureBranchAnalysis.issues, repoUrl || '', prNumber // Pass PR number for feature branch
                                ) : Promise.resolve(null)
                        ]);
                        if (mainEnhancement) {
                            enhancedMainAnalysis = {
                                ...request.mainBranchAnalysis,
                                issues: mainEnhancement.issues.map((issue) => ({
                                    ...issue,
                        ...
                        if (prEnhancement) {
                            enhancedFeatureAnalysis = {
                                ...
                                issues: prEnhancement.issues.map((issue) => ({
                                    ...issue,
                                    message: issue.message || issue.title || issue.description
                                }))
                            };
            ...
            const finalReport = await (this.comparisonAgent.generateFinalReport ?
                this.comparisonAgent.generateFinalReport({
                    comparison: enhancedComparisonData,
                    educationalContent: educationalResearch,
                    prMetadata: request.prMetadata,
                    includeEducation: request.includeEducation !== false
                }) :
                Promise.resolve({
                    report: await this.comparisonAgent.generateReport(enhancedComparisonData),
                    prComment: this.comparisonAgent.generatePRComment(enhancedComparisonData)
                }));
     ...
     */
    async updateSkills(analysisResult, request) {
        if (!request.prMetadata?.author)
            return;
        try {
     ...
     */
    async storeAnalysisReport(analysisResult, request, config, educationalEnhancements) {
        const report = {
     ...
     */
    ensureCompatibleAnalysisResult(analysis) {
        if (!analysis)
            return { issues: [], recommendations: [], scores: null };
     ...
     */
    convertToSkillProfile(developerSkills) {
        if (!developerSkills)
            return null;
     ...
     */
    generateBasicReport(comparison) {
...
## Key Findings
${comparison.insights?.map((insight) => `- ${insight.title}: ${insight.description}`).join('\n') || 'No significant issues found.'}

## Recommendations
${comparison.recommendations?.map((rec, i) => `${i + 1}. ${rec.title || rec}`).join('\n') || 'Continue with best practices.'}
`;
    }
    /**
     * Generate PR comment from comparison data
     */
    generatePRComment(comparison) {
...
_Analysis powered by CodeAI Comparison Agent_`;
     ...
     */
    buildRolePrompt(config, context) {
        const focusAreas = [];
...
Ensure 100% accuracy, professional language, and actionable recommendations.`;
    ...
    analyzeFileTypes(issues) {
        ...
        issues.forEach(issue => {
            ...
            if (file.includes('auth') || file.includes('security'))
                types.security++;
            else if (file.includes('perf') || file.includes('cache'))
                types.performance++;
     ...
     */
    getToolsForLanguage(language, category) {
     ...
     */
    async checkToolAvailability(language) {
        const lang = language || this.language || 'typescript';
     ...
     */
    async initializeSpecializedAgents(language, agents) {
     ...
     */
    getRecommendedModelForLanguage(language) {
        const modelMapping = {
    ...
    extractUserIds(request) {
        const ids = new Set();
        if (request.prMetadata?.author)
            ids.add(request.prMetadata.author);
        // Extract from team profiles if available
        return Array.from(ids);
    }
    calculateSkillAdjustments(analysisResult) {
    ...
    calculateCost(context, config) {
        ...
        const millionTokens = totalTokens / 1000000;
    ...
    calculateOverallScore(comparison) {
        ...
        const newCritical = comparison.newIssues?.filter((i) => i.severity === 'critical').length || 0;
        const newHigh = comparison.newIssues?.filter((i) => i.severity === 'high').length || 0;
     ...
     */
    deduplicateIssuesForEducation(issues) {
        ...
        if (this.logger) {
            ...
            switch (level) {
                case 'debug':
                    ...
                    break; // eslint-disable-line no-console
                case 'info':
...
exports.ComparisonOrchestrator = ComparisonOrchestrator;
