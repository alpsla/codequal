 ...
 *
 * Tests:
 * - Main branch (baseline with no errors)
 * - feat/v9-footer-fixes-pr branch (with 12 ESLint errors restored)
 * - V9 grouped report generation
 * - Issue categorization (NEW vs EXISTING_REST)
 */
...
async function runCodeQualDogfoodingTest(): Promise<void> {
  ...
  try {
    ...
    const reportPath = path.join(outputDir, `codequal-v9-dogfooding-report-${timestamp}.md`);
    ...
    try {
      ...
      if (currentBranch !== 'feat/v9-footer-fixes-pr') {
