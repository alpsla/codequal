...
exports.IssueResolutionDetector = void 0;
const utils_1 = require("@codequal/core/utils");
/**
 * Service to detect which repository issues were fixed in a PR
 */
class IssueResolutionDetector {
    constructor() {
        this.logger = (0, utils_1.createLogger)('IssueResolutionDetector');
    }
    /**
     * Compare PR analysis with existing repo issues to detect fixes
     */
    detectFixedIssues(prAnalysis, existingRepoIssues, repository, prNumber) {
        ...
        for (const category of categories) {
            const prIssueIds = new Set((category.prData || []).map((issue) => this.generateIssueId(issue)));
            const repoIssueIds = new Map((category.repoData || []).map((issue) => [this.generateIssueId(issue), issue]));
...
exports.IssueResolutionDetector = IssueResolutionDetector;
