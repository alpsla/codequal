# V9 SESSION ARCHIVE
**Historical session details - For condensed version, see: [V9_CRITICAL_KNOWLEDGE_BASE.md](./V9_CRITICAL_KNOWLEDGE_BASE.md)**
**Last Updated: November 30, 2025**

---

## SESSION 32: Two-Tier Product Architecture & Tool Registry (November 30, 2025)

### Problem Identified
Need a clear product differentiation strategy and scalable tool management system to:
1. Differentiate between free and paid users
2. Manage tool updates across languages dynamically
3. Optimize AI token usage with two-stage enrichment
4. Support commit-based caching to skip redundant scans

### Solution: Two-Tier Product Model + Dynamic Tool Registry

#### Two-Tier Product Architecture

| Tier | Price | What User Gets | Target Users |
|------|-------|----------------|--------------|
| **BASIC** | Free | Report + Fix Recommendations | Developers wanting insights, users preferring other IDEs |
| **PRO** | $8-10/month | Auto-Fix During Scan + Report | Users wanting automated fixes |

**Marketing Hook:**
- BASIC: "Submit PR -> Get detailed report with fix instructions"
- PRO: "Submit PR -> Get fixed PR back" (vs competitors who only report issues)

#### Two-Stage AI Enrichment Pipeline

**Goal:** Save 40-60% tokens by having cheap model prepare data for expensive model.
...
- Pre-classify issues
- Generate structured data for Stage 2
- Cost: ~$0.001 per issue

Stage 2: Expensive Model (Opus/GPT-4 class)
- Receive pre-processed data (smaller input)
- Generate high-quality fixes
- Create educational content
- Produce final recommendations
- Cost: ~$0.005 per issue (vs $0.01 without Stage 1)
...
type ToolCategory =
  ...
  | 'dead_code' | 'code_duplication' | 'complexity' | 'secrets'
  | 'license' | 'performance' | 'documentation' | 'test_coverage';
```

#### Tool Selection Decision Logic (70% Threshold)

```
New Tool Found by Researcher
         |
Calculate overlap with existing tools
         |
>= 70% overlap? -> YES -> REPLACE if new tool is better
                -> NO  -> ADD as complementary tool
```

**Examples:**
- Ruff vs Flake8: 95% overlap -> REPLACE (Ruff is faster)
- Ruff vs Bandit: 20% overlap -> ADD (different categories)
- ESLint vs Prettier: 30% overlap -> ADD (complementary)

#### Pre-built Docker Images Strategy

```
Quarterly Research Cycle:
Day 1-7:   Researcher identifies tool updates
Day 8-14:  DevOps builds new Docker images
Day 15:    Deploy new images to registry
Day 16:    Update Supabase tool config -> All new scans use updated tools
```

#### Commit-Based Caching (Enhanced)

```typescript
async analyzePR(request: AnalysisRequest): Promise<AnalysisResult> {
  const cacheKey = `${request.repoUrl}:${request.prNumber}:${request.commitSha}`;
  const cached = await redis.get(cacheKey);
  if (cached) {
    return JSON.parse(cached);  // Instant!
  }
  const result = await this.runFullAnalysis(request);
  await redis.setex(cacheKey, 86400, JSON.stringify(result));  // 24h TTL
  return result;
}
```

#### Language Priority Roadmap

| Priority | Language | Market Share | Status |
|----------|----------|--------------|--------|
| P0 | Python | 29.87% | Ready |
| P0 | JavaScript | 20.95% | Ready |
| P0 | Java | 19.09% | Ready |
| P1 | TypeScript | 6.46% | Ready |
| P1 | Go | 4.96% | In Progress |
| P2 | Rust | 2.38% | Planned |
| P2 | PHP | 5.15% | Planned |

**Enterprise Languages (Deferred):** C# (5.73%), C++ (4.86%), C (4.21%)

### Key Decisions Summary

1. Two-Tier Model: BASIC (report) vs PRO (auto-fix)
2. 70% Overlap Threshold: REPLACE if better, ADD if complementary
3. 15 Tool Categories: Comprehensive coverage
4. Quarterly Research: Tools AND agents every 3 months
5. Pre-built Docker Images: Not runtime installation
6. Commit-Based Caching: Skip scan, return cached report
7. Two-Stage AI: Cheap model prepares, expensive model finalizes
8. Language Priority: Python, JS, TS, Java, Go first

---

## SESSION 31: Three-Tier Auto-Fix Architecture (November 28, 2025)

### Problem Identified
VSCode Extension auto-fix testing showed **36% failure rate** (104/288 fixes failed).
Root Cause: AI was generating fixes without access to actual source code.

### Solution: Three-Tier Fix System

| Tier | Source | Confidence | Coverage |
|------|--------|------------|----------|
| **Tier 1** | Tool Native Fixes | HIGH | ~60-70% |
| **Tier 2** | Dedicated Fixer Tools | HIGH | ~15-20% |
| **Tier 3** | AI Generation Fallback | MEDIUM | ~10-15% |

### Two Fix Delivery Modes

| Mode | What It Does | Target Users |
|------|--------------|--------------|
| **Fix During Scan** (Server-Side) | CHANGES code | ALL users via Web App, CI/CD |
| **IDE-Assisted Fix** (Client-Side) | RECOMMENDS only | VSCode users only |

### Cost Optimization

| Source | Token Cost | Coverage |
|--------|------------|----------|
| Tool Native Fix | $0 (FREE) | ~60-70% |
| Tool Metadata | $0 (FREE) | ~15-20% |
| AI Fallback | $$ | ~10-15% |

**Result:** ~85-90% cost reduction vs pure AI approach

### Tier 1: Tool Native Fixes
...
| Language | Fixer Tool | Fixes Issues From |
|----------|------------|-------------------|
| Python | autoflake | Pylint/Flake8 unused imports |
| | pyupgrade | Python 2 syntax |
| | isort | Import ordering |
| | Black | Formatting |
| Java | Sorald | SonarQube/PMD rules (25+) |
| | OpenRewrite | Checkstyle, migrations |
| Multi-lang | Renovate | Dependency vulnerabilities |

### Dynamic Model Selection (New Roles)

| Role | Quality | Cost | Speed | Use Case |
|------|---------|------|-------|----------|
| `issue-classifier` | 40% | 45% | 15% | Rule ID mapping |
| `fix-generator` | 60% | 25% | 15% | High-accuracy fix generation |

---

## SESSION 29: Monorepo Optimization & Tool Stability (November 21, 2025)
