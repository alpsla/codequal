...
import { toolFixRegistry, ToolFixCapability, FixTier } from '../fix-agent/tool-fix-registry';
...
export interface FixCapabilityInfo {
  toolId: string;
  tier: FixTier;
...
export interface ScannerToolGuidance {
  tool: string;
  category: 'Performance' | 'Architecture' | 'Security' | 'Dependencies' | 'Code Quality';
  whatYouGet: string[];
  howToFix: string[];
  resources: string[];
}
...
const SCANNER_TOOL_GUIDANCE: Record<string, ScannerToolGuidance> = {
  // Performance Tools
  'lighthouse': {
    tool: 'Lighthouse',
 ...
 */
export function getFixCapabilityInfo(toolId: string): FixCapabilityInfo {
  const toolLower = toolId.toLowerCase();
  const capability = toolFixRegistry.getToolCapability(toolLower);
 ...
 */
export function isGroupAutoFixable(group: IssueGroup): boolean {
 ...
 */
export function isIssueAutoFixable(issue: EnrichedIssue): boolean {
  ...
  // Determine fix tier
  let fixTier: 1 | 2 | 3 | 'recommendation' = 3;
  if (isRecommendation) fixTier = 'recommendation';
  else if (nativeFix) fixTier = 1;
  else if (['pmd', 'spotbugs'].includes(toolLower)) fixTier = 2;
 ...
 */
export function getScannerToolGuidance(toolId: string): ScannerToolGuidance | undefined {
  const toolLower = toolId.toLowerCase();
 ...
 */
export function getScannerToolsForCategory(category: string): ScannerToolGuidance[] {
  return Object.values(SCANNER_TOOL_GUIDANCE).filter(
    guidance => guidance.category === category
  );
}
 ...
 */
export function generateScannerValueSection(toolId: string, issueCount: number): string {
 ...
 */
export function getTierBadge(tier: FixTier): string {
  switch (tier) {
...
export function getConfidenceDescription(confidence: number): string {
  if (confidence >= 90) return 'Very High - Safe for auto-apply';
  if (confidence >= 70) return 'High - Review recommended';
  if (confidence >= 50) return 'Medium - Manual verification needed';
  if (confidence >= 30) return 'Low - AI guidance only';
...
function getValueProposition(capability: ToolFixCapability): string {
  if (capability.tier === 1 && capability.hasNativeFix) {
...
function getRemediationGuidance(capability: ToolFixCapability): string {
...
function isCommonAutoFixablePattern(rule: string): boolean {
 ...
 */
export function enrichIssueWithFixCapability(issue: EnrichedIssue): EnrichedIssue & {
  fixCapability: FixCapabilityInfo;
  scannerGuidance?: ScannerToolGuidance;
} {
 ...
 */
export function getFixSummaryStats(issues: EnrichedIssue[]): {
  total: number;
  tier1AutoFix: number;
  tier2FixerTool: number;
  tier3AiRequired: number;
