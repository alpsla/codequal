...
exports.getCuratedResourcesForRule = getCuratedResourcesForRule;
exports.enrichIssuesWithAI = enrichIssuesWithAI;
 ...
 */
const WHITELISTED_PATTERN_RULES = {
 ...
 */
function isPatternApplicableToIssue(patternCode, targetFile, issueContext, ruleId // SESSION 92: Added for whitelist check
) {
    var _a;
    ...
    const targetFileName = ((_a = targetFile.split('/').pop()) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
 ...
 */
function getCuratedResourcesForRule(ruleId) {
    const map = {
 ...
 */
async function enrichIssuesWithAI(issues, groups, modelConfigResolver, detectedLanguage = 'java', detectedRepoSize = 'medium') {
    var _a, _b, _c, _d;
    ...
    if (!modelConfigResolver) {
        ...
        const { getSupabasePatternStore } = await Promise.resolve().then(() => __importStar(require('../../fix-agent/fix-pattern-registry/supabase-pattern-store')));
        const { getRuleDescription } = await Promise.resolve().then(() => __importStar(require('../config/rule-descriptions')));
        const patternStore = getSupabasePatternStore();
        let patternsUsed = 0;
        let descriptionsUsed = 0;
        for (const group of groups) {
            const groupIssues = issues.filter(i => i.rule === group.rule && i.tool === group.tool && i.severity === group.severity);
            if (groupIssues.length === 0)
                continue;
            // STEP 1: Try to find a Supabase pattern first (richer fix info)
            let fixFromPattern = null;
            try {
                ...
                if (pattern) {
                    ...
                    const fixTemplate = ((_a = pattern.fixTemplate) === null || _a === void 0 ? void 0 : _a.template) || '';
                    const exampleAfter = ((_c = (_b = pattern.examples) === null || _b === void 0 ? void 0 : _b.find(ex => ex.after)) === null || _c === void 0 ? void 0 : _c.after) || '';
                    ...
                    const isCorrupted = corruptedPatterns.some(phrase => correctedCode.toLowerCase().includes(phrase) ||
                    ...
                    if (correctedCode && !isCorrupted && correctedCode.length > 10) {
                        ...
                        const cleanedPatternCode = (0, formatter_utils_1.cleanAIContent)(correctedCode);
                        ...
                        const targetFile = (representativeIssue === null || representativeIssue === void 0 ? void 0 : representativeIssue.file) || '';
                        // SESSION 92: Pass ruleId for whitelist check
                        const validation = isPatternApplicableToIssue(cleanedPatternCode, targetFile, representativeIssue === null || representativeIssue === void 0 ? void 0 : representativeIssue.snippet, group.rule // SESSION 92: Enable whitelist check
                        );
                ...
                console.log(`[AI Enrichment] Pattern lookup skipped for ${group.rule}: ${err.message}`);
            }
            ...
            for (const issue of groupIssues) {
                const existingCorrectedCode = (_d = issue.fixSuggestion) === null || _d === void 0 ? void 0 : _d.correctedCode;
                ...
                const cleanedCorrectedCode = (0, formatter_utils_1.cleanAIContent)(rawCorrectedCode);
                issue.fixSuggestion = {
    ...
    const tokensByAgent = {};
    try {
        const { SpecializedAgentFactory } = await Promise.resolve().then(() => __importStar(require('../agents/specialized-agents')));
        ...
        const enrichmentPromises = groups.map(async (group) => {
            ...
            try {
                ...
                const fixSuggestion = await SpecializedAgentFactory.generateFixForIssue(issueContext, modelConfigResolver, detectedLanguage, detectedRepoSize);
                ...
                // Apply fix to ALL issues in this group
                // BUG-LSP-001 FIX: Clean correctedCode to remove template patterns
                const cleanedAICorrectedCode = (0, formatter_utils_1.cleanAIContent)(fixSuggestion.correctedCode);
                for (const issue of groupIssues) {
                    issue.fixSuggestion = {
 ...
 */
function normalizeDependencyVersions(issues, groups) {
    var _a, _b, _c, _d;
    ...
    const extractPackageName = (issue) => {
    ...
    const extractVersion = (correctedCode) => {
        if (!correctedCode)
            return null;
    ...
    for (const issue of issues) {
        if (issue.tool !== 'npm-audit' || !((_a = issue.fixSuggestion) === null || _a === void 0 ? void 0 : _a.correctedCode))
            continue;
    ...
    for (const [packageName, pkgGroup] of packageGroups.entries()) {
        ...
        const prefix = ((_b = pkgGroup.versions.find(v => v.startsWith('^'))) === null || _b === void 0 ? void 0 : _b.charAt(0)) ||
            ((_c = pkgGroup.versions.find(v => v.startsWith('~'))) === null || _c === void 0 ? void 0 : _c.charAt(0)) || '';
        ...
        for (const issue of pkgGroup.issues) {
            if ((_d = issue.fixSuggestion) === null || _d === void 0 ? void 0 : _d.correctedCode) {
                const oldCode = issue.fixSuggestion.correctedCode;
                // Replace version in correctedCode
                const updatedCode = oldCode.replace(/"([^"]+)":\s*"([^"]+)"/, (match, pkg, version) => {
