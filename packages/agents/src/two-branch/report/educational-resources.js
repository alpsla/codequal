...
exports.generateEducationalResources = generateEducationalResources;
exports.generateEducationalResourcesBrave = generateEducationalResourcesBrave;
const formatter_utils_1 = require("./formatter-utils");
const ai_enrichment_1 = require("./ai-enrichment");
const documentation_links_1 = require("./documentation-links");
 ...
 */
function extractCVEId(ruleId, title, description) {
    const cvePattern = /CVE-\d{4}-\d{4,}/i;
    // Check rule ID first
    const ruleMatch = ruleId.match(cvePattern);
    if (ruleMatch)
        return ruleMatch[0].toUpperCase();
    // Check title
 ...
 */
function generateEducationalResources(issues, language = 'java') {
...
`;
        ...
        for (const issue of issues) {
            const key = issue.rule || issue.type || 'unknown';
        ...
        for (const [ruleId, { tool, count }] of sortedRules) {
            const docLinks = (0, documentation_links_1.getDocumentationLinks)(ruleId, tool);
            if (docLinks.length > 0) {
                hasToolDocs = true;
                content += `\n**${ruleId}** (${count} occurrence${count > 1 ? 's' : ''}):\n`;
                content += (0, documentation_links_1.formatDocumentationLinksAsMarkdown)(docLinks, 2) + '\n';
            }
        }
        // If no tool-specific docs found, show tool reference pages
        if (!hasToolDocs) {
            const tools = new Set(issues.map(i => { var _a; return (_a = i.tool) === null || _a === void 0 ? void 0 : _a.toLowerCase(); }).filter(Boolean));
            for (const toolName of tools) {
                const toolInfo = documentation_links_1.TOOL_DOCUMENTATION[toolName];
                if (toolInfo === null || toolInfo === void 0 ? void 0 : toolInfo.rulesUrl) {
 ...
 * @param language - Programming language (python, java, typescript, go)
 */
async function generateEducationalResourcesBrave(issues, language = 'java') {
    var _a;
    const resources = getLanguageResources(language);
    ...
    const blockerIssues = issues.filter(i => (i.category === 'NEW' || i.category === 'EXISTING_MODIFIED') &&
        (i.severity === 'critical' || i.severity === 'high'));
    // Rest critical/high: EXISTING_REST + critical/high (not blockers but still important)
    const restCriticalHighIssues = issues.filter(i => i.category === 'EXISTING_REST' &&
        (i.severity === 'critical' || i.severity === 'high'));
    ...
    if (blockerIssues.length > 0) {
        ...
        const blockerFreq = new Map();
        for (const i of blockerIssues)
            blockerFreq.set(i.rule, (blockerFreq.get(i.rule) || 0) + 1);
        ...
        for (const ruleId of blockerRules) {
            ...
            const title = (0, formatter_utils_1.getUserFriendlyTitle)(ruleId, sample ? sample.tool : '');
            // BUG-090 FIX: Use passed language parameter instead of hardcoded 'Java'
            const issueLanguage = (sample && sample.language) ? sample.language : language;
            const count = blockerFreq.get(ruleId) || 0;
            const description = (sample && sample.description) ? sample.description : undefined;
            ...
            if (cveId) {
                ...
                if (docLinks.length > 0) {
                    ...
                    content += (0, documentation_links_1.formatDocumentationLinksAsMarkdown)(docLinks, 3) + '\n';
                }
                else {
                    // Try curated resources first
                    const curated = (0, ai_enrichment_1.getCuratedResourcesForRule)(ruleId);
                    if (curated.length > 0) {
                        ...
                        const category = (sample === null || sample === void 0 ? void 0 : sample.detectedCategory) || 'Code Quality';
                        const fallbackLinks = (0, documentation_links_1.getFallbackDocumentation)(category, issueLanguage);
                        content += (0, documentation_links_1.formatDocumentationLinksAsMarkdown)(fallbackLinks, 2) + '\n';
                    }
                }
            }
            content += `\n`;
    ...
    if (restCriticalHighIssues.length > 0) {
        ...
        for (const i of restCriticalHighIssues)
            restFreq.set(i.rule, (restFreq.get(i.rule) || 0) + 1);
    ...
    if (lowIssues.length > 0) {
        ...
        for (const issue of lowIssues) {
            const tool = ((_a = issue.tool) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || 'other';
