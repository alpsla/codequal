...
exports.generateAnalysisMetadata = generateAnalysisMetadata;
...
exports.groupByTool = groupByTool;
/**
 ...
 */
function generateAnalysisMetadata(metadata, showAgentPerformance = true, showToolPerformance = true, showEfficiencyAnalysis = true, showSystemInfo = true) {
...
`;
    ...
    if (showToolPerformance && metadata.toolPerformance && Array.isArray(metadata.toolPerformance) && metadata.toolPerformance.length > 0) {
        ...
        const actuallyRanTools = metadata.toolPerformance.filter((tool) => {
 ...
 */
function getPersonalizedGreeting(author) {
    return 'Hi';
}
/**
 * Get personalized encouragement based on issue counts
 */
function getPersonalizedEncouragement(blockingCount, resolvedCount) {
    if (resolvedCount > 10) {
 ...
 */
function generatePRComment(issues, groups, metadata) {
...
### 💡 Quick Stats
- Auto-fixable: ${issues.filter(i => canAutoFix({ rule: i.rule, tool: i.tool, severity: i.severity })).length}/${issues.length} issues (${groups.filter(g => canAutoFix(g)).length}/${groups.length} types)
 ...
 */
function generateFooter(groups, ideFixFiles, metadata, enrichedIssues) {
    ...
    if (ideFixFiles.length > 0) {
        ...
        const manifestUrl = manifestFile ? manifestFile.publicUrl : null;
        ...
        if (enrichedIssues && enrichedIssues.length > 0) {
            ...
            const fixableIssues = enrichedIssues.filter(issue => { var _a; return (_a = issue.fixSuggestion) === null || _a === void 0 ? void 0 : _a.correctedCode; });
            ...
            criticalCount = issueFiles.filter(f => f.content.severity === 'critical').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            highCount = issueFiles.filter(f => f.content.severity === 'high').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            mediumCount = issueFiles.filter(f => f.content.severity === 'medium').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            lowCount = issueFiles.filter(f => f.content.severity === 'low').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
        }
        ...
        if (metadata === null || metadata === void 0 ? void 0 : metadata.lspUrl) {
        ...
        if (criticalCount > 0)
            footer += `- 🔴 **"Apply Critical Severity Fixes"** - ${criticalCount} issues\n`;
        if (highCount > 0)
            footer += `- 🟠 **"Apply High Severity Fixes"** - ${highCount} issues\n`;
        if (mediumCount > 0)
            footer += `- 🟡 **"Apply Medium Severity Fixes"** - ${mediumCount} issues\n`;
        if (lowCount > 0)
            footer += `- 🟢 **"Apply Low Severity Fixes"** - ${lowCount} issues\n`;
        ...
        // BUG FIX: Only show URL if file was actually uploaded (prevents 404)
        if (metadata === null || metadata === void 0 ? void 0 : metadata.sarifUrl) {
        ...
        if (metadata === null || metadata === void 0 ? void 0 : metadata.gitlabUrl) {
            // Detect if this is a GitLab repo for customized messaging
            const repoUrl = (metadata === null || metadata === void 0 ? void 0 : metadata.repositoryUrl) || (metadata === null || metadata === void 0 ? void 0 : metadata.repository) || '';
