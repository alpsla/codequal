...
exports.TOOL_DOCUMENTATION = void 0;
exports.getDocumentationLinks = getDocumentationLinks;
exports.getFallbackDocumentation = getFallbackDocumentation;
exports.formatDocumentationLinksAsMarkdown = formatDocumentationLinksAsMarkdown;
/**
 * Tool-level documentation base URLs
 */
exports.TOOL_DOCUMENTATION = {
    // Python Tools
    bandit: {
        name: 'Bandit',
 ...
 */
function generatePMDDocUrl(ruleId) {
...
const CHECKSTYLE_CATEGORY_PATHS = {
 ...
 */
function generateSpotBugsDocUrl(ruleId) {
 ...
 */
function generatePylintDocUrl(ruleId) {
 ...
 */
const CHECKSTYLE_RULE_CATEGORIES = {
    // Annotation checks
 ...
 */
function generateCheckstyleDocUrl(ruleId) {
 ...
 */
function generateGolangciLintDocUrl(ruleId) {
 ...
 */
function generateRuboCopDocUrl(ruleId) {
 ...
 */
function generateClippyDocUrl(ruleId) {
 ...
 */
function generatePHPStanDocUrl(ruleId) {
    return [
 ...
 */
function generatePsalmDocUrl(ruleId) {
 ...
 */
function getDocumentationLinks(ruleId, tool) {
    ...
    switch (normalizedTool) {
        case 'bandit':
            ...
            if (BANDIT_RULES[ruleId])
                return BANDIT_RULES[ruleId];
        ...
        case 'semgrep':
            ...
            if (SEMGREP_RULES[ruleId])
                return SEMGREP_RULES[ruleId];
        ...
        case 'checkstyle': {
            ...
            if (CHECKSTYLE_RULES[ruleId])
                return CHECKSTYLE_RULES[ruleId];
            if (CHECKSTYLE_RULES[`${ruleName}Check`])
                return CHECKSTYLE_RULES[`${ruleName}Check`];
            // Try partial match
        ...
        case 'mypy':
            if (MYPY_RULES[ruleId])
                return MYPY_RULES[ruleId];
        ...
        case 'madge':
            if (ARCHITECTURE_TOOL_RULES['circular-dependency'])
                return ARCHITECTURE_TOOL_RULES['circular-dependency'];
            break;
        ...
        case 'lighthouse':
            if (PERFORMANCE_TOOL_RULES[normalizedRule])
                return PERFORMANCE_TOOL_RULES[normalizedRule];
 ...
 */
function getFallbackDocumentation(category, language = 'python') {
 ...
 */
function formatDocumentationLinksAsMarkdown(links, maxLinks = 3) {
    const iconMap = {
        official: '📚',
        owasp: '🛡️',
        cwe: '🔒',
