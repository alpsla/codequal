...
exports.V9GroupedReportFormatter = void 0;
2; /**
...
const v9_app_score_manager_1 = require("./v9-app-score-manager");
const v9_skill_score_manager_1 = require("./v9-skill-score-manager");
const lsp_sarif_converter_1 = require("./lsp-sarif-converter");
const gitlab_codequality_converter_1 = require("./gitlab-codequality-converter");
const issue_classifier_1 = require("../../fix-agent/issue-classifier");
const ai_fix_prompts_1 = require("../../fix-agent/ai-fix-prompts");
// Load environment variables
dotenv.config();
// Service Imports for Delegation Pattern
const formatter_utils_1 = require("../report/formatter-utils");
const ai_enrichment_1 = require("../report/ai-enrichment");
const category_detector_1 = require("../report/category-detector");
const educational_resources_1 = require("../report/educational-resources");
const business_impact_1 = require("../report/business-impact");
const metadata_footer_1 = require("../report/metadata-footer");
const header_sections_1 = require("../report/header-sections");
const score_calculator_1 = require("../report/score-calculator");
const achievements_1 = require("../report/achievements");
const community_impact_1 = require("../report/community-impact");
// ================================================================
// V9 Grouped Report Formatter
// ================================================================
class V9GroupedReportFormatter {
    /**
    ...
    cleanCorrectedCode(code) {
        ...
        if (codeLines.length > 50) {
            ...
            const firstCodeLine = codeLines.findIndex((line, idx) => idx > 0 &&
     ...
     */
    hasActualCodeFix(issue) {
        return this.getFixType(issue) === 'code';
    }
    /**
     * SESSION 92 FIX: Check if issue has text guidance (but no actual code)
     */
    hasTextGuidanceOnly(issue) {
        return this.getFixType(issue) === 'guidance';
    }
    constructor(modelConfigResolver, language, repoSize) {
        this.supabase = null;
        this.appScoreManager = null;
        this.SHOW_PERF_SUBMETRICS = false;
        this.skillScoreManager = null;
        this.repoPath = undefined; // Local repo path for snippet extraction
        this.repositoryUrl = undefined; // SESSION 74: GitHub URL for remote snippet fetching
        this.userTier = 'basic'; // User tier for report differentiation
        // BUG-76: AI enrichment dependencies
        this.modelConfigResolver = null;
        this.detectedLanguage = 'java';
        this.detectedRepoSize = 'medium';
        this.serviceHealthTracker = null; // ServiceHealthTracker for monitoring
        // Feature toggles for optional sections
        this.SHOW_FIX_COVERAGE = false;
        this.SHOW_QUICK_WINS = false;
        this.SHOW_SYSTEM_INFO = false;
        this.SHOW_AGENT_PERFORMANCE = false; // BUG-110: Disabled - not useful for users
        this.SHOW_TOOL_PERFORMANCE = true; // BUG #8 FIX: Enable tool performance tracking
        this.SHOW_EFFICIENCY_ANALYSIS = true; // BUG #10 FIX: Enable cost analysis
        this.SHOW_FOCUS_AREAS = false; // BUG-110: Disabled - not useful for users
        // BUG-105 FIX: Cached pattern count (fetched from Supabase once per session)
        this.cachedPatternCount = null;
        ...
        try {
            ...
            if (supabaseUrl && supabaseKey) {
                ...
                this.appScoreManager = new v9_app_score_manager_1.AppScoreManager(this.supabase);
                this.skillScoreManager = new v9_skill_score_manager_1.SkillScoreManager(this.supabase);
     ...
     */
    async initializeServiceHealthTracker() {
        if (!this.serviceHealthTracker && this.supabase) {
            try {
                const { ServiceHealthTracker } = await Promise.resolve().then(() => __importStar(require('../monitoring/service-health-tracker')));
     ...
     */
    async uploadWithRetry(filePath, content, options, maxRetries = 3, retryDelay = 1000) {
        var _a;
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const errorCode = ((_a = networkError === null || networkError === void 0 ? void 0 : networkError.cause) === null || _a === void 0 ? void 0 : _a.code) || (networkError === null || networkError === void 0 ? void 0 : networkError.code);
    ...
    async uploadAttachmentsToSupabase(ideFixFiles, metadata, analysisTimestamp // BUG-DOG-04 FIX: Use consistent timestamp across all uploads
    ) {
        var _a;
        ...
        const repoName = ((_a = metadata.repository) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || 'unknown';
        ...
        const uploadSingleFile = async (file) => {
            try {
                ...
                const { data, error } = await this.uploadWithRetry(filePath, fileContent, {
                ...
                updatedFile.publicUrl = urlData.publicUrl;
        ...
        const successCount = updatedFiles.filter(f => f.publicUrl).length;
     ...
     */
    getCuratedResourcesForRule(ruleId) {
        return (0, ai_enrichment_1.getCuratedResourcesForRule)(ruleId);
    }
    /**
    ...
    async enrichIssuesWithAI(issues, groups) {
        return (0, ai_enrichment_1.enrichIssuesWithAI)(issues, groups, this.modelConfigResolver, // null = rule descriptions, non-null = AI
        this.detectedLanguage, this.detectedRepoSize);
    }
    /**
     * Generate grouped report with attachments
     */
    async generateGroupedReport(issues, groups, metadata) {
        var _a;
        ...
        const markdown = [];
        let ideFixFiles = []; // BUG FIX #33: Removed separate location attachments
        ...
        const toolsExecuted = ((_a = metadata.toolPerformance) === null || _a === void 0 ? void 0 : _a.length) || 0;
        if (toolsExecuted === 0) {
        ...
        this.userTier = metadata.userTier || 'basic';
        ...
        if (enrichedIssues.length === 0 && issues.length > 0) {
            ...
            enrichedIssues.push(...issues.map(issue => ({
                ...
                category: (issue.category || 'EXISTING_REST'),
        ...
        if (metadata.agentPerformance && Array.isArray(metadata.agentPerformance)) {
            ...
            metadata.agentPerformance.forEach((agent) => {
        ...
        const updatedGroups = groups.map(group => {
            ...
            const groupIssues = enrichedIssues.filter(issue => issue.rule === group.rule &&
            ...
            if (groupIssues.length === 0) {
                ...
                const fallbackIssues = enrichedIssues.filter(issue => issue.rule === group.rule && issue.tool === group.tool);
        ...
        const updatedBlockingCount = enrichedIssues.filter(i => (i.category === 'NEW' || i.category === 'EXISTING_MODIFIED') &&
        ...
        if (ideFixFiles.length > 0) {
            ...
            const enrichManifestEntry = (f) => {
                var _a, _b;
                const uploadedFile = uploadedFixFiles.find(uf => uf.filename === f.filename);
                const publicUrl = uploadedFile && uploadedFile.publicUrl
                ...
                return {
                    ...
                    occurrences: ((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || ((_b = f.content.locations) === null || _b === void 0 ? void 0 : _b.length) || 0,
                    autoFixable: this.canAutoFix({ rule: f.content.rule, tool: f.content.tool, severity: f.content.severity })
                };
            };
            const manifestFile = {
                groupId: 'all-issues',
            ...
            // Log summary
            const totalWithUrls = ideFixFiles.filter(f => f.publicUrl).length;
            ...
            if (lspUrl)
                metadata.lspUrl = lspUrl;
            if (sarifUrl)
                metadata.sarifUrl = sarifUrl;
            if (gitlabUrl)
                metadata.gitlabUrl = gitlabUrl;
        }
     ...
     */
    async findFullPath(basename) {
        if (!this.repoPath || basename.includes('/')) {
            // Already has path or no repo available
            return null;
        }
        try {
            const result = (0, child_process_1.execSync)(`find "${this.repoPath}" -name "${basename}" -type f 2>/dev/null | head -1`, { encoding: 'utf-8', timeout: 5000 });
     ...
     */
    async extractSnippetsForLocations(issues) {
        if (!this.repoPath) {
        ...
        const { CodeSnippetExtractor } = await Promise.resolve().then(() => __importStar(require('../utils/code-snippet-extractor')));
        ...
        for (let i = 0; i < issues.length; i++) {
            ...
            if (i < SNIPPET_LIMIT && (!snippet || snippet === 'N/A' || snippet.trim().length === 0) && issue.file && issue.line) {
                try {
                    ...
                    snippet = await CodeSnippetExtractor.extractSnippet(fullPath, issue.line, 3, this.repositoryUrl // GitHub fallback URL
                    ) || '';
                }
                catch (error) {
    ...
    deduplicateLocations(locations) {
        ...
        for (const location of locations) {
            ...
            if (!seen.has(key) || (location.snippet && location.snippet.length > (((_b = (_a = seen.get(key)) === null || _a === void 0 ? void 0 : _a.snippet) === null || _b === void 0 ? void 0 : _b.length) || 0))) {
    ...
    generateHeader(metadata) {
        return (0, header_sections_1.generateHeader)(metadata, this.SHOW_PERF_SUBMETRICS);
    }
    /**
     * Generate error report when tool orchestration fails
     * (FALSE POSITIVE BUG FIX)
     */
    generateAnalysisFailureReport(metadata, errorMessage) {
...
`;
    ...
    _REMOVED_generateHeader_LEGACY(metadata) {
...
---`;
    ...
    formatDate(dateString) {
        return (0, formatter_utils_1.formatDate)(dateString);
    }
    /**
     * Format duration in milliseconds to human-readable string
     */
    formatDuration(durationMs) {
        return (0, formatter_utils_1.formatDuration)(durationMs);
    }
    /**
     * Calculate impact score from issues
     * Critical: 5 points, High: 3 points, Medium: 1 point, Low: 0.5 points
     */
    calculateImpact(issues) {
        let impact = 0;
     ...
     */
    async calculateQualityScore(issues, metadata) {
        return (0, score_calculator_1.calculateQualityScore)(issues, metadata, this.appScoreManager, this.skillScoreManager);
    }
    /**
     ...
     */
    async checkCachedScoresForCommit(metadata) {
        return (0, score_calculator_1.checkCachedScoresForCommit)(metadata, this.appScoreManager, this.skillScoreManager);
    }
    async _REMOVED_checkCachedScoresForCommit_LEGACY(metadata) {
        var _a, _b, _c, _d, _e;
        if (!this.appScoreManager || !this.skillScoreManager || !metadata.commitSHA) {
            return null;
        }
        try {
            const supabase = this.appScoreManager.supabase;
            ...
            if (appScore && skillScore) {
                ...
                const categoryScores = {
                    security: (_a = appScore.security_score) !== null && _a !== void 0 ? _a : 50,
                    performance: (_b = appScore.performance_score) !== null && _b !== void 0 ? _b : 50,
                    architecture: (_c = appScore.architecture_score) !== null && _c !== void 0 ? _c : 50,
                    dependency: (_d = appScore.dependency_score) !== null && _d !== void 0 ? _d : 50,
                    codeQuality: (_e = appScore.code_quality_score) !== null && _e !== void 0 ? _e : 50
                };
                // Determine grade
     ...
     */
    async calculateFullV9Score(issues, metadata) {
        return (0, score_calculator_1.calculateFullV9Score)(issues, metadata, this.appScoreManager, this.skillScoreManager);
    }
    async _REMOVED_calculateFullV9Score_LEGACY(issues, metadata) {
        var _a, _b;
        try {
            ...
            const skillScore = Math.round((categoryScores.security + categoryScores.performance + categoryScores.architecture +
            ...
            if (this.appScoreManager && metadata.repository) {
                console.log(`[V9ReportFormatter] 💾 Saving APP score: ${appScore}/100 for ${metadata.repository} PR #${metadata.prNumber || 0} (commit: ${((_a = metadata.commitSHA) === null || _a === void 0 ? void 0 : _a.slice(0, 7)) || 'unknown'})`);
                const supabase = this.appScoreManager.supabase;
            ...
            if (this.skillScoreManager && metadata.prAuthorEmail && metadata.repository) {
                console.log(`[V9ReportFormatter] 💾 Saving Skill score: ${skillScore}/100 for ${metadata.prAuthorEmail} PR #${metadata.prNumber || 0} (commit: ${((_b = metadata.commitSHA) === null || _b === void 0 ? void 0 : _b.slice(0, 7)) || 'unknown'})`);
                const supabase = this.skillScoreManager.supabase;
     ...
     */
    calculateCategoryScore(categoryIssues, baseScore = 100) {
        ...
        return (0, score_calculator_1.calculateCategoryScore)(categoryIssues, baseScore);
    }
    _REMOVED_calculateCategoryScore_LEGACY(categoryIssues) {
     ...
     */
    calculateSimplifiedScore(issues) {
        return (0, score_calculator_1.calculateSimplifiedScore)(issues);
    }
    _REMOVED_calculateSimplifiedScore_LEGACY(issues) {
    ...
    getScoreInterpretation(score) {
        return (0, score_calculator_1.getScoreInterpretation)(score);
    }
    _REMOVED_getScoreInterpretation_LEGACY(score) {
        if (score >= 90) {
            return {
                emoji: '🏆',
                label: 'Excellent',
     ...
     */
    async generateExecutiveSummary(issues, groups, metadata) {
        var _a, _b, _c;
        ...
        const blockingIssues = issues.filter(i => (i.category === 'NEW' || i.category === 'EXISTING_MODIFIED') &&
        ...
        const autoFixableIssues = issuesNeedingFixes.filter(i => safeAutoApplyGroups.some(g => g.rule === i.rule && g.tool === i.tool && g.severity === i.severity));
        ...
        const technicallyAutoFixableIssues = issuesNeedingFixes.filter(i => autoFixableGroups.some(g => g.rule === i.rule && g.tool === i.tool && g.severity === i.severity));
...
- NEW issues: ${((_a = qualityResult.breakdown.newIssuesDeduction) === null || _a === void 0 ? void 0 : _a.toFixed(1)) || '0.0'} (${issues.filter(i => i.category === 'NEW').length} issues)
  _→ Issues introduced in this PR_

- EXISTING_MODIFIED issues: ${((_b = qualityResult.breakdown.existingModifiedDeduction) === null || _b === void 0 ? void 0 : _b.toFixed(1)) || '0.0'} (${issues.filter(i => i.category === 'EXISTING_MODIFIED').length} issues)
  _→ Pre-existing issues in modified files_

- EXISTING_REST issues: ${((_c = qualityResult.breakdown.existingRestDeduction) === null || _c === void 0 ? void 0 : _c.toFixed(1)) || '0.0'} (${issues.filter(i => i.category === 'EXISTING_REST').length} issues)
...
|----------|----------|------|--------|-----|-------|-------|
| 🔒 Security | ${byDetectedCategory['Security'].critical} | ${byDetectedCategory['Security'].high} | ${byDetectedCategory['Security'].medium} | ${byDetectedCategory['Security'].low} | **${byDetectedCategory['Security'].total}** | **${(_e = (_c = (_b = (_a = qualityResult.breakdown) === null || _a === void 0 ? void 0 : _a.categoryScores) === null || _b === void 0 ? void 0 : _b.security) !== null && _c !== void 0 ? _c : (_d = qualityResult.categoryScores) === null || _d === void 0 ? void 0 : _d.security) !== null && _e !== void 0 ? _e : 'N/A'}/100** |
| ⚡ Performance | ${byDetectedCategory['Performance'].critical} | ${byDetectedCategory['Performance'].high} | ${byDetectedCategory['Performance'].medium} | ${byDetectedCategory['Performance'].low} | **${byDetectedCategory['Performance'].total}** | **${(_k = (_h = (_g = (_f = qualityResult.breakdown) === null || _f === void 0 ? void 0 : _f.categoryScores) === null || _g === void 0 ? void 0 : _g.performance) !== null && _h !== void 0 ? _h : (_j = qualityResult.categoryScores) === null || _j === void 0 ? void 0 : _j.performance) !== null && _k !== void 0 ? _k : 'N/A'}/100** |
| 🏗️ Architecture | ${byDetectedCategory['Architecture'].critical} | ${byDetectedCategory['Architecture'].high} | ${byDetectedCategory['Architecture'].medium} | ${byDetectedCategory['Architecture'].low} | **${byDetectedCategory['Architecture'].total}** | **${(_q = (_o = (_m = (_l = qualityResult.breakdown) === null || _l === void 0 ? void 0 : _l.categoryScores) === null || _m === void 0 ? void 0 : _m.architecture) !== null && _o !== void 0 ? _o : (_p = qualityResult.categoryScores) === null || _p === void 0 ? void 0 : _p.architecture) !== null && _q !== void 0 ? _q : 'N/A'}/100** |
| 📦 Dependencies | ${byDetectedCategory['Dependencies'].critical} | ${byDetectedCategory['Dependencies'].high} | ${byDetectedCategory['Dependencies'].medium} | ${byDetectedCategory['Dependencies'].low} | **${byDetectedCategory['Dependencies'].total}** | **${(_v = (_t = (_s = (_r = qualityResult.breakdown) === null || _r === void 0 ? void 0 : _r.categoryScores) === null || _s === void 0 ? void 0 : _s.dependency) !== null && _t !== void 0 ? _t : (_u = qualityResult.categoryScores) === null || _u === void 0 ? void 0 : _u.dependency) !== null && _v !== void 0 ? _v : 'N/A'}/100** |
| ✨ Code Quality | ${byDetectedCategory['Code Quality'].critical} | ${byDetectedCategory['Code Quality'].high} | ${byDetectedCategory['Code Quality'].medium} | ${byDetectedCategory['Code Quality'].low} | **${byDetectedCategory['Code Quality'].total}** | **${(_0 = (_y = (_x = (_w = qualityResult.breakdown) === null || _w === void 0 ? void 0 : _w.categoryScores) === null || _x === void 0 ? void 0 : _x.codeQuality) !== null && _y !== void 0 ? _y : (_z = qualityResult.categoryScores) === null || _z === void 0 ? void 0 : _z.codeQuality) !== null && _0 !== void 0 ? _0 : 'N/A'}/100** |
...
${await this.generateTrendsAndRecommendations(issues, metadata)}`;
     ...
     */
    generateKeyFindings(issues, groups, blockingIssues) {
        return (0, header_sections_1.generateKeyFindings)(issues, groups, blockingIssues);
    }
    _REMOVED_generateKeyFindings_LEGACY(issues, groups, blockingIssues) {
        const findings = [];
     ...
     */
    async generateCriticalBlockers(groups, blockingIssues, repoPath) {
        return await (0, header_sections_1.generateCriticalBlockers)(groups, blockingIssues, repoPath);
    }
    _REMOVED_generateCriticalBlockers_LEGACY(groups, blockingIssues) {
        ...
        const severityWeight = (sev) => sev === 'critical' ? 100 : sev === 'high' ? 60 : 0;
        const categoryWeight = (cat) => {
     ...
     */
    generateQuickWins(groups, autoFixableGroups) {
        return (0, header_sections_1.generateQuickWins)(groups, autoFixableGroups);
    }
    _REMOVED_generateQuickWins_LEGACY(groups, autoFixableGroups) {
     ...
     */
    async generateTrendsAndRecommendations(issues, metadata) {
        let content = '';
        ...
        if (this.skillScoreManager && metadata.prAuthorEmail) {
            try {
                ...
                if (history && history.length > 1 && !history.every((v) => v === history[0])) {
        ...
        const autoFixableIssues = activeIssues.filter(i => this.isSafeToAutoApply({ rule: i.rule, tool: i.tool, severity: i.severity }));
        ...
        const autoFixableCount = issues.filter(i => this.canAutoFix({ rule: i.rule, tool: i.tool, severity: i.severity })).length;
        if (this.userTier === 'pro' || this.userTier === 'enterprise') {
     ...
     */
    getUserFriendlyTitle(rule, tool) {
        return (0, formatter_utils_1.getUserFriendlyTitle)(rule, tool);
    }
    /**
    ...
    detectCategory(rule, tool, message) {
        return (0, category_detector_1.detectCategory)(rule, tool, message);
    }
    /**
     ...
     */
    calculateRiskLevel(category, severity, rule) {
        return (0, category_detector_1.calculateRiskLevel)(category, severity, rule);
    }
    /**
     * Get category-specific context and guidance
     * Phase E: Category-aware recommendations
     */
    getCategoryContext(category, severity) {
        return (0, category_detector_1.getCategoryContext)(category, severity);
    }
    /**
     * Get priority guidance for fixing issues
     * Phase E: Priority-based action plan
     */
    getPriorityGuidance(category, severity, count, riskLevel) {
        return (0, category_detector_1.getPriorityGuidance)(category, severity, count, riskLevel);
    }
    /**
     ...
     */
    getIssueDescription(rule, tool, severity, message) {
     ...
     */
    getGenericFixGuidance(rule, tool, severity) {
...
5. Consider using IDE plugins for ${tool} to get inline suggestions`;
     ...
     */
    async generateGroupSection(group, allIssues, expanded, ideFixFiles) {
        ...
        const groupIssues = allIssues.filter(i => i.rule === group.rule && i.tool === group.tool && i.severity === group.severity);
        ...
        const representativeWithAI = groupIssues.find(i => { var _a; return (_a = i.fixSuggestion) === null || _a === void 0 ? void 0 : _a.issueDescription; }) || representative;
        let issueDesc;
        if ((_a = representativeWithAI === null || representativeWithAI === void 0 ? void 0 : representativeWithAI.fixSuggestion) === null || _a === void 0 ? void 0 : _a.issueDescription) {
            ...
            const representativeMessage = (representative === null || representative === void 0 ? void 0 : representative.message) || group.description;
        ...
        section += `**Category**: ${(representative === null || representative === void 0 ? void 0 : representative.category) || group.category}`;
        ...
        // Phase E: Category-specific enhancements
        const detectedCategory = this.detectCategory(group.rule, group.tool, (representative === null || representative === void 0 ? void 0 : representative.message) || group.description);
        ...
        let exampleIssue = representative;
        // First, try to find an issue with an existing snippet
        if (!(exampleIssue === null || exampleIssue === void 0 ? void 0 : exampleIssue.snippet) || exampleIssue.snippet === 'N/A' || exampleIssue.snippet.trim().length === 0) {
        ...
        if (!(exampleIssue === null || exampleIssue === void 0 ? void 0 : exampleIssue.snippet) || exampleIssue.snippet === 'N/A' || exampleIssue.snippet.trim().length === 0) {
            ...
            exampleIssue = groupIssues.find(i => i.file && i.line &&
                !i.file.includes('_jmhTest') &&
                !i.file.includes('generated')) || groupIssues.find(i => !!i.file && !!i.line) || representative;
        }
        if (exampleIssue === null || exampleIssue === void 0 ? void 0 : exampleIssue.file) {
            ...
            if ((!snippet || snippet === 'N/A' || snippet.trim().length === 0) && exampleIssue.file && exampleIssue.line) {
                ...
                try {
                    ...
                    // SESSION 74: Try local file first, then GitHub API fallback
                    snippet = await CodeSnippetExtractor.extractSnippet(fullPath, exampleIssue.line, 3, this.repositoryUrl // GitHub fallback URL
                    ) || undefined;
            ...
            else if (((_b = representative === null || representative === void 0 ? void 0 : representative.fixSuggestion) === null || _b === void 0 ? void 0 : _b.correctedCode) && typeof representative.fixSuggestion.correctedCode === 'string') {
        ...
        if (expanded) {
            ...
            const hasValidSnippet = (representative === null || representative === void 0 ? void 0 : representative.snippet) && representative.snippet !== 'N/A' && representative.snippet.trim().length > 0;
            const hasValidFix = ((_c = representative === null || representative === void 0 ? void 0 : representative.fixSuggestion) === null || _c === void 0 ? void 0 : _c.correctedCode) && typeof representative.fixSuggestion.correctedCode === 'string' && representative.fixSuggestion.correctedCode.trim().length > 0;
            if (representative === null || representative === void 0 ? void 0 : representative.fixSuggestion) {
                    ...
                    if (cleanCorrectedCode && cleanCorrectedCode.length >= 20) {
                        ...
                        if (this.userTier === 'pro' || this.userTier === 'enterprise') {
                            ...
                            if (hasValidSnippet) {
                                ...
                                const exampleFileName = ((_e = (_d = exampleIssue === null || exampleIssue === void 0 ? void 0 : exampleIssue.file) === null || _d === void 0 ? void 0 : _d.split('/').pop()) === null || _e === void 0 ? void 0 : _e.replace('.java', '').replace('.ts', '').replace('.py', '')) || '';
                                ...
                                if (codeMatchesFile) {
                                    ...
                                    const language = (representative === null || representative === void 0 ? void 0 : representative.file) ? this.getLanguageFromFile(representative.file) : 'text';
                                    section += `\`\`\`${language}\n`;
            ...
            if (((_f = representative === null || representative === void 0 ? void 0 : representative.fixSuggestion) === null || _f === void 0 ? void 0 : _f.bestPractices) && representative.fixSuggestion.bestPractices.length > 0) {
    ...
    cleanAIContent(content) {
        return (0, formatter_utils_1.cleanAIContent)(content);
    }
    /**
     ...
     */
    generateCheckStyleAutoFixGuide(issueCount) {
...
`;
     ...
     */
    async generateIDEFixFile(group, allIssues) {
        const groupIssues = allIssues.filter(i => i.rule === group.rule && i.tool === group.tool && i.severity === group.severity);
        const representative = groupIssues[0];
        if (!representative)
            return null;
     ...
     */
    async generateCursorFixData(group, groupIssues, representative) {
        var _a;
        ...
        return {
            ...
            description: ((_a = representative.fixSuggestion) === null || _a === void 0 ? void 0 : _a.explanation) || '',
     ...
     */
    async generateLSPAndSARIFFormats(enrichedIssues, groups, metadata, analysisTimestamp // BUG-DOG-04 FIX: Use consistent timestamp across all uploads
    ) {
        var _a;
        try {
            const converter = new lsp_sarif_converter_1.LSPSARIFConverter();
            const gitlabConverter = new gitlab_codequality_converter_1.GitLabCodeQualityConverter();
     ...
     */
    async uploadSingleFile(analysisId, filename, content, service, metadata) {
        if (!this.supabase)
            return undefined;
        try {
            ...
            const { data, error } = await this.uploadWithRetry(`${analysisId}/${filename}`, uploadContent, {
                contentType: 'application/json',
                cacheControl: '3600',
                upsert: true
            });
     ...
     */
    extractFixPattern(group, representative) {
        const fix = representative.fixSuggestion;
        const classification = (0, issue_classifier_1.classifyIssue)(group.rule, group.tool);
        ...
        if (classification.fixTier <= 2 && classification.fixable) {
            ...
            const cleanedCode = this.cleanCorrectedCode(fix === null || fix === void 0 ? void 0 : fix.correctedCode);
            // BUG-LSP-001: If fix was rejected (empty after cleaning), mark as manual review
            const requiresManualReview = !cleanedCode && (fix === null || fix === void 0 ? void 0 : fix.correctedCode);
            return {
                ...
                instructions: (fix === null || fix === void 0 ? void 0 : fix.fix) || 'Apply the suggested fix',
        ...
        const aiPrompt = (0, ai_fix_prompts_1.getOptimizedPrompt)(issueContext);
        // All Tier 3 issues get AI-generated fixes with dynamic prompts
        const cleanedTier3Code = this.cleanCorrectedCode(fix === null || fix === void 0 ? void 0 : fix.correctedCode);
        // BUG-LSP-001: If fix was rejected (empty after cleaning), mark as manual review
        const requiresManualReview = !cleanedTier3Code && (fix === null || fix === void 0 ? void 0 : fix.correctedCode);
        return {
            ...
            instructions: (fix === null || fix === void 0 ? void 0 : fix.fix) || `AI-generated fix for ${group.rule}`,
            // BUG-LSP-001: Add manual review info when fix was rejected
            manualReview: requiresManualReview ? {
                reason: 'FIX_GENERATION_FAILED',
     ...
     */
    determineIssueCategory(issueType, tool) {
     ...
     */
    getFixerToolForRule(tool, issueType) {
        const normalizedTool = tool.toLowerCase();
        // TypeScript/JavaScript
        if (['eslint', 'typescript-eslint'].includes(normalizedTool))
            return 'eslint';
        if (normalizedTool === 'prettier')
            return 'prettier';
        // Python
        ...
        if (issueType === 'quality')
            return 'autoflake';
        if (issueType === 'compatibility')
            return 'pyupgrade';
        // Java
        if (['pmd', 'checkstyle', 'spotbugs'].includes(normalizedTool))
            return 'sorald';
        // Go
        if (normalizedTool === 'golangci-lint')
     ...
     */
    getFixerCommand(tool, issueType) {
        const normalizedTool = tool.toLowerCase();
        // TypeScript/JavaScript
        if (['eslint', 'typescript-eslint'].includes(normalizedTool))
            return 'eslint --fix';
        ...
        if (issueType === 'compatibility')
            return 'pyupgrade --py38-plus';
        // Java
        if (['pmd', 'checkstyle', 'spotbugs'].includes(normalizedTool))
            return 'sorald repair --source';
        // Go
        if (normalizedTool === 'golangci-lint')
    ...
    detectVulnerabilityType(message) {
        ...
        if (msg.includes('openssl') && (msg.includes('secadv') || // OpenSSL security advisory
     ...
     */
    getVulnerabilityTypeInfo(vulnType, severity) {
    ...
    getImpactSummary(rule, tool, severity, message) {
        var _a;
        ...
        const firstSentence = ((_a = whatText.match(/^[^.!?]+[.!?]/)) === null || _a === void 0 ? void 0 : _a[0]) || whatText.substring(0, 120);
        return firstSentence.trim() + (whatText.length > 120 ? '...' : '');
    }
    /**
     * ENHANCEMENT: Calculate priority score for sorting in manifest
     */
    calculatePriority(severity, category, fileCount) {
        let score = 0;
     ...
     */
    extractRequiredImports(representative) {
        var _a;
        const correctedCode = (_a = representative.fixSuggestion) === null || _a === void 0 ? void 0 : _a.correctedCode;
        const fix = (typeof correctedCode === 'string') ? correctedCode : '';
        const imports = [];
        if (fix.includes('AtomicBoolean'))
            imports.push('java.util.concurrent.atomic.AtomicBoolean');
        if (fix.includes('AtomicInteger'))
            imports.push('java.util.concurrent.atomic.AtomicInteger');
        if (fix.includes('AtomicLong'))
            imports.push('java.util.concurrent.atomic.AtomicLong');
        if (fix.includes('Collections.emptyList'))
     ...
     */
    generateMapping(issues, groups, metadata, ideFixFiles) {
     ...
     */
    generateBusinessImpact(issues, groups) {
        return (0, business_impact_1.generateBusinessImpact)(issues, groups, this.detectedLanguage, this.userTier);
    }
    _REMOVED_legacyGenerateBusinessImpact(issues, groups) {
        var _a;
        // BLOCKERS ONLY: NEW/EXISTING_MODIFIED + critical/high
        const blocking = issues.filter(i => (i.category === 'NEW' || i.category === 'EXISTING_MODIFIED') &&
            (i.severity === 'critical' || i.severity === 'high'));
        ...
        if (autoFixableGroups.length > 0) {
            ...
            const severityHours = { critical: 2, high: 1.5, medium: 1, low: 0.5 };
            let autoFixOriginalHours = 0;
            let autoFixAdjustedHours = 0;
            for (const g of autoFixableGroups) {
                const perIssue = (_a = severityHours[g.severity]) !== null && _a !== void 0 ? _a : 1;
...
${(() => {
            ...
            const blockingAutoFixable = blocking.filter(i => this.canAutoFix({ rule: i.rule, tool: i.tool, severity: i.severity })).length;
...
**Note:** Each issue group section above includes detailed business impact analysis specific to that issue type.`;
     ...
     */
    getExploitCostExplanation(criticalCount, highCount, securityCount) {
        return (0, business_impact_1.getExploitCostExplanation)(criticalCount, highCount, securityCount);
    }
    /**
     * Get risk impact level helper
     * BUG FIX #75: Consider blocking status and severity for accurate risk assessment
     */
    getRiskImpactLevel(categoryIssues) {
        return (0, business_impact_1.getRiskImpactLevel)(categoryIssues);
    }
    /**
     ...
     */
    generatePROFixSummary(issues, groups, metadata) {
        const lines = [];
        ...
        if (codeFixIssues.length > 0) {
            ...
            for (const category of categories) {
                ...
                if (categoryIssues.length === 0)
                    continue;
                const categoryGroups = groups.filter(g => categoryIssues.some(i => i.rule === g.rule && i.tool === g.tool));
     ...
     */
    getFixConfidence(issue) {
        // SESSION 92 FIX: Use hasActualCodeFix to check for real fixes
        if (!this.hasActualCodeFix(issue))
            return 0;
        // Use severity confidence if available
        if (issue.severityConfidence === 'high')
            return 95;
        if (issue.severityConfidence === 'medium')
            return 85;
        if (issue.severityConfidence === 'low')
            return 70;
        // Default based on category
        if (issue.detectedCategory === 'Security')
            return 92;
        if (issue.detectedCategory === 'Code Quality')
            return 88;
        return 85;
    }
    /**
     ...
     */
    classifyManualReviewReason(issue) {
     ...
     */
    getManualReviewRecommendation(issue) {
        const classification = this.classifyManualReviewReason(issue);
        switch (classification) {
            case 'Major Dependency Upgrade':
     ...
     */
    calculateIssueWeightedSkillScore(issues) {
        return (0, business_impact_1.calculateIssueWeightedSkillScore)(issues);
    }
    /**
     * Generate Educational Resources based on actual issues found
     */
    generateEducationalResources(issues) {
...
`;
        ...
        return (0, educational_resources_1.generateEducationalResources)(issues, this.detectedLanguage);
    }
    async generateEducationalResourcesBrave(issues) {
        // BUG-090 FIX: Pass language parameter for language-specific resources
        return (0, educational_resources_1.generateEducationalResourcesBrave)(issues, this.detectedLanguage);
    }
    /**
    ...
    discoverTeamFromGit(repoPath) {
        try {
            ...
            const result = (0, child_process_1.execSync)(`git log --since="${sinceDate}" --format="%ae:::%an" --no-merges`, { cwd: repoPath, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
     ...
     */
    calculateRepoStats(repoPath, language, baseBranch) {
        const defaultStats = {
            totalFiles: 0,
            totalLinesOfCode: 0,
            filesModified: 0,
        ...
        try {
            ...
            const totalFilesResult = (0, child_process_1.execSync)(`find . -type f -not -path './.git/*' | wc -l`, { cwd: repoPath, encoding: 'utf-8', timeout: 30000 });
            ...
            for (const ext of extensions) {
                try {
                    const filesResult = (0, child_process_1.execSync)(`find . -type f -name "${ext}" -not -path './.git/*' | head -500`, { cwd: repoPath, encoding: 'utf-8', timeout: 30000 });
                    const files = filesResult.trim().split('\n').filter(f => f);
                    for (const file of files) {
                        try {
                            const lines = parseInt((0, child_process_1.execSync)(`wc -l < "${file}"`, { cwd: repoPath, encoding: 'utf-8', timeout: 5000 }).trim(), 10);
            ...
            if (baseBranch) {
                try {
                    const diffStats = (0, child_process_1.execSync)(`git diff --shortstat ${baseBranch}...HEAD 2>/dev/null || git diff --shortstat HEAD~10...HEAD 2>/dev/null || echo ""`, { cwd: repoPath, encoding: 'utf-8', timeout: 30000 });
     ...
     */
    async generateSkillsTracking(issues, metadata) {
        var _a, _b;
        ...
        try {
            ...
            const developerBaseline = await this.skillScoreManager.getBaselineScore(metadata.prAuthorEmail, metadata.repository);
            ...
            const calculateSkillCategoryScore = (categoryIssues) => {
                if (categoryIssues.length === 0) {
            ...
            const currentPRScore = Math.round((categoryScores.security + categoryScores.performance + categoryScores.architecture +
            ...
            const normalizeEmailForDedup = (email) => {
                if (!email)
                    return '';
            ...
            supabaseLeaderboard = supabaseLeaderboard.filter((dev) => {
            ...
            for (const gitDev of gitTeammates) {
                ...
                const existsInSupabase = teamLeaderboard.some((dev) => normalizeEmailForDedup(dev.email) === normalizedGitEmail ||
            ...
            const currentDevIndex = teamLeaderboard.findIndex((d) => normalizeEmail(d.email) === normalizedAuthorEmail ||
            ...
            console.log(`[Skills] DEBUG: Before sort, ${metadata.prAuthor} score is ${(_a = teamLeaderboard.find((d) => normalizeEmail(d.email) === normalizedAuthorEmail)) === null || _a === void 0 ? void 0 : _a.score}`);
            console.log(`[Skills] DEBUG: Before sort, all entries with matching normalized email:`);
            teamLeaderboard.forEach((d, idx) => {
            ...
            console.log(`[Skills] DEBUG: After sort, ${metadata.prAuthor} score is ${(_b = teamLeaderboard.find((d) => normalizeEmail(d.email) === normalizedAuthorEmail)) === null || _b === void 0 ? void 0 : _b.score}`);
            ...
            const rank = teamLeaderboard.findIndex((d) => normalizeEmail(d.email) === normalizedAuthorEmail) + 1;
            ...
            teamLeaderboard.forEach((dev, idx) => {
            ...
            if (categoryScores.security < teamAvg)
                weakCategories.push('Security');
            if (categoryScores.performance < teamAvg)
                weakCategories.push('Performance');
            if (categoryScores.architecture < teamAvg)
                weakCategories.push('Architecture');
            if (categoryScores.dependencies < teamAvg)
                weakCategories.push('Dependencies');
            if (categoryScores.codeQuality < teamAvg)
                weakCategories.push('Code Quality');
            ...
            if (teamLeaderboard.length > 0) {
                ...
                const humanPerformers = teamLeaderboard.filter((dev) => !isAIAgent(dev));
                ...
                const deduplicatedPerformers = humanPerformers.reduce((acc, dev) => {
                    ...
                    if (existing) {
                        ...
                        const weightedScore = ((existing.score * (existing.totalPRs || 1)) +
                            (dev.score * (dev.totalPRs || 1))) / totalPRs;
                ...
                deduplicatedPerformers.sort((a, b) => (b.score || 0) - (a.score || 0));
                ...
                deduplicatedPerformers.slice(0, 5).forEach((dev, idx) => {
     ...
     */
    generateGitBasedSkillsTracking(issues, metadata) {
        try {
    ...
    getStatusEmoji(yourScore, teamAvg, teamSize = 1) {
        // Solo developer - no comparison needed
        if (teamSize === 1)
            return '👤 Solo Developer';
        // Team comparison
        if (yourScore >= teamAvg + 10)
            return '🌟 Excellent';
        if (yourScore > teamAvg)
     ...
     */
    async generateXPAndAchievements(userEmail) {
        var _a;
        ...
        try {
            ...
            const analysisCount = (skillHistory === null || skillHistory === void 0 ? void 0 : skillHistory.length) || 0;
            let totalXp = analysisCount * 25;
            // Bonus XP for high scores
            const highScores = (skillHistory || []).filter((s) => s.overall_score >= 80).length;
            totalXp += highScores * 15;
            // Calculate level from XP
            const levelInfo = (0, achievements_1.calculateLevel)(totalXp);
            const progressBar = (0, achievements_1.generateXpProgressBar)(totalXp, levelInfo.nextLevelXp);
            ...
            if (analysisCount >= 1) {
                achievements.push({
                    ...
                    category: 'milestone',
                    tier: 'common',
                    unlockedAt: new Date(((_a = skillHistory[0]) === null || _a === void 0 ? void 0 : _a.created_at) || Date.now()),
                    xpValue: 10
                });
            }
            ...
            let achievementStyle = 'professional';
            try {
                const { data: prefs } = await this.supabase
                    ...
                    .single();
                if ((prefs === null || prefs === void 0 ? void 0 : prefs.achievement_style) === 'gamified') {
            ...
            if (achievements.length > 0) {
                content += (0, achievements_1.generateAchievementsSection)(achievements, achievementStyle, 3);
            }
            else {
     ...
     * Session 66: Integrated for tier differentiation
     */
    async generateCommunityImpact(userEmail) {
        if (!userEmail || !this.supabase) {
            return '';
        }
        try {
        ...
        `)
            ...
            if (!contribError && contributions && contributions.length > 0) {
                ...
                topPatterns = contributions
                    .filter((c) => { var _a; return ((_a = c.fix_patterns) === null || _a === void 0 ? void 0 : _a.status) === 'active'; })
                    .map((c) => {
                    var _a, _b, _c, _d, _e, _f;
                    return ({
                        patternId: c.pattern_id,
                        patternName: ((_a = c.fix_patterns) === null || _a === void 0 ? void 0 : _a.name) || ((_b = c.fix_patterns) === null || _b === void 0 ? void 0 : _b.rule_id),
                        ruleId: (_c = c.fix_patterns) === null || _c === void 0 ? void 0 : _c.rule_id,
                        language: 'java', // Default for now
                        contributedAt: new Date(c.contributed_at),
                        usageCount: ((_d = c.fix_patterns) === null || _d === void 0 ? void 0 : _d.apply_count) || 0,
                        usersHelped: Math.max(1, Math.floor((((_e = c.fix_patterns) === null || _e === void 0 ? void 0 : _e.apply_count) || 0) / 3)),
                        timeSavedMinutes: (((_f = c.fix_patterns) === null || _f === void 0 ? void 0 : _f.apply_count) || 0) * 5
                    });
                });
                totalPatterns = topPatterns.length;
                totalUsageCount = topPatterns.reduce((sum, p) => sum + p.usageCount, 0);
            }
            else {
                ...
                console.log(`[CommunityImpact] Contribution tracking not yet available: ${(contribError === null || contribError === void 0 ? void 0 : contribError.message) || 'no data'}`);
            }
            ...
            return (0, community_impact_1.generateCommunityImpactSection)(impact, privacyPrefs, this.userTier);
        }
        catch (error) {
     ...
     */
    generateAnalysisMetadata(metadata) {
        return (0, metadata_footer_1.generateAnalysisMetadata)(metadata, this.SHOW_AGENT_PERFORMANCE, this.SHOW_TOOL_PERFORMANCE, this.SHOW_EFFICIENCY_ANALYSIS, this.SHOW_SYSTEM_INFO);
    }
    _REMOVED_generateAnalysisMetadata_LEGACY(metadata) {
...
`;
        ...
        if (this.SHOW_AGENT_PERFORMANCE && metadata.agentPerformance && Array.isArray(metadata.agentPerformance) && metadata.agentPerformance.length > 0) {
            ...
            const activeAgents = metadata.agentPerformance.filter((agent) => {
...
`;
                activeAgents.forEach((agent) => {
                    ...
                    if (model === 'N/A' && this.modelConfigResolver) {
                        ...
                        if (agentName.includes('security'))
                            ...
                            role = 'architecture';
                        else if (agentName.includes('dependencies') || agentName.includes('dependency'))
                            role = 'dependency';
                        try {
                            // Synchronously get cached model config (avoid await in forEach)
                            const modelConfig = (_b = (_a = this.modelConfigResolver).getCachedConfiguration) === null || _b === void 0 ? void 0 : _b.call(_a, role, this.detectedLanguage, this.detectedRepoSize);
                            if (modelConfig === null || modelConfig === void 0 ? void 0 : modelConfig.primary_model) {
        ...
        if (this.SHOW_TOOL_PERFORMANCE && metadata.toolPerformance && Array.isArray(metadata.toolPerformance) && metadata.toolPerformance.length > 0) {
            ...
            const allTools = metadata.toolPerformance.filter((tool) => (tool.duration || 0) > 0);
...
`;
            // Calculate totals
            const totalCost = metadata.agentPerformance.reduce((sum, agent) => sum + (agent.cost || 0), 0);
            const totalIssues = metadata.agentPerformance.reduce((sum, agent) => sum + (agent.issuesFound || agent.issues || 0), 0);
            const totalTime = metadata.agentPerformance.reduce((sum, agent) => sum + (agent.duration || 0), 0);
            content += `\n**Overall Efficiency:**\n`;
            ...
            agentEfficiency.forEach((agent, idx) => {
            ...
            const expensiveAgents = agentEfficiency.filter((a) => a.cost >= 0.0001 && a.costPerIssue > 0.05);
            if (expensiveAgents.length > 0) {
                content += `\n**💡 Optimization Opportunities:**\n`;
                expensiveAgents.forEach((agent) => {
                    content += `- Consider optimizing **${agent.name}** (high cost/issue: $${agent.costPerIssue.toFixed(4)})\n`;
                });
            }
            else if (agentEfficiency.every((a) => a.cost === 0 || a.cost < 0.0001)) {
                content += `\n**💡 Cost Optimization:**\n`;
        ...
        if (metadata.toolPerformance && Array.isArray(metadata.toolPerformance) && metadata.toolPerformance.length > 0) {
            const toolEfficiency = metadata.toolPerformance
                ...
                .filter((tool) => tool.issues > 0) // Only include tools that found issues
                .sort((a, b) => b.efficiency - a.efficiency);
...
`;
                ...
                toolEfficiency.forEach((tool, idx) => {
                    const rank = idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}.`;
...
`;
 ...
 - **Issue Grouping:** ${metadata.totalGroups || 'Enabled'} unique issue types`;
     ...
     */
    generatePRComment(issues, groups, metadata) {
        return (0, metadata_footer_1.generatePRComment)(issues, groups, metadata);
    }
    _REMOVED_generatePRComment_LEGACY(issues, groups, metadata) {
        const blocking = issues.filter(i => (i.category === 'NEW' || i.category === 'EXISTING_MODIFIED') &&
            (i.severity === 'critical' || i.severity === 'high'));
...
- ⏳ **Needs Attention**: ${noFixCount} issues`;
                ...
                const safeCount = issues.filter(i => this.isSafeToAutoApply({ rule: i.rule, tool: i.tool, severity: i.severity })).length;
...
\`\`\``;
     ...
     */
    getPersonalizedGreeting(author) {
     ...
     */
    getPersonalizedEncouragement(blockingCount, resolvedCount) {
        if (resolvedCount > 10) {
     ...
     * Generate footer
     */
    generateFooter(groups, ideFixFiles, metadata, enrichedIssues) {
        return (0, metadata_footer_1.generateFooter)(groups, ideFixFiles, metadata, enrichedIssues);
    }
    _REMOVED_generateFooter_LEGACY(groups, ideFixFiles) {
        ...
        if (ideFixFiles.length > 0) {
            ...
            const totalFixable = issueFiles.reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            ...
            if (manifestFile && manifestFile.content.files) {
                const filesObj = manifestFile.content.files;
                autoFixableCount = Object.values(filesObj).flat().reduce((sum, entry) => sum + (entry.autoFixable ? entry.occurrences : 0), 0);
            }
            const criticalCount = issueFiles.filter(f => f.content.severity === 'critical').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            const highCount = issueFiles.filter(f => f.content.severity === 'high').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            const mediumCount = issueFiles.filter(f => f.content.severity === 'medium').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            const lowCount = issueFiles.filter(f => f.content.severity === 'low').reduce((sum, f) => { var _a; return sum + (((_a = f.content.metadata) === null || _a === void 0 ? void 0 : _a.total_occurrences) || 0); }, 0);
            ...
            if (highCount > 0)
                footer += `- 🟠 High: ${highCount} (lazy loaded after critical)\n`;
            if (mediumCount > 0)
                footer += `- 🟡 Medium: ${mediumCount} (lazy loaded after high)\n`;
            if (lowCount > 0)
                footer += `- 🟢 Low: ${lowCount} (lazy loaded after medium)\n`;
    ...
    // Helper methods
    sanitizeGroupId(group) {
        return `${group.rule}-${group.severity}-${group.tool}`.toLowerCase().replace(/[^a-z0-9-]/g, '-');
    }
    groupBySeverity(issues) {
        return (0, metadata_footer_1.groupBySeverity)(issues);
    }
    groupByCategory(issues) {
        return (0, metadata_footer_1.groupByCategory)(issues);
    }
    groupByTool(issues) {
        return (0, metadata_footer_1.groupByTool)(issues);
    }
}
exports.V9GroupedReportFormatter = V9GroupedReportFormatter;
/**
 ...
 */
V9GroupedReportFormatter.TEMPLATE_PATTERNS = [
    /should be:/i,
    /change to:/i,
    /replace with:/i,
    /instead of:/i,
