...
exports.AppScoreManager = void 0;
class AppScoreManager {
     ...
     */
    async getAppBaseline(repository) {
        try {
     ...
     */
    async getAppTrend(repository, limit = 5) {
        try {
            const { data, error } = await this.supabase
                .from('pr_analysis_history')
                .select('app_overall_score, app_security_score, app_performance_score, app_architecture_score, app_dependency_score, app_code_quality_score')
     ...
     */
    async getRepositoryHealthSummary(repository) {
        try {
            const { data, error } = await this.supabase
                .from('pr_analysis_history')
                .select('app_overall_score, app_security_score, app_performance_score, app_architecture_score, app_dependency_score, app_code_quality_score')
            ...
            const weakestCategory = Object.entries(categories).reduce((min, [cat, score]) => score < categories[min] ? cat : min, 'security');
...
exports.AppScoreManager = AppScoreManager;
