...
`;
  ...
  private generateToolJobYaml(jobName: string, pvcName: string, tool: string, language: string, selectedFiles?: string[]): string {
    // Use the ARM analyzer images from the registry (built on Oracle A1.Flex)
    const languageVersions: Record<string, string> = {
      ...
      'cpp': 'lang-cpp-v4.7',  // No ARM version built yet
      'csharp': 'lang-csharp-v3.2-arm',
      'php': 'lang-php-v3.4-arm',
      'perl': 'lang-perl-v4.6',  // No ARM version built yet
    ...
    const toolCommands: Record<string, string> = {
      ...
      'pmd': selectedFiles
        ? `${fileListCommand}cd /workspace/repo && cat /tmp/selected_files.txt | grep -v '/test/' | xargs pmd pmd -R category/java/errorprone.xml,category/java/security.xml -f text   ${process.env.TOOL_THREADS || '4'} 2>&1 || true`
        : `cd /workspace/repo && pmd pmd -d . --exclude '**/test/**' -R category/java/errorprone.xml,category/java/security.xml -f text   ${process.env.TOOL_THREADS || '4'} 2>&1 || true`,
      ...
      'trivy': `cd /workspace/repo && trivy fs --scanners vuln --format json  . 2>&1 || true`,
