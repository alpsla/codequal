...
exports.SpecializedAgentFactory = exports.DependencyAgent = exports.CodeQualityAgent = exports.ArchitectureAgent = exports.PerformanceAgent = exports.SecurityAgent = void 0;
const openai_1 = __importDefault(require("openai"));
const simple_openrouter_client_1 = require("../services/simple-openrouter-client");
const examples_database_1 = require("./examples-database");
/**
...
class BaseSpecializedAgent {
    constructor(role, modelConfig) {
        ...
        if ((_a = process.env.OPENROUTER_API_KEY) === null || _a === void 0 ? void 0 : _a.startsWith('sk-or-')) {
        ...
        this.openRouter = new openai_1.default(openRouterConfig);
    }
    /**
    ...
    getCategoryFromRole(role) {
        ...
        if (normalized.includes('architecture'))
            return 'Architecture';
        if (normalized.includes('dependency') || normalized.includes('dependen'))
            return 'Dependencies';
     ...
     */
    async generateFixSuggestion(issue, modelOverride) {
        var _a, _b;
        ...
        const modelToUse = modelOverride || ((_a = this.modelConfig) === null || _a === void 0 ? void 0 : _a.primary_model);
        if (!modelToUse) {
            throw new Error(`[${this.agentRole}] No model configured! ` +
        ...
        const fallbackModel = (_b = this.modelConfig) === null || _b === void 0 ? void 0 : _b.fallback_model;
        const systemPrompt = this.getSystemPrompt();
        const userPrompt = this.buildPrompt(issue);
        try {
            // COST FIX: Use simple client (no retries, fallback on 401 only)
            const aiClient = (0, simple_openrouter_client_1.getSimpleOpenRouterClient)();
     ...
     */
    buildPromptWithExamples(issue) {
        // Get 2 examples for this category
        const examples = (0, examples_database_1.getExamplesForCategory)(this.category, 2);
...
Provide JSON response following system prompt structure.`;
    ...
    parseAIResponse(response, issue) {
        ...
        if (!correctedCode && response.includes(issue.file.split('/').pop() || '')) {
            ...
            const linesWithCode = response.split('\n').filter(line => /^\s{2,}/.test(line) && // Indented
...
${lineNum + 3}: // Context: ${fileName} line ${lineNum}`;
    ...
    getDefaultFix(issue) {
        ...
        return {
            fix: roleSpecificFix[this.agentRole] ||
 ...
 * BUG-76 FIX: Compact system prompt + JSON examples
 */
class ArchitectureAgent extends BaseSpecializedAgent {
    constructor(modelConfig) {
...
CRITICAL: Be RULE-SPECIFIC. NO generic boilerplate. Reference the EXACT architectural issue type, tool, and rule name.`;
     ...
     */
    getSpecificGuidanceForIssue(issue) {
        var _a;
        const description = issue.description.toLowerCase();
        const title = ((_a = issue.title) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
 ...
 * BUG-76 FIX: Compact system prompt + JSON examples
 */
class CodeQualityAgent extends BaseSpecializedAgent {
    constructor(modelConfig) {
...
If no code snippet: Generate realistic template for ${className} based on issue type.`;
     ...
     */
    getIssueSpecificGuidance(issue) {
        const type = issue.type.toLowerCase();
 ...
 * BUG-76 FIX: Compact system prompt + JSON examples
 */
class DependencyAgent extends BaseSpecializedAgent {
    constructor(modelConfig) {
...
class SpecializedAgentFactory {
     ...
     */
    static async generateFixForIssue(issue, modelConfigResolver, // ModelConfigResolver from orchestrator
    language, repoSize) {
        ...
        const modelConfig = await modelConfigResolver.getModelConfiguration(role, language, repoSize);
     ...
     */
    static createAgent(category, modelConfig) {
     ...
     */
    static getRoleForCategory(category) {
...
exports.SpecializedAgentFactory = SpecializedAgentFactory;
