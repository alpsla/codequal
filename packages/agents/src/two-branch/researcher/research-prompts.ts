...
STRICTLY search for and consider ONLY models released within the LAST 6 MONTHS from ${new Date().toISOString().split('T')[0]}.
- Models older than 6 months: DO NOT INCLUDE - COMPLETELY IGNORE
- If newer versions exist, MUST use the LATEST (e.g., v4.x over v3.x)
- NO outdated models allowed (e.g., claude-3.5 when claude-4.x exists)
- Models within last 3 months: PRIORITIZE THESE
- This is a HARD requirement - no exceptions for "good but old" models
- IMPORTANT: Use Date() for dynamic date calculation, never hardcode dates
...
**RESEARCH APPROACH:**
1. **Cross-Market Analysis**: Compare models from ALL providers released in last 6 months ONLY from ${new Date().toISOString().split('T')[0]}
2. **Specific Requirements**: Focus on capabilities needed for this exact role
3. **Primary + Fallback**: Identify best model AND a reliable fallback option (both from last 6 months)
...
**MARKET RESEARCH - LAST 6 MONTHS ONLY:**
Search across ALL providers for models released in the LAST 6 MONTHS from ${new Date().toISOString().split('T')[0]}:
- OpenAI: Whatever their LATEST models are (must be < 6 months old)
- Anthropic: Whatever their LATEST models are (e.g., v4.x NOT v3.5)
- Google: Whatever their LATEST models are (e.g., v2.5 NOT v2.0)
- DeepSeek: Whatever their LATEST models are (e.g., v3.1 NOT older)
- Meta: Whatever their LATEST models are (e.g., v3.3 NOT v3.1)
- Mistral: Whatever their LATEST models are (must be < 6 months old)
- Cohere: Whatever their LATEST models are (must be < 6 months old)
- Others: Any new providers with models from last 6 months
- Emerging: New providers and models from last 6 months ONLY
...
5. Map web names to OpenRouter IDs (use LATEST versions only)
...
**🚨 CRITICAL TIME REQUIREMENT 🚨**
STRICTLY search for ONLY models released within the LAST 6 MONTHS from ${new Date().toISOString().split('T')[0]}.
- Models older than 6 months: COMPLETELY REJECT
- Models within last 6 months: ACCEPTABLE
...
Search for the absolute best model for fast text parsing available in the last 6 months from ${new Date().toISOString().split('T')[0]}.`,
...
YOU MUST ONLY search for and consider models released within the LAST 6 MONTHS from ${new Date().toISOString().split('T')[0]}.
...
- ONLY models released in the last 6 months MAX from ${new Date().toISOString().split('T')[0]}
...
2. **Provider APIs**: Check official documentation for models released in the last 6 months ONLY
3. **Tech News**: Look for AI model launches from the last 6 months ONLY  
4. **GitHub**: Search for model releases with dates within last 6 months
5. **Research Papers**: Find models published in the last 6 months ONLY

**STRICT DISCOVERY CRITERIA:**
- ✅ MANDATORY: Models released within the last 6 months from ${new Date().toISOString().split('T')[0]}
