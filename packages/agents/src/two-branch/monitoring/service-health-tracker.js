...
exports.ServiceHealthTracker = void 0;
class ServiceHealthTracker {
     ...
     * Fallback to error_logs table if service_health_events doesn't exist
     */
    async fallbackToErrorLogs(event, originalError) {
        if (!this.supabase)
            return;
        try {
            ...
            await this.supabase
                ...
                details: {
                    ...
                    analysis_id: event.analysis_id,
                    metadata: event.metadata,
                    original_error: originalError === null || originalError === void 0 ? void 0 : originalError.message
                },
                status_code: event.status_code,
                created_at: new Date().toISOString()
            });
    ...
    async getHealthMetrics(params) {
        ...
        try {
            ...
            const events = data || [];
            const failures = events.filter(e => e.event_type.includes('failure') || e.event_type.includes('error')).length;
            const successes = events.filter(e => e.event_type.includes('success')).length;
            const byEventType = {};
            const byStatusCode = {};
...
exports.ServiceHealthTracker = ServiceHealthTracker;
