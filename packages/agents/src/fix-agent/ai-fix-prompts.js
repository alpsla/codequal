...
exports.PERFORMANCE_FIX_PROMPTS = exports.QUALITY_FIX_PROMPTS = exports.SECURITY_FIX_PROMPTS = void 0;
exports.isRecommendationCategory = isRecommendationCategory;
exports.generateDynamicPrompt = generateDynamicPrompt;
exports.buildAIFixRequest = buildAIFixRequest;
exports.getOptimizedPrompt = getOptimizedPrompt;
exports.getAIFixPrompt = getAIFixPrompt;
/**
 ...
 */
const RECOMMENDATION_ONLY_CATEGORIES = [
    'secrets',
    'iac_security',
    'container_security',
 ...
 */
function isRecommendationCategory(category) {
 ...
 */
function buildRecommendationUserPrompt(context) {
    const parts = [];
    ...
    if (context.category === 'secrets') {
        if (context.secretType)
            parts.push(`SECRET TYPE: ${context.secretType}`);
        parts.push('');
        ...
        if (context.containerImage)
            parts.push(`IMAGE: ${context.containerImage}`);
        if (context.vulnerabilityId)
            parts.push(`VULNERABILITY: ${context.vulnerabilityId}`);
        parts.push('');
        ...
        if (context.framework)
            parts.push(`GRAPHQL SERVER: ${context.framework}`);
        parts.push('');
        ...
        if (context.className)
            parts.push(`PACKAGE/MODULE: ${context.className}`);
        parts.push('');
    ...
    const metadata = [];
    if (context.value)
        metadata.push(`Affected Value: ${context.value}`);
    if (context.framework)
        metadata.push(`Framework: ${context.framework}`);
 ...
 */
function buildDynamicUserPrompt(context) {
    ...
    if (context.variableName)
        metadata.push(`Variable: ${context.variableName}`);
    if (context.functionName)
        metadata.push(`Function: ${context.functionName}`);
    if (context.className)
        metadata.push(`Class: ${context.className}`);
    if (context.value)
        metadata.push(`Value: ${context.value}`);
    if (context.framework)
        metadata.push(`Framework: ${context.framework}`);
    if (metadata.length > 0) {
        parts.push('CONTEXT:');
 ...
 */
function determineRequiredContext(context) {
    const required = ['file'];
 ...
 */
function calculateMaxTokens(context) {
    ...
    if (contextLength > 500)
        tokens += 200; // Longer context may need longer fix
...
function calculateTemperature(context) {
    ...
    if (context.category === 'maintainability')
        return 0.3;
    return 0.2; // Default
}
/**
 ...
 */
function determineOutputFormat(context) {
 ...
 */
function buildAIFixRequest(context) {
 ...
 */
function getOptimizedPrompt(context) {
...
function getAIFixPrompt(ruleId, category) {
...
exports.SECURITY_FIX_PROMPTS = {};
exports.QUALITY_FIX_PROMPTS = {};
exports.PERFORMANCE_FIX_PROMPTS = {};
