 ...
 */
export const RULE_EQUIVALENTS: Record<string, string[]> = {
  // =========================================================================
 ...
 */
export const CWE_TO_CANONICAL: Record<string, string> = {
 ...
 */
export function normalizeRuleId(toolRuleId: string): string {
  // Check exact match in any variant list
  for (const [canonical, variants] of Object.entries(RULE_EQUIVALENTS)) {
 ...
 */
export function canonicalFromCwe(cweId: string): string | undefined {
  // Normalize CWE format
  const normalizedCwe = cweId.toUpperCase().startsWith('CWE-')
    ? cweId.toUpperCase()
    : `CWE-${cweId}`;

  return CWE_TO_CANONICAL[normalizedCwe];
}
 ...
 */
export function getEquivalentRules(ruleId: string): string[] {
  // First normalize to canonical
  const canonical = normalizeRuleId(ruleId);

  // Return all variants if canonical exists
  if (RULE_EQUIVALENTS[canonical]) {
    return RULE_EQUIVALENTS[canonical];
  }
 ...
 */
export function areRulesEquivalent(ruleId1: string, ruleId2: string): boolean {
 ...
 */
export function getPatternLookupKeys(
  ruleId: string,
  cweId?: string
): { canonical: string; lookupKeys: string[] } {
  const canonical = normalizeRuleId(ruleId);
 ...
 */
export function registerRuleMapping(canonical: string, toolRuleId: string): void {
  if (!RULE_EQUIVALENTS[canonical]) {
    RULE_EQUIVALENTS[canonical] = [];
  }

  if (!RULE_EQUIVALENTS[canonical].includes(toolRuleId)) {
    RULE_EQUIVALENTS[canonical].push(toolRuleId);
  }
}
 ...
 */
export function getRuleNormalizerStats(): {
  ...
  return {
    totalCanonical: Object.keys(RULE_EQUIVALENTS).length,
    totalMappings: Object.values(RULE_EQUIVALENTS).flat().length,
    categoryCounts,
  };
}
