...
exports.fixPatternGuidance = exports.FixPatternGuidanceService = void 0;
exports.getFixGuidance = getFixGuidance;
exports.addFixGuidance = addFixGuidance;
exports.getPromptAdditions = getPromptAdditions;
exports.formatGuidanceForPrompt = formatGuidanceForPrompt;
exports.trackFixFailure = trackFixFailure;
exports.getFailuresNeedingReview = getFailuresNeedingReview;
exports.markFailureReviewed = markFailureReviewed;
...
const FALLBACK_GUIDANCE = new Map([
...
// Service Class
// ============================================================================
class FixPatternGuidanceService {
    constructor() {
        this.supabase = null;
        this.initialized = false;
        this.cache = new Map();
        this.failureCache = new Map(); // SESSION 81: In-memory failure tracking
    ...
    async loadAllGuidance() {
        ...
        try {
            ...
            console.log(`[FixGuidance] Loaded ${(data === null || data === void 0 ? void 0 : data.length) || 0} guidance entries`);
        }
        catch (e) {
     ...
     */
    async getGuidance(ruleId, language, tool = 'any') {
     ...
     */
    async recordFixAttempt(ruleId, language, tool, success, failureReason) {
        await this.ensureInitialized();
     ...
     */
    async getPromptAdditions(ruleId, language, tool = 'any') {
        const guidance = await this.getGuidance(ruleId, language, tool);
        return (guidance === null || guidance === void 0 ? void 0 : guidance.promptAdditions) || null;
    }
    /**
     * Get all related rules that might conflict
     */
    async getRelatedRules(ruleId, language) {
        const guidance = await this.getGuidance(ruleId, language);
        return (guidance === null || guidance === void 0 ? void 0 : guidance.relatedRules) || [];
    }
    /**
     * Format guidance for inclusion in AI prompt
     */
    async formatForPrompt(ruleId, language, tool = 'any') {
        const guidance = await this.getGuidance(ruleId, language, tool);
        if (!guidance)
            return '';
        let prompt = '';
     ...
     */
    async getFailuresNeedingReview(limit = 20) {
        await this.ensureInitialized();
        if (!this.supabase || !this.supabaseAvailable) {
     ...
     */
    async markFailureReviewed(failureId, status, reviewedBy, notes, guidanceId) {
        await this.ensureInitialized();
        if (!this.supabase || !this.supabaseAvailable) {
     ...
     */
    generateGuidanceFromFailure(failure) {
        const antiPatterns = [];
        const relatedRules = [...(failure.regressionRules || [])];
...
exports.FixPatternGuidanceService = FixPatternGuidanceService;
FixPatternGuidanceService.instance = null;
// ============================================================================
// Constants
// ============================================================================
const FAILURE_TABLE_NAME = 'fix_failure_tracking';
// Singleton export
exports.fixPatternGuidance = FixPatternGuidanceService.getInstance();
// Convenience functions
async function getFixGuidance(ruleId, language, tool) {
    return exports.fixPatternGuidance.getGuidance(ruleId, language, tool);
}
async function addFixGuidance(guidance) {
    return exports.fixPatternGuidance.addGuidance(guidance);
}
async function getPromptAdditions(ruleId, language, tool) {
    return exports.fixPatternGuidance.getPromptAdditions(ruleId, language, tool);
}
async function formatGuidanceForPrompt(ruleId, language, tool) {
    return exports.fixPatternGuidance.formatForPrompt(ruleId, language, tool);
}
// SESSION 81: Failure tracking convenience functions
async function trackFixFailure(params) {
    return exports.fixPatternGuidance.trackFixFailure(params);
}
async function getFailuresNeedingReview(limit) {
    return exports.fixPatternGuidance.getFailuresNeedingReview(limit);
}
async function markFailureReviewed(failureId, status, reviewedBy, notes, guidanceId) {
    return exports.fixPatternGuidance.markFailureReviewed(failureId, status, reviewedBy, notes, guidanceId);
}
