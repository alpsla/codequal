...
exports.AI_FIXER_USER_ID = exports.FixPatternRegistry = void 0;
exports.getFixPatternRegistry = getFixPatternRegistry;
exports.resetFixPatternRegistry = resetFixPatternRegistry;
const uuid_1 = require("uuid");
const fix_security_validator_1 = require("./fix-security-validator");
const supabase_pattern_store_1 = require("./supabase-pattern-store");
// =============================================================================
...
const ruleIndex = new Map(); // ruleId -> patternIds
const userRecords = new Map(); // userId -> record
// =============================================================================
// Fix Pattern Registry Implementation
// =============================================================================
class FixPatternRegistry {
    constructor() {
        this.securityValidator = (0, fix_security_validator_1.getFixSecurityValidator)();
        this.supabaseInitialized = false;
        this.patterns = patternStore;
        this.ruleToPatterns = ruleIndex;
        this.users = userRecords;
        this.supabaseStore = (0, supabase_pattern_store_1.getSupabasePatternStore)();
    ...
    async lookup(request) {
        ...
        try {
            const supabasePatterns = await this.supabaseStore.lookupPatterns(request.ruleId, request.tool, request.fileType, (_a = request.activeOnly) !== null && _a !== void 0 ? _a : false);
            ...
            console.debug('[FixPatternRegistry] Supabase lookup failed, using in-memory:', err.message);
        }
    ...
    async capture(request) {
        try {
            ...
            const securityResult = this.securityValidator.validate(request.beforeCode, request.afterCode, request.ruleId, request.filePath);
            ...
            return {
                success: false,
                status: 'rejected',
                message: `Failed to capture pattern: ${error.message}`,
                autoApproved: false,
            };
        }
    }
    ...
    async apply(request) {
        ...
        try {
            ...
            let templateToApply = (_a = pattern.fixTemplate) === null || _a === void 0 ? void 0 : _a.template;
            if (!templateToApply) {
                // Try to get fixed code from examples
                const example = (_b = pattern.examples) === null || _b === void 0 ? void 0 : _b[0];
                if (example === null || example === void 0 ? void 0 : example.after) {
            ...
            const variables = request.variables || this.extractVariables(request.fileContent, pattern.detection);
            ...
            // Apply template
            const fixedCode = this.applyTemplate(request.fileContent, request.lineNumber, effectiveFixTemplate, variables);
            // Generate diff
            ...
            console.log(`[FixPatternRegistry:apply] Error applying pattern ${request.patternId.substring(0, 8)}: ${error.message}`);
            return {
                success: false,
                confidence: 0,
                error: `Failed to apply pattern: ${error.message}`,
            };
        }
    }
    /**
     ...
     */
    async updateStatus(patternId, status, reviewerId) {
    ...
    async recordApplication(patternId, success, reverted) {
        ...
        this.supabaseStore.recordApplication(patternId, success, reverted).catch((err) => {
            console.debug('[FixPatternRegistry] Failed to record application to Supabase:', err.message);
        });
    }
    /**
     * List all patterns
    ...
    analyzeDiff(request) {
        ...
        let transformationType = 'replace';
     ...
     */
    extractDiffVariables(before, after) {
        const variables = {};
     ...
     */
    extractVariables(code, detection) {
        const variables = {};
        if (detection.regex) {
     ...
     */
    applyTemplate(originalCode, lineNumber, template, variables) {
        var _a;
        ...
        if (template.indentation === 'preserve') {
            ...
            const indent = ((_a = targetLine.match(/^\s*/)) === null || _a === void 0 ? void 0 : _a[0]) || '';
            result = result
                .split('\n')
     ...
     */
    computeDerivedVariables(variables, defaults) {
        const computed = {};
     ...
     */
    banUser(userId, reason, reviewerId) {
    ...
    isUserBanned(userId) {
        ...
        return (record === null || record === void 0 ? void 0 : record.trustStatus) === 'banned';
    }
    /**
     * Update user trust status after pattern approval
     * Call this when an admin approves a pattern
     */
    async onPatternApproved(patternId) {
        const pattern = this.patterns.get(patternId);
        if (!pattern)
            return;
     ...
     */
    async submitAIFix(submission, options = {}) {
        var _a;
        ...
        try {
            ...
            const isVerified = (_a = options.verified) !== null && _a !== void 0 ? _a : false;
            // SESSION 77: Extract context key for context-aware pattern matching
            const contextKey = (0, supabase_pattern_store_1.extractContextKey)(submission.ruleId, submission.beforeCode);
            const pattern = {
                id: (0, uuid_1.v4)(),
                ruleId: submission.ruleId,
                tool: submission.tool,
            ...
            try {
                ...
                if (saved) {
                    console.log(`[FixPatternRegistry] AI Fix persisted to Supabase: ${pattern.id.substring(0, 8)} → ${pattern.status}`);
                }
                else {
                    console.log(`[FixPatternRegistry] AI Fix saved to memory only (Supabase unavailable): ${pattern.id.substring(0, 8)}`);
                }
            }
            catch (persistError) {
                // Don't fail the whole operation if persistence fails
                console.warn(`[FixPatternRegistry] Failed to persist to Supabase (saved to memory): ${persistError.message}`);
            }
            console.log(`[FixPatternRegistry] AI Fix submitted: ${pattern.id.substring(0, 8)} → ${pattern.status}`);
            return {
                success: true,
                pattern,
                patternId: pattern.id,
            ...
            return {
                success: false,
                status: 'rejected',
                message: `Failed to submit AI fix: ${error.message}`,
                autoApproved: false,
            };
        }
    }
    ...
    getAIFixerStats() {
        const aiFixerUserId = 'ai-fixer@codequal.dev';
        const allPatterns = Array.from(this.patterns.values()).filter((p) => p.metadata.createdBy === aiFixerUserId);
        return {
...
exports.FixPatternRegistry = FixPatternRegistry;
// =============================================================================
// Built-in Patterns
// =============================================================================
const BUILT_IN_PATTERNS = [
    // GitHub Actions Shell Injection Fix
    {
        ...
        description: 'Moves GitHub context variables from direct interpolation to env: block to prevent shell injection',
 ...
 */
exports.AI_FIXER_USER_ID = 'ai-fixer@codequal.dev';
// =============================================================================
// Singleton Instance
// =============================================================================
let registryInstance = null;
function getFixPatternRegistry() {
    if (!registryInstance) {
 ...
 */
function resetFixPatternRegistry() {
    registryInstance = null;
}
exports.default = FixPatternRegistry;
