...
export type ManualReviewReason =
  | 'CONTEXT_REQUIRED'      // Need to understand business logic
  | 'SECURITY_DECISION'     // Owner must decide on security trade-off
  | 'ARCHITECTURE_DECISION' // Requires architectural change
  | 'BREAKING_CHANGE'       // Fix would break existing behavior
  | 'MULTIPLE_OPTIONS'      // Several valid fixes exist
  | 'EXTERNAL_DEPENDENCY'   // Fix requires changes outside codebase
  | 'DATA_MIGRATION'        // Requires data/schema changes
  | 'AI_CAN_HELP';          // AI can generate fix with context
...
export interface ManualReviewInfo {
  ...
  aiCanHelp: boolean;            // Can AI generate a fix suggestion?
 ...
 */
export const SECURITY_MANUAL_REVIEW: Record<string, ManualReviewInfo> = {
 ...
 */
export const QUALITY_MANUAL_REVIEW: Record<string, ManualReviewInfo> = {
 ...
 */
export const PERFORMANCE_MANUAL_REVIEW: Record<string, ManualReviewInfo> = {
 ...
 */
export function getManualReviewInfo(
  ruleId: string,
  category: 'security' | 'quality' | 'performance'
): ManualReviewInfo | null {
  const maps = {
    security: SECURITY_MANUAL_REVIEW,
    quality: QUALITY_MANUAL_REVIEW,
    performance: PERFORMANCE_MANUAL_REVIEW,
  };
 ...
 */
export function generateManualReviewMessage(
  ruleId: string,
  category: 'security' | 'quality' | 'performance',
  filePath: string,
  lineNumber: number
 ...
 */
export function canAIHelp(ruleId: string, category: 'security' | 'quality' | 'performance'): boolean {
  const info = getManualReviewInfo(ruleId, category);
  return info?.aiCanHelp ?? false;
}

/**
 * Get AI prompt hint for generating fix
 */
export function getAIPromptHint(ruleId: string, category: 'security' | 'quality' | 'performance'): string | null {
  const info = getManualReviewInfo(ruleId, category);
  return info?.aiPromptHint ?? null;
}
