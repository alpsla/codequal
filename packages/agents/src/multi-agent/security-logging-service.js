...
exports.productionSecurityLoggingConfig = exports.defaultSecurityLoggingConfig = exports.SecurityLoggingService = void 0;
exports.createSecurityLoggingService = createSecurityLoggingService;
...
class SecurityLoggingService {
    constructor(config, supabaseClient) {
        this.logger = (0, utils_1.createLogger)('SecurityLoggingService');
        this.eventBuffer = [];
        ...
        if (config.enabled) {
            this.logger.info('Security logging service initialized', {
                backends: Object.keys(config.backends).filter(k => config.backends[k]?.enabled),
...
codequal_auth_rate_per_minute ${metrics.auth.rate}
     ...
     */
    calculateRate(metricKey, windowSeconds) {
...
📝 Details: ${JSON.stringify(event.details, null, 2)}`;
     ...
     */
    async storeEventsInSupabase(events) {
     ...
     */
    async sendToExternalLogger(events) {
 ...
 */
function createSecurityLoggingService(config, supabaseClient) {
    return new SecurityLoggingService(config, supabaseClient);
}
/**
 * Default configuration for development
 */
exports.defaultSecurityLoggingConfig = {
    enabled: true,
    backends: {
        console: {
            enabled: true,
 ...
 */
exports.productionSecurityLoggingConfig = {
    enabled: true,
    backends: {
        supabase: {
            enabled: true,
