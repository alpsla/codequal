...
class MultiAgentExecutor {
     ...
     */
    constructor(config, repositoryData, options = {}) {
        this.logger = (0, utils_1.createLogger)('MultiAgentExecutor');
        this.agents = new Map();
    ...
    async execute() {
        ...
        try {
            ...
            const result = {
                ...
                strategy: (this.config.strategy || this.config.executionMode),
                config: this.config,
            ...
            this.logger.error('Error executing multi-agent analysis:', error);
            // Create error result
            return {
                analysisId,
                id: analysisId, // Support both properties
    ...
    createAgent(config, name) {
        ...
        return agent_factory_1.AgentFactory.createAgent(config.role, config.provider || config.agentType, mergedConfig);
    }
    /**
     * Execute repository data provider
     * @returns Repository data or undefined if not available
     */
    async executeRepositoryProvider() {
        if (!this.config.repositoryProvider) {
        ...
        try {
            ...
            this.results.set('repository-provider', {
                ...
                tokenUsage: result.metadata?.tokenUsage || undefined
            });
            if (this.options.debug) {
            ...
            this.logger.error('Error executing repository provider agent:', error);
            // Store error
            this.results.set('repository-provider', {
                config: this.config.repositoryProvider,
                result: null,
     ...
     */
    async executeParallel(repoData) {
        if (this.options.debug) {
     ...
     * @param repoData Repository data
     */
    async executeSequential(repoData) {
        if (this.options.debug) {
     ...
     * @param repoData Repository data
     */
    async executeSpecialized(repoData) {
        if (this.options.debug) {
        ...
        const primaryData = {
            ...
            position: primaryConfig?.position || types_1.AgentPosition.PRIMARY
        };
        ...
        for (let i = 0; i < secondaryAgents.length; i++) {
            ...
            const secondaryData = {
                ...
                position: secondaryConfig.position || types_1.AgentPosition.SECONDARY,
                primaryResult
            };
    ...
    async executeOrchestrator() {
        ...
        try {
            ...
            this.logger.error('Error executing orchestrator agent:', error);
            // Store error
            this.results.set('orchestrator', {
                config: this.config.orchestrator,
                result: null,
     ...
     */
    async executeReporter(orchestratedResults) {
        if (!this.config.reporter) {
            return undefined;
        }
        ...
        try {
            ...
            this.logger.error('Error executing reporter agent:', error);
            // Store error
            this.results.set('reporter', {
                config: this.config.reporter,
                result: null,
     ...
     */
    async executeRepositoryInteraction(finalResults) {
        if (!this.config.repositoryInteraction) {
            return;
        }
        ...
        try {
            ...
            this.logger.error('Error executing repository interaction agent:', error);
            // Store error
            this.results.set('repository-interaction', {
                config: this.config.repositoryInteraction,
                result: null,
     ...
     */
    async executeAgent(agent, name, data) {
        try {
            const startTime = Date.now();
            if (this.options.debug) {
            ...
            this.logger.error(`Error executing agent ${name}:`, error);
            // Get agent configuration
            let config;
            // Handle different agent types based on the agent naming scheme
            if (name === 'primary') {
            ...
            const executionMode = this.config.executionMode || this.config.strategy;
            if ((executionMode === 'sequential' || executionMode === types_1.AnalysisStrategy.SEQUENTIAL) && name === 'primary'
     ...
     */
    async executeFallback(failedAgentName, data) {
        // Check for fallback agents, using all available config properties
        let hasFallbacks = (this.config.fallbacks && this.config.fallbacks.length > 0) ||
        ...
        if (!hasFallbacks) {
            ...
            hasFallbacks = !!this.config.fallbackAgents?.length;
        }
        if (!hasFallbacks) {
        ...
        if (failedAgentName === 'primary') {
            if (this.config.primary) {
                ...
                failedAgentPosition = this.config.agents[0].position || types_1.AgentPosition.PRIMARY;
            }
            else {
                this.logger.warn('Primary agent configuration not found, using default role');
                failedAgentRole = 'code_quality';
                failedAgentPosition = types_1.AgentPosition.PRIMARY;
            }
        }
            ...
            if (this.config.secondaries && this.config.secondaries.length > index) {
                ...
                failedAgentPosition = this.config.agents[index + 1].position || types_1.AgentPosition.SECONDARY;
            }
            else {
                this.logger.warn(`Secondary agent ${index} configuration not found, using default role`);
                failedAgentRole = 'code_quality';
                failedAgentPosition = types_1.AgentPosition.SECONDARY;
            }
        }
        else {
        ...
        let applicableFallbacks = [];
        // Try all possible fallback sources
        if (this.config.fallbackAgents && this.config.fallbackAgents.length > 0) {
            // New format
            applicableFallbacks = this.config.fallbackAgents.filter(fallback => fallback.role === failedAgentRole);
        }
        else if (this.config.fallbacks && this.config.fallbacks.length > 0) {
            // Old format
            applicableFallbacks = this.config.fallbacks.filter(fallback => fallback.role === failedAgentRole);
        }
        else if (this.config.fallbackAgents && this.config.fallbackAgents.length > 0) {
        ...
        const sortedFallbacks = applicableFallbacks.sort((a, b) => (b.priority || 0) - (a.priority || 0));
            ...
            try {
                ...
                if (!fallbackAgent) {
                    ...
                    const mergedConfig = {
                        ...
                        position: fallbackConfig.position || types_1.AgentPosition.FALLBACK,
                        focusAreas: fallbackConfig.focusAreas,
                        debug: this.options.debug
                    };
                    // Create agent
                    const provider = fallbackConfig.provider || fallbackConfig.agentType || agent_registry_1.AgentProvider.GOOGLE;
                    fallbackAgent = agent_factory_1.AgentFactory.createAgent(fallbackConfig.role, provider, mergedConfig);
                ...
                this.logger.error(`Fallback agent ${fallbackConfig.agentType || fallbackConfig.provider} failed:`, fallbackError);
            }
        }
