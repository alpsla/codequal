...
const agent_evaluation_data_1 = require("./agent-evaluation-data");
...
class AgentSelector {
    constructor(evaluationData) {
        this.logger = (0, utils_1.createLogger)('AgentSelector');
        // Use provided evaluation data or fall back to mock data
        this.evaluationData = evaluationData || agent_evaluation_data_1.mockAgentEvaluationData;
    }
    /**
     ...
     */
    selectAgent(role, repoContext, prContext, preferences) {
        ...
        if (!bestAgent) {
            ...
            bestAgent = agent_registry_1.AgentProvider.CLAUDE;
        ...
        const config = {
            provider: bestAgent,
            role,
            position: types_1.AgentPosition.PRIMARY, // Default position, can be overridden
            temperature: agent_evaluation_data_1.defaultTemperatures[role],
     ...
     */
    selectMultiAgentConfiguration(roles, repoContext, prContext, preferences, secondaryAgentCriteria) {
        ...
        if (secondaryAgentCriteria) {
            ...
            const useSecondaryAgent = (0, agent_evaluation_data_1.shouldUseSecondaryAgent)(repoContext, prContext, mockPrimaryResult, secondaryAgentCriteria);
            if (useSecondaryAgent) {
                ...
                for (const role of roles) {
                    ...
                    const secondaryAgentConfig = this.selectComplementaryAgent(role, primaryAgent.provider, repoContext, prContext, preferences);
                    secondaryAgentConfig.position = types_1.AgentPosition.SECONDARY;
        ...
        for (const role of roles) {
            ...
            const fallbackConfigs = this.selectFallbackAgents(role, primaryConfig.provider, repoContext, prContext, preferences);
        ...
        const expectedCost = this.calculateExpectedCost(primaryAgents, secondaryAgents, fallbackAgents, useMCP);
        // Find the primary agent with highest role
        const primaryAgent = primaryAgents.reduce((prev, current) => {
            const prevScore = this.calculateAgentScores(prev.role, repoContext, prContext, preferences)[prev.provider];
            const currentScore = this.calculateAgentScores(current.role, repoContext, prContext, preferences)[current.provider];
            return currentScore > prevScore ? current : prev;
        }, primaryAgents[0]);
        // Calculate confidence score based on agent scores
        const confidence = this.calculateConfidenceScore(primaryAgents, secondaryAgents, repoContext, prContext);
        // Generate explanation
        const explanation = this.generateSelectionExplanation(primaryAgent, secondaryAgents, fallbackAgents, useMCP, repoContext, prContext);
        return {
            primaryAgent,
            secondaryAgents,
            fallbackAgents,
     ...
     */
    calculateAgentScores(role, repoContext, prContext, preferences) {
        const scores = Object.values(agent_registry_1.AgentProvider).reduce((acc, provider) => {
            acc[provider] = 0;
            return acc;
        }, {});
        // Iterate through each agent provider
        for (const provider of Object.values(agent_registry_1.AgentProvider)) {
     ...
     */
    selectComplementaryAgent(role, primaryProvider, repoContext, prContext, preferences) {
        ...
        if (!bestAgent) {
            ...
            const availableProviders = Object.values(agent_registry_1.AgentProvider).filter(p => p !== primaryProvider);
            bestAgent = availableProviders[0] || agent_registry_1.AgentProvider.CLAUDE;
        }
        // Construct the agent configuration
        const config = {
            provider: bestAgent,
            role,
            position: types_1.AgentPosition.SECONDARY,
            temperature: agent_evaluation_data_1.defaultTemperatures[role] + 0.1, // Slightly higher temperature for diversity
        };
     ...
     */
    selectFallbackAgents(role, primaryProvider, repoContext, prContext, preferences) {
        // Calculate scores for each agent
        const scores = this.calculateAgentScores(role, repoContext, prContext, preferences);
        // Remove the primary provider from consideration
        delete scores[primaryProvider];
        ...
        return fallbackProviders.map((provider, index) => ({
            ...
            priority: index + 1,
            temperature: agent_evaluation_data_1.defaultTemperatures[role] - 0.05, // Slightly lower temperature for reliability
        }));
    }
    /**
     ...
     */
    shouldUseMCP(repoContext, prContext, preferences) {
     ...
     */
    optimizeForLanguage(config, language) {
     ...
     */
    calculateExpectedCost(primaryAgents, secondaryAgents, fallbackAgents, useMCP) {
        ...
        // Base cost per agent type (in USD)
        const baseCosts = {
            [agent_registry_1.AgentProvider.CLAUDE]: 0.05,
            [agent_registry_1.AgentProvider.OPENAI]: 0.03,
            [agent_registry_1.AgentProvider.DEEPSEEK_CODER]: 0.02,
            [agent_registry_1.AgentProvider.GEMINI_2_5_PRO]: 0.04,
            [agent_registry_1.AgentProvider.MCP_CODE_REVIEW]: 0.06,
            [agent_registry_1.AgentProvider.MCP_DEPENDENCY]: 0.05,
            [agent_registry_1.AgentProvider.MCP_CODE_CHECKER]: 0.05,
            [agent_registry_1.AgentProvider.MCP_REPORTER]: 0.04,
        };
     ...
     */
    calculateConfidenceScore(primaryAgents, secondaryAgents, repoContext, _prContext) {
     ...
     */
    generateSelectionExplanation(primaryAgent, secondaryAgents, fallbackAgents, useMCP, repoContext, _prContext) {
        ...
        if (secondaryAgents.length > 0) {
            const secondaryProviderNames = secondaryAgents.map(agent => this.getProviderDisplayName(agent.provider));
        ...
        if (fallbackAgents.length > 0) {
            const fallbackProviderNames = fallbackAgents.map(agent => this.getProviderDisplayName(agent.provider));
            explanation += `Fallback agents include ${fallbackProviderNames.join(', ')}. `;
        }
        // Add MCP info
        if (useMCP) {
            explanation += `Using Model Control Plane for enhanced processing due to ${repoContext.size.totalFiles > 5000 ? 'large repository size' : 'complex PR changes'}.`;
        }
        return explanation;
    }
    /**
     ...
     */
    categorizeSizePerformance(totalFiles) {
        if (totalFiles < 100) {
            return 'small';
        }
     ...
     */
    categorizeComplexityPerformance(complexity) {
        if (complexity < 25) {
            return 'simple';
        }
    ...
    getRoleDisplayName(role) {
        switch (role) {
            case agent_registry_1.AgentRole.CODE_QUALITY:
                return 'code quality';
            case agent_registry_1.AgentRole.SECURITY:
                return 'security';
            case agent_registry_1.AgentRole.PERFORMANCE:
                return 'performance';
            case agent_registry_1.AgentRole.EDUCATIONAL:
                return 'educational content';
            case agent_registry_1.AgentRole.REPORT_GENERATION:
