...
export class AgentFactory {
  ...
  static createAgent(role: AgentRole, provider: AgentProvider | ProviderGroup, config: Record<string, unknown> = {}): Agent {
    ...
    switch (agentProvider) {
      ...
      case AgentProvider.MCP_DEEPSEEK:
        throw new Error(
          `MCP agent implementation has been archived. ` +
  ...
  private static getClaudePromptForRole(role: AgentRole): string {
    const prompts: Record<AgentRole, string> = {
      ...
      [AgentRole.ARCHITECTURE]: 'claude_architecture_analysis_template',
      ...
      [AgentRole.DEEPWIKI]: 'claude_deepwiki_template'
    };
  ...
  private static getOpenAIPromptForRole(role: AgentRole): string {
    const prompts: Record<AgentRole, string> = {
      ...
      [AgentRole.ARCHITECTURE]: 'openai_architecture_analysis_template',
      ...
      [AgentRole.DEEPWIKI]: 'openai_deepwiki_template'
    };
  ...
  private static getDeepSeekPromptForRole(role: AgentRole): string {
    const prompts: Record<AgentRole, string> = {
      ...
      [AgentRole.ARCHITECTURE]: 'deepseek_architecture_analysis_template',
      ...
      [AgentRole.DEEPWIKI]: 'deepseek_deepwiki_template'
    };
  ...
  private static getGeminiPromptForRole(role: AgentRole): string {
    const prompts: Record<AgentRole, string> = {
      ...
      [AgentRole.ARCHITECTURE]: 'gemini_architecture_analysis_template',
      ...
      [AgentRole.DEEPWIKI]: 'gemini_deepwiki_template'
    };
  ...
  private static readonly DEFAULT_MCP_TOOLS: Record<AgentRole, string> = {
    ...
    [AgentRole.ARCHITECTURE]: 'architecture-analysis',
  ...
  private static readonly RECOMMENDED_PROVIDERS: Record<AgentRole, AgentProvider> = {
    ...
    [AgentRole.ARCHITECTURE]: AgentProvider.CLAUDE,
    ...
    [AgentRole.DEEPWIKI]: AgentProvider.OPENAI
  };
