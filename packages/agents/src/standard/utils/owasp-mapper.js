...
exports.owaspMapper = exports.OWASPMapper = exports.OWASP_CATEGORIES = void 0;
/**
...
class OWASPMapper {
    constructor(customCategories) {
        this.categories = customCategories || exports.OWASP_CATEGORIES;
        // Sort by priority
     ...
     */
    scorePattern(pattern, issue, text) {
        var _a;
        let score = 0;
        ...
        if (pattern.keywords && pattern.keywords.length > 0) {
            ...
            const keywordMatches = pattern.keywords.filter(kw => text.includes(kw.toLowerCase()));
        ...
        if (pattern.categories && issue.category) {
            ...
            const categoryMatch = pattern.categories.some(cat => issue.category.toLowerCase().includes(cat.toLowerCase()) ||
        ...
        if (pattern.types && issue.type) {
            ...
            const typeMatch = pattern.types.some(type => issue.type.toLowerCase().includes(type.toLowerCase()));
            if (typeMatch) {
                score += 0.1;
            }
        }
        // Check file patterns (5% weight)
        if (pattern.filePatterns && ((_a = issue.location) === null || _a === void 0 ? void 0 : _a.file)) {
            maxPossibleScore += 0.05;
            const fileMatch = pattern.filePatterns.some(regex => regex.test(issue.location.file));
            if (fileMatch) {
                score += 0.05;
            }
        }
     ...
     */
    addCustomPattern(categoryCode, pattern) {
...
exports.OWASPMapper = OWASPMapper;
// Export singleton instance for convenience
exports.owaspMapper = new OWASPMapper();
