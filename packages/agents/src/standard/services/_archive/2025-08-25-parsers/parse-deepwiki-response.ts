...
export async function parseDeepWikiResponse(content: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Match numbered items with new format (e.g., "1. **Issue type:** Security")
    // OR old format (e.g., "1. **Title**: Description") OR bullet points (e.g., "- **Title**: Description")
    // OR direct title format (e.g., "Title: Issue description")
    // OR new format with severity-category-id (e.g., "**[HIGH-DEPENDENCY-001] Title**")
    const itemMatch = line.match(/^(\d+)\.\s+\*\*Issue type:\*\*/i) ||
                      line.match(/^(\d+)\.\s+\*\*Title:\*\*\s+(.+)/) ||
                      line.match(/^(\d+\.|-|\*)\s+(.+)/) || 
    ...
    if (itemMatch) {
      ...
      // Format 4: "**Exact file path:** source/core/constants.ts"
      const exactFileMatch = line.match(/\*\*Exact file path:\*\*\s*([^\n]+)/i);
      const fileLineMatch = line.match(/(?:File Path and Line Number|File|file):\s*\*?\*?([^,\n]+?)(?:,\s*(?:Line|line)s?\s*(\d+|multiple lines)?)?/i);
      
      if (exactFileMatch && currentIssue.location.file === 'unknown') {
        currentIssue.location.file = exactFileMatch[1].trim();
      } else if (fileLineMatch && currentIssue.location.file === 'unknown') {
        ...
        // Check for separate line number format: "**Line number:** 10"
        const exactLineMatch = line.match(/\*\*Line number:\*\*\s*(\d+)/i);
        const lineMatch = line.match(/(?:Line|line):\s*(\d+)/) || line.match(/\(line (\d+)\)/);
        if (exactLineMatch) {
          currentIssue.location.line = parseInt(exactLineMatch[1]);
        } else if (lineMatch) {
