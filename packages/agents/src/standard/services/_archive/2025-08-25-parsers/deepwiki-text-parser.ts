...
export class DeepWikiTextParser {
  ...
  parseTextResponse(text: string): ParsedIssue[] {
    ...
    // Split by numbered items with new format: "1. **Issue type:**" or old format "1. **Title**"
    const pattern = /(\d+\.\s+\*\*(?:Issue type:|Title:)[^]*?)(?=\d+\.\s+\*\*(?:Issue type:|Title:)|$)/gi;
    ...
    if (matches.length === 0) {
      ...
      matches.forEach((match, index) => {
        const issue = this.parseIssueItem(match, index);
        if (issue) {
          issues.push(issue);
        }
      });
  ...
  private parseIssueItem(text: string, index: number): ParsedIssue | null {
    try {
      // Extract issue type (if present)
      const typeMatch = text.match(/\*\*Issue type:\*\*\s*(.+)/i);
      const issueType = typeMatch ? typeMatch[1].trim() : '';
      
      // Extract severity
      const severityMatch = text.match(/\*\*Severity:\*\*\s*(\w+)/i);
      const severity = severityMatch ? severityMatch[1].toLowerCase() as 'critical' | 'high' | 'medium' | 'low' : this.detectSeverity(text, issueType);
      
      // Extract exact file path (new format)
      const filePathMatch = text.match(/\*\*Exact file path:\*\*\s*([^\n]+)/i) || 
                           text.match(/\*\*File:\*\*\s*([^\n]+)/i) ||
                           text.match(/(?:in|at|file:?\s*)`?([^`\s,]+\.[a-z]+)`?(?:[:,]\s*line\s*(\d+))?/i);
      const file = filePathMatch ? filePathMatch[1].trim() : 'unknown';
      
      // Extract line number (new format)
      const lineMatch = text.match(/\*\*Line number:\*\*\s*(\d+)/i) || 
                       text.match(/\*\*Line:\*\*\s*(\d+)/i) ||
                       (filePathMatch && filePathMatch[2] ? [null, filePathMatch[2]] : null);
      ...
      // Extract title (from issue type or first line)
      const titleMatch = text.match(/^([^:*]+)/);
      const title = issueType || (titleMatch ? titleMatch[1].replace(/\*\*/g, '').trim() : `Issue ${index + 1}`);
      
      // Categorize based on issue type or keywords
      const category = issueType.toLowerCase().includes('security') ? 'security' :
                      issueType.toLowerCase().includes('performance') ? 'performance' :
                      issueType.toLowerCase().includes('quality') ? 'code-quality' :
                      this.detectCategory(text);
      
      // Extract code snippet if present
      const codeMatch = text.match(/\*\*Code snippet:\*\*[\s\S]*?```[a-z]*\n?([\s\S]*?)```/) ||
                       text.match(/```[a-z]*\n?([\s\S]*?)```/);
