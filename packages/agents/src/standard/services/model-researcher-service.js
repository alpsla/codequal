...
class ModelResearcherService {
    ...
    calculateContextSpecificScore(model, context) {
        ...
        if (context.language === 'Python' && modelLower.includes('code'))
            score += 10;
        if (context.language === 'Rust' && modelLower.includes('opus'))
            score += 10;
        ...
        if (context.framework === 'Machine Learning' &&
            (modelLower.includes('opus') || modelLower.includes('claude')))
            score += 15;
        // Task-specific scoring
        if (context.task_type === 'code-analysis' && !modelLower.includes('chat'))
            score += 10;
