...
export interface IDiffAnalyzer {
     ...
     */
    mapIssuesToChanges(issues: Array<{
        id: string;
        title: string;
        severity: string;
        location: string;
    }>, changes: ChangeAnalysis, diff: GitDiff): Promise<IssueMapping[]>;
    /**
     * Verify which issues were actually fixed
     */
    verifyFixes(mainIssues: Array<{
        id: string;
        title: string;
        severity: string;
    }>, prChanges: ChangeAnalysis, diff: GitDiff): Promise<FixVerification[]>;
    /**
