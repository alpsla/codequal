...
exports.IssueDuplicator = exports.EnhancedIssueMatcher = void 0;
/**
...
class EnhancedIssueMatcher {
    ...
    matchIssues(issue1, issue2) {
        ...
        if (lineShiftMatch.isMatch)
            return lineShiftMatch;
     ...
     */
    contentOnlyMatch(issue1, issue2) {
     ...
     */
    exactLocationMatch(issue1, issue2) {
     ...
     */
    contentBasedMatch(issue1, issue2) {
        if (!this.isSameFile(issue1, issue2) || !this.isSameCategory(issue1, issue2)) {
            return { isMatch: false, confidence: 0, matchType: 'none' };
        }
     ...
     */
    extractCodeFingerprint(issue) {
        const codeSnippet = issue.codeSnippet || issue.evidence?.snippet;
        if (!codeSnippet)
            return null;
        ...
        if (funcCalls)
            patterns.push(...funcCalls);
        // Variable assignments
        const assignments = normalized.match(/\b\w+\s*=/g);
        if (assignments)
            patterns.push(...assignments);
        // SQL patterns
    ...
    codeSnippetsMatch(issue1, issue2) {
        const snippet1 = issue1.codeSnippet || issue1.evidence?.snippet;
        const snippet2 = issue2.codeSnippet || issue2.evidence?.snippet;
    ...
    calculateDescriptionSimilarity(issue1, issue2) {
...
exports.EnhancedIssueMatcher = EnhancedIssueMatcher;
/**
    ...
    deduplicateIssues(issues) {
        ...
        for (let i = 0; i < issues.length; i++) {
            ...
            if (duplicates.length === 0) {
                ...
                const allCandidates = [
                    { issue: currentIssue, index: i, match: { confidence: 100 } },
                    ...duplicates
                ];
...
exports.IssueDuplicator = IssueDuplicator;
