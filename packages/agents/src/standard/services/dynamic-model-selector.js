...
class DynamicModelSelector {
    ...
    async selectModelsForRole(requirements) {
        ...
        logger.info(`Selected primary: ${primary.id} (score: ${(_a = primary.totalScore) === null || _a === void 0 ? void 0 : _a.toFixed(2)})`);
        logger.info(`Selected fallback: ${fallback.id} (score: ${(_b = fallback.totalScore) === null || _b === void 0 ? void 0 : _b.toFixed(2)})`);
        ...
        try {
            ...
            return models.map((m) => {
                ...
                return ({
                    ...
                    pricing: {
                        prompt: parseFloat(((_a = m.pricing) === null || _a === void 0 ? void 0 : _a.prompt) || '0'),
                        completion: parseFloat(((_b = m.pricing) === null || _b === void 0 ? void 0 : _b.completion) || '0')
                    }
                });
            });
        }
     ...
     */
    scoreModels(models, requirements) {
        ...
        return models.map(model => {
            ...
            const version = parseFloat(((_a = modelName.match(/(\d+(?:\.\d+)?)/)) === null || _a === void 0 ? void 0 : _a[1]) || '0');
            if (version > 0 && version < 2.0) {
                // Check if newer versions exist
                const hasNewer = models.some(m => {
                    var _a;
                    const otherVersion = parseFloat(((_a = m.model.toLowerCase().match(/(\d+(?:\.\d+)?)/)) === null || _a === void 0 ? void 0 : _a[1]) || '0');
            ...
            if (qualityWeight > 0.6) {
                if (versionScore >= 0.8)
                    alignmentBonus += 0.2; // Bonus for v2.5+
                if (versionScore <= 0.4)
                    alignmentBonus -= 0.3; // Penalty for v1.5
                if (tierScore >= 0.8)
                    alignmentBonus += 0.1; // Bonus for premium models
                if (costScore > 0.7)
                    alignmentBonus -= 0.1; // Penalty for too cheap
            }
            // Strong cost preference  
            if (costWeight > 0.6) {
                if (costScore >= 0.8)
                    alignmentBonus += 0.2; // Bonus for cheap models
                if (tierScore >= 0.8)
                    alignmentBonus -= 0.1; // Penalty for premium
            }
            // Strong speed preference
            if (speedWeight > 0.6) {
                if (speedScore >= 0.7)
                    alignmentBonus += 0.2; // Bonus for fast models
                if (contextNorm > 0.7)
                    alignmentBonus -= 0.1; // Penalty for large context
            }
    ...
    getVersionScore(modelName) {
        ...
        if (version >= 5)
            return 1.0; // GPT-5, Claude 5, etc.
        if (version >= 4.5)
            return 0.95; // GPT-4.5, etc.
        if (version >= 4)
            return 0.9; // GPT-4, Claude 4.x
        if (version >= 3.5)
            return 0.75; // GPT-3.5, Claude 3.5
        if (version >= 3)
            return 0.7; // GPT-3, Claude 3.x
        if (version >= 2.5)
            return 0.8; // Gemini 2.5 (more advanced than 2.0)
        if (version >= 2)
            return 0.6; // Gemini 2.0, etc.
        if (version >= 1.5)
            return 0.4; // Version 1.5 - OLDER than 2.0!
        if (version >= 1)
            return 0.3; // Version 1.x
        return 0.2; // Very old versions
    }
    /**
     * Get tier score from model name indicators
     */
    getTierScore(modelName) {
        // Premium tier indicators
        if (modelName.includes('opus') || modelName.includes('ultra'))
            return 1.0;
        if (modelName.includes('pro') || modelName.includes('advanced'))
            return 0.9;
        if (modelName.includes('large') || modelName.includes('xl'))
            return 0.8;
        // Mid tier
        if (modelName.includes('sonnet') || modelName.includes('chat'))
            return 0.6;
        if (modelName.includes('standard') || modelName.includes('base'))
            return 0.5;
        // Speed-optimized tier (still good quality but optimized differently)
        if (modelName.includes('flash') && modelName.includes('2.5'))
            return 0.7; // Flash 2.5 is good quality
        if (modelName.includes('flash'))
            return 0.5;
        if (modelName.includes('turbo'))
            return 0.5;
        // Lower tier
        if (modelName.includes('haiku') || modelName.includes('mini'))
            return 0.3;
        if (modelName.includes('nano') || modelName.includes('tiny'))
            return 0.2;
        if (modelName.includes('small') || modelName.includes('lite'))
            return 0.25;
     ...
     */
    selectFallback(scoredModels, primary) {
        // Try to find a fallback from a different provider
        const differentProvider = scoredModels.find(m => m.provider !== primary.provider && m.id !== primary.id);
