...
exports.LanguageAgentFactory = exports.LanguageRouter = void 0;
const CloudAnalysisClient_1 = require("../../two-branch/services/CloudAnalysisClient");
class LanguageRouter {
    constructor() {
        ...
        this.cloudClient = new CloudAnalysisClient_1.CloudAnalysisClient();
    }
    /**
     * Detect primary language from file extensions
     */
    async detectLanguage(files) {
     ...
     */
    async routeAnalysis(repository, language, categories = ['security', 'quality']) {
        const config = this.languageConfigs.get(language);
        if (!config) {
        ...
        if (uniqueTools.length > 0) {
            const batchResults = await this.cloudClient.batchAnalyze(repository, uniqueTools, { branch: 'main' });
     ...
     */
    async detectFromRepository(repositoryPath) {
     ...
     */
    getMissingToolsScript(language) {
        const config = this.languageConfigs.get(language);
        if (!config)
            return '';
        const missing = [];
...
class LanguageAgentFactory {
     ...
     */
    async createAgent(files, repository) {
        // Detect language
     ...
     */
    async analyzeRepository(repository, files, categories = ['security', 'quality', 'dependencies']) {
        // Detect language
        const detection = await this.router.detectLanguage(files);
        // Route to appropriate tools
        const results = await this.router.routeAnalysis(repository, detection.primary, categories);
        // Calculate coverage
