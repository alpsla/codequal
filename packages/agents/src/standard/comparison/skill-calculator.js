...
exports.SkillCalculator = void 0;
class SkillCalculator {
    /**
     * Calculate skill impact from comparison results
     */
    calculateSkillImpact(comparison, userProfile, historicalIssues) {
        const adjustments = [];
        ...
        if (resolved && resolved.length > 0) {
            const resolvedIssues = resolved.map((r) => r.issue || r);
        ...
        if (historicalIssues && historicalIssues.length > 0) {
            const stillExists = historicalIssues.filter(issue => !resolved.some((r) => (r.issue?.id || r.id) === issue.id));
     ...
     */
    calculatePositivePoints(category, count) {
     ...
     */
    calculatePositivePointsBySeverity(issues) {
     ...
     */
    calculateNegativePoints(category, critical, high) {
     ...
     */
    calculateNegativePointsBySeverity(issues) {
     ...
     */
    applyAdjustments(currentSkills, adjustments) {
     ...
     */
    generateRecommendations(adjustments, categoryChanges) {
...
exports.SkillCalculator = SkillCalculator;
