...
const report_generator_v8_final_1 = require("./report-generator-v8-final");
const skill_calculator_1 = require("./skill-calculator");
const issue_matcher_enhanced_1 = require("../services/issue-matcher-enhanced");
...
class ComparisonAgent {
    constructor(logger, modelService, // Deprecated - using DynamicModelSelector
    skillProvider, // BUG-012 FIX: Accept skill provider for persistence
    options) {
        this.logger = logger;
        ...
        this.primaryModel = '';
        this.fallbackModel = '';
        this.useV8Generator = true; // Always use V8
        // Initialize V8 generator will be done after model config is available
        this.reportGeneratorV8 = null; // Will be initialized in initialize()
        ...
        this.skillCalculator = new skill_calculator_1.SkillCalculator();
    ...
    async initialize(config) {
        ...
        try {
            const supabaseConfig = await (0, monitoring_1.getDynamicModelConfig)('comparator', config.language, this.mapComplexityToSize(config.complexity));
            if (supabaseConfig) {
        ...
        if (!this.modelConfig && config.modelConfig) {
            this.modelConfig = {
        ...
        this.reportGeneratorV8 = new report_generator_v8_final_1.ReportGeneratorV8Final({
    ...
    async analyze(input) {
        ...
        try {
            ...
            const aiAnalysis = await this.performAIComparison(input.mainBranchAnalysis, input.featureBranchAnalysis, input.prMetadata);
            ...
            if (input.userProfile) {
                skillTracking = this.skillCalculator.calculateSkillImpact(comparison, input.userProfile, input.historicalIssues || []);
            }
            ...
            if (input.generateReport !== false) {
                ...
                const featureTestCoverage = input.featureBranchAnalysis.testCoverage?.overall ||
                    input.featureBranchAnalysis.metadata?.testCoverage?.overall ||
                    0;
                const mainTestCoverage = input.mainBranchAnalysis.testCoverage?.overall ||
                    input.mainBranchAnalysis.metadata?.testCoverage?.overall ||
                    0;
                ...
                markdownReport = await this.generateReport(reportData);
            }
            // Generate PR comment
            ...
            if (this.modelConfigId) {
                await (0, monitoring_1.trackDynamicAgentCall)({
                    agent: 'comparator',
                    operation: 'analyze',
            ...
            if (this.fallbackModel && this.modelConfig) {
                try {
                    ...
                    let markdownReport;
                    if (input.generateReport !== false) {
                        markdownReport = await this.generateReport(comparison);
                    }
                    return {
                        success: true,
                        comparison,
     ...
     */
    extractPRCommentFromV8(comparison) {
        ...
        if (newIssuesCount > 0) {
            ...
            (comparison.newIssues || []).slice(0, 3).forEach((issue) => {
     ...
     */
    async performAIComparison(mainAnalysis, featureAnalysis, prMetadata) {
     ...
     */
    buildComparisonPrompt(mainAnalysis, featureAnalysis, prMetadata) {
...
Provide confidence scores and reasoning for each finding.`;
     ...
     */
    convertAIAnalysisToComparison(aiAnalysis, prMetadata) {
        // Extract the actual issues from ComparisonIssue wrappers
        const extractIssues = (comparisonIssues) => {
            return comparisonIssues.map(ci => ({
                ...
                codeSnippet: ci.issue.codeSnippet,
                suggestion: ci.issue.suggestion,
                remediation: ci.issue.remediation,
                recommendation: ci.issue.recommendation || ci.issue.suggestion || ci.issue.remediation,
                // Preserve location data from JSON format
                file: ci.issue.file,
                line: ci.issue.line,
                column: ci.issue.column,
                location: ci.issue.location || (ci.issue.file ? {
                    file: ci.issue.file,
                    line: ci.issue.line,
                    column: ci.issue.column
                } : undefined)
        ...
        return {
            ...
            modifiedIssues: aiAnalysis.modifiedIssues.issues.map(mi => mi.issue || mi),
        ...
        }; // TODO: Update ComparisonResult type
    }
    /**
     ...
     */
    performRealComparison(mainAnalysis, featureAnalysis) {
        ...
        const matcher = new issue_matcher_enhanced_1.EnhancedIssueMatcher();
        ...
        const matchedFeatureIndices = new Set();
        ...
        const preserveIssueData = (issue) => {
            ...
            const preservedIssue = {
                ...
                message: issue.message || issue.title || issue.description || 'Unknown Issue',
                title: issue.title || issue.message,
                // FIX: Don't just copy title as description - check if description is different
                description: (issue.description && issue.description !== issue.title)
                    ? issue.description
        ...
        return {
            ...
            overallAssessment: {
                ...
                prRecommendation: newIssues.some((i) => i.severity === 'critical') ? 'block' :
                    newIssues.filter((i) => i.severity === 'high').length > 2 ? 'review' : 'approve',
                confidence: 0.85
            },
            skillDevelopment: {
     ...
     */
    adaptComparisonToV8Format(comparison) {
        ...
        return {
            ...
            score: comparison.score || this.calculateScore(comparison),
            ...
            analysisId: comparison.analysisId || 'v8-report'
        };
    }
    /**
    ...
    log(level, message, data) {
        if (this.logger) {
            ...
            const logFn = console[level]; // eslint-disable-line no-console
