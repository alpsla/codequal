...
exports.ReportGeneratorV8Final = void 0;
const report_generator_v8_fixes_1 = require("./report-generator-v8-fixes");
const dynamic_model_selector_1 = require("../services/dynamic-model-selector");
class ReportGeneratorV8Final {
    // HTMLFormatter removed - using markdown directly
    constructor(modelConfig) {
        try {
            this.modelConfigResolver = new dynamic_model_selector_1.DynamicModelSelector();
            this.fixes = new report_generator_v8_fixes_1.ReportGeneratorV8Fixes();
        ...
        this.logger = {
            info: (msg, ...args) => console.log(`[V8] ${msg}`, ...args),
            error: (msg, ...args) => console.error(`[V8] ERROR: ${msg}`, ...args),
            warn: (msg, ...args) => console.warn(`[V8] WARN: ${msg}`, ...args)
        };
    }
    /**
    ...
    formatIssuesAsMarkdown(title, issues) {
    ...
    async generateReport(comparisonResult) {
        try {
            ...
            if (v8CompatibleResult.mainBranch?.issues) {
                v8CompatibleResult.mainBranch.issues = v8CompatibleResult.mainBranch.issues.map(issue => this.fixes.adjustSeverityForTestFiles(issue));
            }
            if (v8CompatibleResult.prBranch?.issues) {
                v8CompatibleResult.prBranch.issues = v8CompatibleResult.prBranch.issues.map(issue => this.fixes.adjustSeverityForTestFiles(issue));
            }
            // Also adjust severity for the mapped arrays
            v8CompatibleResult.addedIssues = v8CompatibleResult.addedIssues.map(issue => this.fixes.adjustSeverityForTestFiles(issue));
            v8CompatibleResult.fixedIssues = v8CompatibleResult.fixedIssues.map(issue => this.fixes.adjustSeverityForTestFiles(issue));
            v8CompatibleResult.persistentIssues = v8CompatibleResult.persistentIssues.map(issue => this.fixes.adjustSeverityForTestFiles(issue));
...
`;
    ...
    async generateReportWithContext(comparisonResult, context) {
    ...
    async selectOptimalModel(role, complexity) {
    ...
    getLanguageSpecificResources(language) {
        const resources = {
            'typescript': [
    ...
    convertToComparisonResult(analysisResult) {
        return {
            success: true,
    ...
    getContextBasedModel(issueCount, complexity) {
...
<body>
    ...
    async generateHeader(comparisonResult) {
        ...
        if (repoName === 'repository') {
            ...
            else if (comparisonResult.repositoryUrl) {
                repoName = comparisonResult.repositoryUrl.split('/').slice(-2).join('/');
            }
        }
        ...
        const prMetadata = comparisonResult.prMetadata;
        if (prMetadata) {
    ...
    generateExecutiveSummary(comparisonResult) {
        // Import fixes
...
`;
    ...
    calculateMaintainabilityScore(issues) {
        const maintainabilityIssues = issues.filter(i => i.category === 'code-quality' ||
    ...
    generatePRDecision(comparisonResult) {
        ...
        const hasVulnerabilities = [...newIssues, ...unchangedIssues].some(issue => issue.type === 'vulnerability' ||
...
${unchangedIssues.filter(i => i.severity === 'critical').slice(0, 3).map(i => `- ${i.title || i.message} (${i.location?.file || 'unknown'})`).join('\n')}
` : ''}

*Note: Issues in test files are automatically downgraded in severity as they don't affect production code.*
`;
    }
    async generateConsolidatedIssues(comparisonResult) {
    ...
    formatDetailedIssues(issues, prefix) {
        let content = '';
        // Group by severity
    ...
    async formatDetailedIssuesWithFixes(issues, prefix, fixSuggestions) {
        let content = '';
        // Group by severity
        const grouped = this.groupBySeverity(issues);
        ['critical', 'high', 'medium', 'low'].forEach(severity => {
    ...
    formatSingleIssue(issue, id, showCode) {
        ...
        const file = enhancedIssue.location?.file || enhancedIssue.file;
        const line = enhancedIssue.location?.line || enhancedIssue.line;
    ...
    formatSingleIssueWithFix(issue, id, showCode, fixSuggestion) {
        // Enhance issue context
        const enhancedIssue = this.fixes.enhanceIssueContext(issue);
        const title = enhancedIssue.title || enhancedIssue.message || 'Issue';
        let content = `##### [${id}] ${title}\n\n`;
    ...
    generateSecurityAnalysis(comparisonResult) {
        // Import fixes
        // const { V8ReportFixes } = require('./report-generator-v8-comprehensive-fix'); // TODO: Convert to import
    ...
    generatePerformanceAnalysis(comparisonResult) {
        // Import fixes
        // const { V8ReportFixes } = require('./report-generator-v8-comprehensive-fix'); // TODO: Convert to import
        // Get NEW issues only for this analysis
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
    ...
    generateCodeQualityAnalysis(comparisonResult) {
...
`;
    ...
    generateArchitectureAnalysis(comparisonResult) {
...
`;
    ...
    generateProperArchitectureDiagram(comparisonResult) {
...
- **Security:** ${hasSecurityIssues ? '⚠️ Issues detected' : '✅ Clean'}`;
    ...
    generateDependenciesAnalysis(comparisonResult) {
...
`;
    ...
    generateBreakingChanges(comparisonResult) {
...
`;
    ...
    detectBreakingChangesFromIssues(comparisonResult) {
        const changes = [];
        const issues = comparisonResult.prBranch?.issues || [];
        // Detect API changes
    ...
    assessMigrationComplexity(changes) {
        if (changes.length === 0)
            return 'None';
        if (changes.length === 1)
            return 'Low';
    ...
    assessBreakingChangeRisk(changes) {
        if (changes.length === 0)
            return 'None';
    ...
    generateEducationalInsights(comparisonResult) {
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
        const fixedIssues = comparisonResult.fixedIssues || comparisonResult.resolvedIssues || [];
    ...
    getSpecificEducationalResources(issues) {
        const resources = [];
...
\n`;
    ...
    getLearningPoint(issue) {
        ...
        if (category === 'security') {
            ...
            if (title.includes('xss'))
                return '🔒 XSS Prevention: Sanitize user input and encode output';
            if (title.includes('auth'))
                return '🔒 Authentication: Implement proper session management';
            return '🔒 Security: Follow OWASP guidelines for secure coding';
        }
        else if (category === 'performance') {
            if (title.includes('n+1'))
                return '⚡ Query Optimization: Batch database queries to avoid N+1 problems';
            if (title.includes('memory'))
                return '⚡ Memory Management: Monitor and fix memory leaks';
            if (title.includes('async'))
                return '⚡ Async Performance: Use proper async/await patterns';
            return '⚡ Performance: Profile and optimize critical paths';
        }
        else if (category === 'testing') {
            if (title.includes('coverage'))
                return '🧪 Test Coverage: Aim for 80%+ coverage on critical paths';
            if (title.includes('mock'))
                return '🧪 Test Isolation: Use proper mocking strategies';
            return '🧪 Testing: Write tests before fixing bugs (TDD)';
        }
        else if (category === 'code-quality') {
            if (title.includes('complexity'))
                return '📝 Code Simplicity: Break down complex functions';
            if (title.includes('naming'))
                return '📝 Clear Naming: Use descriptive variable and function names';
            return '📝 Clean Code: Follow SOLID principles';
        }
        return '';
    }
    getCategoryInsight(category, issues) {
    ...
    extractIssueType(issue) {
        const title = (issue.title || issue.message || '').toLowerCase();
        if (title.includes('null') || title.includes('undefined'))
            return 'null-safety';
        if (title.includes('type') || title.includes('typescript'))
            return 'type-safety';
        if (title.includes('async') || title.includes('promise'))
        ...
        if (title.includes('auth') || title.includes('permission'))
            return 'authentication';
        if (title.includes('test') || title.includes('coverage'))
            return 'testing';
        return issue.category || 'general';
    }
    getBestPracticeExample(issueType) {
        const examples = {
...
\`\`\``
    ...
    generateQuickTips(issuesByCategory, severityCount) {
        const tips = [];
    ...
    generatePersonalizedLearningPath(comparisonResult) {
        const issues = comparisonResult.prBranch?.issues || [];
...
`;
    ...
    generateSkillTracking(comparisonResult) {
        const fixedIssues = comparisonResult.fixedIssues || comparisonResult.resolvedIssues || [];
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
        const unchangedIssues = comparisonResult.unchangedIssues || comparisonResult.persistentIssues || [];
        ...
        if (fixedCounts.critical > 0)
            content += `| Fixed Critical | ${fixedCounts.critical} | +${(fixedCounts.critical * severityScores.critical).toFixed(1)} | 🟢 +${severityScores.critical} each |\n`;
        if (fixedCounts.high > 0)
            content += `| Fixed High | ${fixedCounts.high} | +${(fixedCounts.high * severityScores.high).toFixed(1)} | 🟢 +${severityScores.high} each |\n`;
        if (fixedCounts.medium > 0)
            content += `| Fixed Medium | ${fixedCounts.medium} | +${(fixedCounts.medium * severityScores.medium).toFixed(1)} | 🟢 +${severityScores.medium} each |\n`;
        if (fixedCounts.low > 0)
            content += `| Fixed Low | ${fixedCounts.low} | +${(fixedCounts.low * severityScores.low).toFixed(1)} | 🟢 +${severityScores.low} each |\n`;
        if (newCounts.critical > 0)
            content += `| New Critical | ${newCounts.critical} | -${(newCounts.critical * severityScores.critical).toFixed(1)} | 🔴 -${severityScores.critical} each |\n`;
        if (newCounts.high > 0)
            content += `| New High | ${newCounts.high} | -${(newCounts.high * severityScores.high).toFixed(1)} | 🔴 -${severityScores.high} each |\n`;
        if (newCounts.medium > 0)
            content += `| New Medium | ${newCounts.medium} | -${(newCounts.medium * severityScores.medium).toFixed(1)} | 🔴 -${severityScores.medium} each |\n`;
        if (newCounts.low > 0)
            content += `| New Low | ${newCounts.low} | -${(newCounts.low * severityScores.low).toFixed(1)} | 🔴 -${severityScores.low} each |\n`;
    ...
    generateTeamSkillsComparison(comparisonResult) {
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
        const fixedIssues = comparisonResult.fixedIssues || comparisonResult.resolvedIssues || [];
    ...
    generateFinancialImpact(comparisonResult) {
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
        const fixedIssues = comparisonResult.fixedIssues || comparisonResult.resolvedIssues || [];
    ...
    calculateSkillScore(issues, category) {
    ...
    calculateImprovement(fixedIssues, category) {
        return fixedIssues.filter(i => i.category === category).length * 5;
    }
    getTrendIndicator(issues, category) {
    ...
    generateAchievements(fixedIssues) {
        const achievements = [];
    ...
    generateTeamGrowthAreas(skills, issues) {
        const areas = [];
    ...
    generateKnowledgeSharingOpportunities(skills, fixedIssues) {
        const opportunities = [];
        ...
        issues.forEach(issue => {
            ...
            switch (issue.category) {
                case 'security':
                    ...
                    break;
                case 'performance':
                    breakdown.performance += points;
                    break;
                case 'testing':
                    breakdown.testing += points;
                    break;
    ...
    calculateCategoryImpact(issues, category) {
        const categoryIssues = issues.filter(i => i.category === category);
        let impact = 0;
        categoryIssues.forEach(issue => {
    ...
    generateBusinessImpact(comparisonResult) {
        // Import fixes
        // const { V8ReportFixes } = require('./report-generator-v8-comprehensive-fix'); // TODO: Convert to import
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
    ...
    calculateCategoryRisk(issues, category) {
    ...
    calculateComplianceRisk(issues) {
        const complianceIssues = issues.filter(i => i.title?.toLowerCase().includes('compliance') ||
            i.title?.toLowerCase().includes('gdpr') ||
            i.title?.toLowerCase().includes('pci'));
        return complianceIssues.length * 15;
    }
    getComplianceImpact(issues) {
    ...
    getComplianceLikelihood(issues) {
        const risk = this.calculateComplianceRisk(issues);
        if (risk >= 30)
            return 'Very Likely';
        if (risk >= 15)
    ...
    getRecommendedTimeline(issues) {
        const critical = this.countBySeverity(issues, 'critical');
        if (critical > 0)
            return '⚠️ **Immediate Action Required** - Fix critical issues before deployment';
        return '📅 **Recommended Timeline** - Address high priority issues within 1 sprint';
    }
    calculateAffectedUsers(issues) {
        const critical = this.countBySeverity(issues, 'critical');
    ...
    calculateServiceDegradation(issues) {
    ...
    calculateDataRisk(issues) {
        const dataIssues = issues.filter(i => i.title?.toLowerCase().includes('data') ||
    ...
    calculateBrandImpact(issues) {
        const critical = this.countBySeverity(issues, 'critical');
        if (critical > 0)
            return '🔴 High - Potential PR crisis';
        return '🟢 Low - Minimal brand impact';
    }
    generateActionItems(comparisonResult) {
        const issues = comparisonResult.prBranch?.issues || [];
...
`;
    ...
    identifyAutomationOpportunities(issues) {
        const opportunities = [];
        const categories = new Set(issues.map(i => i.category));
    ...
    generateAIIDEIntegration(comparisonResult) {
        // Import fixes
        // const { V8ReportFixes } = require('./report-generator-v8-comprehensive-fix'); // TODO: Convert to import
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
    ...
    generateIssueSpecificFixCommands(issues) {
        const fixCommands = [];
...
\`\`\``);
    ...
    calculateTimeSaved(issues) {
        return issues.reduce((sum, issue) => {
            const time = issue.estimatedFixTime || 15;
            return sum + (time / 60);
        }, 0);
    }
    ...
    generateEnhancedSecurityFixSuggestions(issues) {
        const suggestions = [];
        issues.forEach(issue => {
...
`);
    ...
    generateEnhancedPerformanceFixSuggestions(issues) {
        const suggestions = [];
        issues.forEach(issue => {
...
`);
    ...
    generateSecurityFixCommands(issues) {
        return `
...
`;
    ...
    generatePerformanceFixCommands(issues) {
        return `
...
`;
    ...
    generateDependencyFixCommands(issues) {
        return `
...
`;
    ...
    generateCodeQualityFixCommands(issues) {
        return `
...
`;
    ...
    async generateSecurityFixSuggestions(issues) {
        let suggestions = '';
...
`;
    ...
    generateGenericSecuritySuggestion(issue) {
...
`;
    ...
    generatePerformanceFixSuggestions(issues) {
...
`;
    ...
    generateDependencyFixSuggestions(issues) {
        const suggestions = [];
        issues.forEach(issue => {
    ...
    generateCodeQualityFixSuggestions(issues) {
        const suggestions = [];
...
`);
    ...
    generatePRComment(comparisonResult) {
        // Import fixes
        // const { V8ReportFixes } = require('./report-generator-v8-comprehensive-fix'); // TODO: Convert to import
        const newIssues = comparisonResult.newIssues || comparisonResult.addedIssues || [];
        const fixedIssues = comparisonResult.resolvedIssues || comparisonResult.fixedIssues || [];
    ...
    async generateReportMetadata(comparisonResult) {
        const metadata = comparisonResult.metadata;
        const timestamp = new Date().toISOString();
        ...
        const repoUrl = comparisonResult.repositoryUrl || comparisonResult.repository;
        if (repoUrl) {
...
`;
    ...
    getVerifiedEducationalResources(category) {
        const resources = {
            'security': [
    ...
    generateMockCodeSnippet(issue) {
        // This should not be used in production
        return `// Code snippet not available`;
    }
    generateMockFixedCode(issue) {
        // This should not be used in production  
    ...
    getTrend(mainCount, prCount) {
        if (prCount < mainCount)
            return '📈 Improving';
        if (prCount > mainCount)
            return '📉 Declining';
        return '➡️ Stable';
    }
    getTrendArrow(before, after) {
        if (after < before)
            return '↓';
        if (after > before)
            return '↑';
    ...
    calculateComplexity(comparisonResult) {
        const issues = comparisonResult.prBranch?.issues || [];
        const complexity = Math.min(10, Math.round(issues.length / 10));
        return complexity;
    }
    formatTechnicalDebt(issues) {
    ...
    calculateTotalFixTime(issues) {
        return issues.reduce((sum, issue) => {
            const time = issue.estimatedFixTime || 30;
            return sum + (time / 60);
        }, 0);
    }
    calculateRiskScore(critical, high) {
    ...
    getAffectedOperations(change) {
        // Mock implementation
        return 'GET /api/users, POST /api/users';
    }
    detectAntiPatterns(issues) {
        const patterns = [];
    ...
    getChangeIndicator(before, after) {
    ...
    estimatePerformanceImpact(issues) {
        return Math.min(50, issues.length * 5);
    }
    renderHTMLContent(markdown, comparisonResult) {
    ...
    convertMarkdownToHTML(markdown, comparisonResult) {
...
exports.ReportGeneratorV8Final = ReportGeneratorV8Final;
