...
const utils_1 = require("../../../core/src/utils");
const ModelVersionSync_1 = require("../../../core/src/services/model-selection/ModelVersionSync");
const research_prompts_1 = require("./research-prompts");
/**
 * RESEARCHER Agent implementation
 */
class ResearcherAgent {
    constructor(authenticatedUser, config) {
        this.authenticatedUser = authenticatedUser;
        this.researcherCache = null;
        this.logger = (0, utils_1.createLogger)('ResearcherAgent');
     ...
     */
    async conductResearchAndUpdate() {
     ...
     */
    async dynamicModelDiscovery(researcherModel) {
        ...
        const discoveryPrompt = research_prompts_1.RESEARCH_PROMPTS.DYNAMIC_MODEL_DISCOVERY;
     ...
     */
    extractProvidersFromDiscovery(discoveredModels) {
     ...
     */
    async researchProviderModels(provider, researcherModel) {
        const results = [];
        ...
        return [
            {
                model: {
                    ...
                    tier: ModelVersionSync_1.ModelTier.PREMIUM,
            ...
            },
            {
                model: {
                    ...
                    tier: ModelVersionSync_1.ModelTier.STANDARD,
     ...
     */
    async analyzeAndScoreModels(researchResults) {
     ...
     */
    calculateCostEffectiveness(model) {
        if (!model.capabilities || !model.pricing)
            return 0;
        const { capabilities, pricing } = model;
        // 1. Quality Score (40% weight) - Primary factor
        const qualityScore = ((capabilities.codeQuality || 0) * 0.35 + // Most important for code analysis
        ...
        const performanceScore = ((capabilities.speed || 0) * 0.4 +
        ...
        if (this.researchConfig.prioritizeCost) {
            ...
            finalScore = (qualityScore * 0.4 +
                recencyScore * 0.25 +
                performanceScore * 0.2 +
                costScore * 0.15);
        }
        // Bonus for premium models with exceptional quality
        if (qualityScore > 9.0 && model.tier === ModelVersionSync_1.ModelTier.PREMIUM) {
    ...
    calculateRecencyScore(releaseDate) {
        ...
        if (monthsDiff <= 3)
            return 10.0; // Brand new
        if (monthsDiff <= 6)
            return 9.0; // Very recent  
        if (monthsDiff <= 12)
            return 8.0; // Recent
        if (monthsDiff <= 18)
            return 6.0; // Somewhat recent
        if (monthsDiff <= 24)
            return 4.0; // Getting old
     ...
     */
    calculateCostScore(pricing) {
        const avgCost = (pricing.input + pricing.output) / 2;
        // Cost scoring: Very cheap=10, expensive=1
        // $0.1/M = 10, $1/M = 8, $5/M = 6, $15/M = 4, $50/M = 1
        if (avgCost <= 0.2)
            return 10.0; // Ultra cheap (Gemini Flash)
        if (avgCost <= 1.0)
            return 9.0; // Very cheap (DeepSeek)
        if (avgCost <= 3.0)
            return 8.0; // Cheap (GPT-4o mini)
        if (avgCost <= 8.0)
            return 6.0; // Moderate (GPT-4o)
        if (avgCost <= 15.0)
            return 4.0; // Expensive (Claude)
        if (avgCost <= 30.0)
     ...
     */
    async generateConfigurationRecommendations(scoredModels) {
        const updates = [];
     ...
     */
    async generateRoleSpecificRecommendations(agentRole, languages, sizeCategories) {
     ...
     */
    getRoleSpecificPrompt(agentRole) {
        ...
        const dynamicPrompt = research_prompts_1.RESEARCH_PROMPTS.DYNAMIC_MODEL_DISCOVERY
    ...
    getRoleSpecificRequirements(agentRole) {
        switch (agentRole) {
            case 'security':
                return research_prompts_1.RESEARCH_PROMPTS.SECURITY_AGENT_RESEARCH;
            case 'performance':
                return research_prompts_1.RESEARCH_PROMPTS.PERFORMANCE_AGENT_RESEARCH;
            case 'architecture':
                return research_prompts_1.RESEARCH_PROMPTS.ARCHITECTURE_AGENT_RESEARCH;
            case 'codeQuality':
                return research_prompts_1.RESEARCH_PROMPTS.CODE_QUALITY_AGENT_RESEARCH;
            default:
                return research_prompts_1.RESEARCH_PROMPTS.AGENT_REQUIREMENT_RESEARCH;
        }
    }
    /**
     * Perform cross-market research for a specific agent role
     */
    async crossMarketRoleResearch(agentRole, researchPrompt) {
     ...
     */
    simulateCrossMarketResearch(agentRole) {
     ...
     */
    async evaluateRoleSpecificUpdate(context, roleRecommendation) {
     ...
     */
    async generateContextualRecommendation(context, scoredModels) {
        // Get current optimal model for this context
        const repositoryContext = {
            language: context.language,
            sizeCategory: context.sizeCategory,
     ...
     */
    findBestModelForContext(context, scoredModels) {
     ...
     */
    calculateImprovement(currentModel, recommendedModel) {
        if (!currentModel) {
            return {
                shouldUpdate: true,
        ...
        let reason = '';
        if (costSaving > 10)
            reason += `${costSaving.toFixed(1)}% cost saving. `;
        if (performanceGain > 5)
            reason += `${performanceGain.toFixed(1)}% performance gain. `;
     ...
     */
    calculateOverallPerformance(model) {
        if (!model.capabilities)
            return 5; // Default moderate performance
        const { capabilities } = model;
        return ((capabilities.codeQuality || 5) * 0.3 +
     ...
     */
    calculateUpdatePriority(context, improvement) {
     ...
     */
    generateResearchSummary(researchResults, appliedUpdates) {
        const totalCostSavings = appliedUpdates.reduce((total, update) => {
            return total + (update.expectedImprovement.costSaving || 0);
        }, 0);
        const performanceImprovements = appliedUpdates.filter(update => (update.expectedImprovement.performanceGain || 0) > 0).length;
        return {
     ...
     */
    async scheduleRegularUpdates(intervalHours = 24) {
     ...
     */
    async scheduleQuarterlyMetaResearch(intervalDays = 90) {
     ...
     */
    async storeMetaResearchResult(metaResult) {
        try {
     ...
     */
    async useResearcherForContext(language, sizeCategory, agentRole, frameworks, complexity) {
        ...
        const contextRequest = this.generateContextRequest(language, sizeCategory, agentRole, frameworks, complexity);
        ...
        try {
            ...
            this.researcherCache = {
                currentModel: {
                    ...
                    tier: dbConfig.tier || ModelVersionSync_1.ModelTier.STANDARD,
     ...
     * Re-cache template with new researcher model
     */
    async cacheTemplateWithNewResearcher() {
        try {
     ...
     * Generate context-specific request that references cached template
     */
    generateContextRequest(language, sizeCategory, agentRole, frameworks, complexity) {
        const currentYear = new Date().getFullYear();
...
• Detail Level (5%): Thorough analysis`
     ...
     */
    generateMetaResearchPrompt(currentModel) {
        const currentYear = new Date().getFullYear();
...
**OUTPUT FORMAT:**
{
  ...
  "upgradeRecommendation": {
     ...
     */
    simulateMetaResearchResponse(currentModel) {
     ...
     */
    async upgradeResearcher(newProvider, newModel, newVersion, reason, capabilities, pricing, tier) {
        if (!this.researcherCache) {
     ...
     */
    async findModelInfo(provider, model, version) {
