/**
 * V9 Ruby Lite E2E Test
 *
 * Tests the complete V9 analysis flow for Ruby:
 * - BaseToolOrchestrator (universal foundation)
 * - RubyToolOrchestrator (extends base, language-specific)
 * - Universal tool configuration
 *
 * Tools tested:
 * - RuboCop (Ruby linter, Rails support)
...
async function runRubyLiteE2ETest(scenario: TestScenario): Promise<void> {
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🧪 Testing: ${scenario.name}`);
  console.log(`${'='.repeat(80)}\n`);

  const startTime = Date.now();
  const repoPath = `/tmp/test-repo-ruby-${Date.now()}`;
  ...
  try {
    ...
    const tools = toolResolver.getToolsForLanguage('ruby');
    ...
    const reportPath = `/tmp/ruby-v9-report-${Date.now()}.json`;
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                        V9 RUBY LITE E2E TEST                                 ║
║  Tools: RuboCop, Brakeman, bundler-audit, semgrep                           ║
╚══════════════════════════════════════════════════════════════════════════════╝
`);

  for (const scenario of TEST_SCENARIOS) {
    await runRubyLiteE2ETest(scenario);
  }
}
