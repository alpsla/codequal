/**
 * Python Pattern Calibration WITH CODE CONTEXT
 *
 ...
 *
 * Usage:
 *   PYTHON_TEST_REPO=adeyosemanputra/pygoat MAX_ISSUES=50 npx ts-node tests/integration/python/calibrate-python-with-context.ts
 */
...
async function getCalibrationModel(): Promise<string> {
  ...
  try {
    ...
    calibrationModel = process.env.CALIBRATION_MODEL || 'openai/gpt-4o-mini';
...
interface IssueWithContext {
  ...
  message: string;
  severity: string;
  codeSnippet: string;  // Actual code from file!
}
...
function extractCodeSnippet(filePath: string, line: number): string {
  try {
    ...
    const pathsToTry = [
      filePath,
      filePath.replace('/private', ''),  // macOS /private/tmp -> /tmp
      filePath.replace(/^\/private/, ''), // Same but only at start
...
/**
 * Generate fix pattern using AI with actual code context
 */
...
async function generatePatternWithContext(
  ...
  const prompt = `Fix this ${issue.tool} issue in Python code:
...
IMPORTANT: Return ONLY valid JSON. The correctedCode must be actual Python code, not an explanation.`;

  try {
    // Use SimpleOpenRouterClient with dynamically configured model from Supabase
    const client = getOpenRouterClient();
    const model = await getCalibrationModel();
    const response = await client.chat({
      systemPrompt: 'You are an expert Python code fixer. Return only valid JSON.',
      userPrompt: prompt,
      model: model,  // Dynamic model from Supabase (ai_fixer/python/any)
    });
    ...
    // Validate we got actual code, not an error message
    ...
    const pattern = {
      ...
      source: 'ai_generated',
      ai_model: model,  // Dynamic model from Supabase
      ai_confidence: 90,
      verified: false,
      apply_count: 0,
      success_count: 0,
      revert_count: 0,
      tags: ['python', 'ai-generated', 'calibration'],
    };
...
async function calibrate() {
  ...
  const testDir = `/tmp/python-calibrate-ctx-${Date.now()}`;
...
`);
  ...
  try {
    ...
    // Set global repo path for relative file resolution
    ...
    // Get unique rules (one per rule to avoid duplicates)
