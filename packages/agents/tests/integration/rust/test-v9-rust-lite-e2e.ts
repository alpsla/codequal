/**
 * V9 Rust Lite E2E Test
 *
 * Tests the complete V9 analysis flow for Rust:
 * - BaseToolOrchestrator (universal foundation)
 * - RustToolOrchestrator (extends base, language-specific)
 * - Universal tool configuration
 *
 * Tools tested:
 * - clippy (700+ lint rules)
...
async function runRustLiteE2ETest(scenario: TestScenario): Promise<void> {
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🧪 Testing: ${scenario.name}`);
  console.log(`${'='.repeat(80)}\n`);

  const startTime = Date.now();
  const repoPath = `/tmp/test-repo-rust-${Date.now()}`;
  ...
  try {
    ...
    const tools = toolResolver.getToolsForLanguage('rust');
    console.log(`   ✅ Configured ${tools.length} tools`);
    tools.forEach(tool => {
      console.log(`      - ${tool.name} (${tool.category})`);
    });

    console.log('\n🏃 Step 2: Running RustToolOrchestrator...');
    ...
    const reportPath = `/tmp/rust-v9-report-${Date.now()}.json`;
...
`);
  ...
  for (const scenario of TEST_SCENARIOS) {
    await runRustLiteE2ETest(scenario);
  }
}
