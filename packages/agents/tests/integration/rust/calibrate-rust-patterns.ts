/**
 * Rust Pattern Calibration Script
 *
 * Runs the full fix flow on Rust repositories to populate Supabase with patterns:
...
const TEST_REPO = process.env.RUST_TEST_REPO || 'tokio-rs/tokio';
...
async function getPatternStats(): Promise<PatternStats> {
  ...
  const rustTools = ['clippy', 'cargo-audit', 'cargo-deny', 'semgrep'];
  const byTool: Record<string, number> = {};
  let rustRelated = 0;

  for (const p of patterns || []) {
    byTool[p.tool] = (byTool[p.tool] || 0) + 1;
    if (rustTools.includes(p.tool) || p.rule_id?.includes('rust') || p.rule_id?.includes('clippy')) {
      rustRelated++;
    }
  }
...
async function calibrateRustRepo(): Promise<void> {
  const startTime = Date.now();
  const repoUrl = `https://github.com/${TEST_REPO}`;
  const testDir = `/tmp/rust-calibrate-${Date.now()}`;
  const repoPath = `${testDir}/repo`;

  console.log(`
╔══════════════════════════════════════════════════════════════════════════════╗
║                        RUST PATTERN CALIBRATION                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
`);
  ...
  console.log(`   Rust-related patterns: ${initialStats.rustRelated}`);
  ...
  try {
    ...
    console.log('\n🔍 Step 2: Running Rust analysis...');
    const orchestrator = new RustToolOrchestrator();
    ...
    console.log(`   Rust-related patterns: ${finalStats.rustRelated} (${finalStats.rustRelated - initialStats.rustRelated > 0 ? '+' : ''}${finalStats.rustRelated - initialStats.rustRelated})`);
