...
async function getCalibrationModel(): Promise<string> {
  ...
  try {
    ...
    // Try java-specific config first, fall back to python config
    try {
      const config = await resolver.getModelConfiguration('ai_fixer', 'java', 'any');
      calibrationModel = config.primary_model;
    } catch {
      // Fall back to python config if java not configured
...
async function generatePatternWithContext(
  ...
  const prompt = `Fix this ${issue.tool} issue in Java code:
...
IMPORTANT: Return ONLY valid JSON. The correctedCode must be actual Java code, not an explanation.`;
  ...
  try {
    ...
    const response = await client.chat({
      systemPrompt: 'You are an expert Java code fixer. Return only valid JSON.',
      userPrompt: prompt,
      model: model,  // Dynamic model from Supabase
    });
    ...
    const pattern = {
      ...
      file_types: ['java'],  // Match existing pattern format
      detection: {
        rules: [issue.rule],
        patterns: [],
      },
      ...
      success_count: 0,
      revert_count: 0,
      tags: ['java', 'ai-generated', 'calibration'],
    };
...
async function calibrate() {
  ...
  const testDir = `/tmp/java-calibrate-ctx-${Date.now()}`;
