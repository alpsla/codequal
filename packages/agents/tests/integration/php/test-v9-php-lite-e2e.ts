/**
 * V9 PHP Lite E2E Test
 *
 * Tests the complete V9 analysis flow for PHP:
 * - BaseToolOrchestrator (universal foundation)
 * - PHPToolOrchestrator (extends base, language-specific)
 * - Universal tool configuration
 *
 * Tools tested:
 * - PHPStan (static analysis, levels 0-9)
 * - Psalm (type inference, taint analysis)
 * - PHP_CodeSniffer (PSR-12 style)
...
async function runPHPLiteE2ETest(scenario: TestScenario): Promise<void> {
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🧪 Testing: ${scenario.name}`);
  console.log(`${'='.repeat(80)}\n`);

  const startTime = Date.now();
  const repoPath = `/tmp/test-repo-php-${Date.now()}`;
  ...
  try {
    ...
    const tools = toolResolver.getToolsForLanguage('php');
    console.log(`   ✅ Configured ${tools.length} tools`);
    tools.forEach(tool => {
      console.log(`      - ${tool.name} (${tool.category})`);
    });

    console.log('\n🏃 Step 2: Running PHPToolOrchestrator...');
    ...
    const reportPath = `/tmp/php-v9-report-${Date.now()}.json`;
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                         V9 PHP LITE E2E TEST                                 ║
║  Tools: PHPStan, Psalm, PHPCS, composer-audit, semgrep                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
`);

  for (const scenario of TEST_SCENARIOS) {
    await runPHPLiteE2ETest(scenario);
  }
}
