/**
 * Test DeepWiki with Gemini model for repository analysis
 * This script uses the native Google Gemini model supported by DeepWiki
 */
...
// Use Google's Gemini model which is natively supported by DeepWiki
...
function formatOutputFilename(repoUrl) {
  ...
  return path.join(OUTPUT_DIR, `${repoName}-gemini-${timestamp}.md`);
}
...
/**
 * Run repository analysis with Gemini
 */
async function runAnalysis() {
  console.log('DeepWiki Repository Analysis with Gemini');
  console.log('=========================================');
  ...
  try {
    ...
    const messages = [
      { 
        role: 'system', 
        content: 'You are an expert code analyst powered by Google Gemini.'
      },
      { 
        role: 'user', 
