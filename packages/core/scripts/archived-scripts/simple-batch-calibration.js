  // Validate OpenAI key as well
  if (!API_KEYS.openai || batchMode) {
    ...
    if (API_KEYS.openai) {
      ...
      try {
        ...
        if (!batchMode) {
          API_KEYS.openai = await question('Enter valid OpenAI API key: ');
        } else {
          console.log('Skipping OpenAI tests in batch mode.');
          API_KEYS.openai = null; // Set to null to skip
        }
      }
    } else {
      if (!batchMode) {
        API_KEYS.openai = await question('Enter OpenAI API key: ');
      } else {
        console.log('No OpenAI API key found. Skipping OpenAI tests.');
      }
    }
  }  // Validate OpenAI key as well
  if (!API_KEYS.openai || batchMode) {
...
/**
 * Simplified Batch Calibration Script for CodeQual
 * 
...
const size = args.find(arg => arg.startsWith('--size='))?.split('=')[1];
...
const RESULTS_FILE = path.join(OUTPUT_DIR, 'batch-calibration-results.json');
 ...
 */
async function main() {
  console.log('=== Batch Calibration Script ===');
  ...
  // Get valid API keys even in batch mode
  if (!API_KEYS.anthropic || batchMode) {
    ...
    if (API_KEYS.anthropic) {
      ...
      try {
        ...
        console.error('❌ Anthropic API key is invalid:', error.message);
        
        if (!batchMode) {
          API_KEYS.anthropic = await question('Enter valid Anthropic API key: ');
        } else {
          console.log('Skipping Anthropic tests in batch mode.');
          API_KEYS.anthropic = null; // Set to null to skip
        }
      }
    } else {
      if (!batchMode) {
        API_KEYS.anthropic = await question('Enter Anthropic API key: ');
      } else {
        console.log('No Anthropic API key found. Skipping Anthropic tests.');
      }
    }
  }
    ...
    for (const testSize of sizesToTest) {
      ...
      for (const repo of repos) {
        ...
        for (const { provider, model } of MODELS) {
          ...
          if ((provider === 'anthropic' && !API_KEYS.anthropic) || 
              (provider === 'openai' && !API_KEYS.openai)) {
            console.log(`Skipping ${provider}/${model} - API key missing.`);
            continue;
          }
          
          const modelKey = `${provider}/${model}`;
          
          // Skip already completed models in batch mode
          if (batchMode && 
              results[testLanguage][testSize][repo][modelKey] && 
          ...
          // Confirm testing if not in batch mode
          if (!batchMode) {
            const answer = await question(`Test ${modelKey} for ${testLanguage}/${testSize}/${repo}? (y/n/all): `);
            if (answer.toLowerCase() !== 'y' && answer.toLowerCase() !== 'all') {
              console.log(`Skipping ${modelKey}.`);
              continue;
            }
            
            if (answer.toLowerCase() === 'all') {
              console.log('Enabling batch mode for remaining tests.');
              batchMode = true;
            }
          }
          ...
          for (const category of PROMPT_CATEGORIES) {
            // Skip already tested categories in batch mode
            if (batchMode && 
