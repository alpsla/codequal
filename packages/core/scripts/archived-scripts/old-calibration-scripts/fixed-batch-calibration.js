#!/usr/bin/env node
/**
 * Fixed Batch Calibration Script for CodeQual
 * 
...
const RESULTS_FILE = path.join(OUTPUT_DIR, 'fixed-calibration-results.json');
 ...
 * 
 * This configuration was created using the fixed batch calibration system.
 */
 ...
 */
async function main() {
  console.log('=== Fixed Batch Calibration Script ===');
  ...
  if (API_KEYS.anthropic) {
    if (await validateApiKey('anthropic')) {
      ...
      const newKey = await question('Enter valid Anthropic API key: ');
      if (newKey) {
        API_KEYS.anthropic = newKey;
        if (await validateApiKey('anthropic')) {
          validProviders.push('anthropic');
        }
      }
    ...
    const newKey = await question('Anthropic API key not found. Enter key (or press Enter to skip): ');
    if (newKey) {
      API_KEYS.anthropic = newKey;
      if (await validateApiKey('anthropic')) {
        validProviders.push('anthropic');
  ...
  if (API_KEYS.openai) {
    if (await validateApiKey('openai')) {
      ...
      const newKey = await question('Enter valid OpenAI API key: ');
      if (newKey) {
        API_KEYS.openai = newKey;
        if (await validateApiKey('openai')) {
          validProviders.push('openai');
        }
      }
    ...
    const newKey = await question('OpenAI API key not found. Enter key (or press Enter to skip): ');
    if (newKey) {
      API_KEYS.openai = newKey;
      if (await validateApiKey('openai')) {
        validProviders.push('openai');
