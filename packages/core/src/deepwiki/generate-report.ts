...
interface RepoSummary {
  ...
  chatTests: number;
  wikiTests: number;
  bestChatModel: BestChatModel | null;
  bestWikiModel: BestWikiModel | null;
}
...
// Create a logger
const logger = new ScriptLogger({ prefix: 'ReportGenerator' });
 ...
 * @returns HTML report
 */
function generateReport(metrics: MetricsData): string {
  // Create HTML
  const html = `
<!DOCTYPE html>
<html lang="en">
 ...
 * @returns Formatted provider and model
 */
 ...
 * @returns Formatted model
 */
function formatModel(model: BestChatModel | BestWikiModel | null): string {
  if (!model) {
 ...
 */
function getProviderColor(provider: string, alpha = 1): string {
...
async function main(): Promise<void> {
  logger.info('DeepWiki Performance Report Generator');
  logger.info('====================================');
  logger.info(`Metrics file: ${config.metricsFile}`);
  logger.info(`Output file: ${config.outputFile}`);
  
  try {
    // Read metrics file
    const metricsJson = await fs.promises.readFile(config.metricsFile, 'utf8');
    const metrics = JSON.parse(metricsJson) as MetricsData;
    
    logger.info(`Read metrics for ${metrics.totalTests} tests`);
    ...
    logger.info(`Generated report at ${config.outputFile}`);
