...
exports.RepositoryCacheManager = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
/**
 * Default cache invalidation options
 */
const DEFAULT_CACHE_INVALIDATION_OPTIONS = {
...
class RepositoryCacheManager {
     ...
     */
    constructor(supabaseUrl, supabaseKey, logger, invalidationOptions) {
     ...
     */
    async checkCacheStatus(repository, branch) {
        try {
     ...
     */
    async getCachedAnalysis(repository, branch) {
        try {
     ...
     */
    async storeAnalysis(repository, branch, commitHash, result, provider, model) {
        try {
     ...
     * @returns Success status
     */
    async invalidateCache(repository, branch, reason) {
        try {
     ...
     */
    async updateCommitCount(repository, branch, commitCount, significantChanges) {
        try {
     ...
     * @param branch Repository branch
     */
    async updateCacheHitMetrics(repository, branch) {
        try {
            // Update cache hit metrics
     ...
     */
    shouldInvalidateCache(cacheData) {
        // Check commit count
        if (this.invalidationOptions.maxCommitsBeforeInvalidation &&
        ...
        if (this.invalidationOptions.maxAgeMs) {
            const analysisTimestamp = cacheData.latest_analysis_timestamp ? new Date(cacheData.latest_analysis_timestamp).getTime() : 0;
        ...
        if (cacheData.cache_expiry) {
            const expiryTimestamp = cacheData.cache_expiry ? new Date(cacheData.cache_expiry).getTime() : 0;
            const currentTime = new Date().getTime();
     ...
     */
    updateInvalidationOptions(options) {
...
exports.RepositoryCacheManager = RepositoryCacheManager;
