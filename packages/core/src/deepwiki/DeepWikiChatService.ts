...
export class DeepWikiChatService {
  ...
  private readonly SYSTEM_PROMPTS = {
     ...
     */
    REPOSITORY_CONTEXT: 
      "You are an AI assistant that specializes in helping developers understand code repositories. " +
  ...
  async sendMessage(
    ...
    try {
      ...
      // Get chat completion
      const response = await this.deepWikiClient.getChatCompletionForRepo(
        repository,
        {
          messages,
          modelConfig,
   ...
   */
  private extractContentFromResponse(response: Record<string, unknown>): string {
