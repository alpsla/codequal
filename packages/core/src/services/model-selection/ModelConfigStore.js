...
exports.ModelConfigStore = void 0;
...
class ModelConfigStore {
     ...
     */
    async getModelConfig(language, sizeCategory) {
        try {
            // Normalize language
            ...
            return {
                ...
                model: record.model,
                testResults: {
                    status: record.test_results.status,
                ...
                notes: record.notes,
                optimal_models: {},
                testing_status: record.test_results.status,
     ...
     */
    async updateModelConfig(language, sizeCategory, config) {
        try {
            // Normalize language
            const normalizedLang = language.toLowerCase();
     ...
     */
    async storeCalibrationResults(language, sizeCategory, results) {
        try {
            // Normalize language
            const normalizedLang = language.toLowerCase();
     ...
     */
    async getCalibrationResults(language, sizeCategory) {
        try {
            // Normalize language
            const normalizedLang = language.toLowerCase();
    ...
    async getAllModelConfigs() {
        try {
            ...
            for (const record of data) {
                ...
                const sizeCategory = record.size_category;
     ...
     * @returns Whether the storage was successful
     */
    async storeValidatedModel(provider, model, language, sizeCategory, notes) {
        try {
            const normalizedLang = language.toLowerCase();
     ...
     */
    async syncConfigurations(configMap) {
        try {
            ...
            for (const [language, sizeConfigs] of Object.entries(dbConfigs)) {
                ...
                for (const [sizeCategory, config] of Object.entries(sizeConfigs)) {
                    mergedConfigs[language][sizeCategory] = config;
                }
            }
