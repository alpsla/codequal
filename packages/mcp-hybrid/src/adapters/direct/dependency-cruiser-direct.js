...
exports.DependencyCruiserDirectAdapter = void 0;
const child_process_1 = require("child_process");
const base_adapter_1 = require("./base-adapter");
class DependencyCruiserDirectAdapter extends base_adapter_1.DirectToolAdapter {
    constructor() {
        super();
    ...
    async runDependencyCruiser(files) {
        return new Promise((resolve, reject) => {
            const args = ['depcruise', '--output-type', 'json', ...files];
            const process = (0, child_process_1.spawn)('npx', args, { timeout: this.requirements.timeout });
            let stdout = '';
            let stderr = '';
    ...
    filterFindingsByRole(violations, role) {
    ...
    calculateMetricsByRole(output, role, filesAnalyzed) {
        const baseMetrics = {
            filesAnalyzed,
    ...
    async healthCheck() {
        return new Promise((resolve) => {
            const process = (0, child_process_1.spawn)('npx', ['depcruise', '--version'], { timeout: 5000 });
            process.on('close', (code) => {
                resolve(code === 0);
            });
            process.on('error', () => {
    ...
    canAnalyze(context) {
        ...
        return context.repository.languages.some(lang => supportedLanguages.includes(lang.toLowerCase()));
    }
    ...
    getMetadata() {
        return {
            ...
            author: 'CodeQual',
            supportedRoles: ['architecture', 'dependency'],
...
//# sourceMappingURL=dependency-cruiser-direct.js.map