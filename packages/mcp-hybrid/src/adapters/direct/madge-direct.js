...
exports.madgeDirectAdapter = exports.MadgeDirectAdapter = void 0;
...
class MadgeDirectAdapter extends base_adapter_1.DirectToolAdapter {
    constructor() {
        super(...arguments);
        this.id = 'madge-direct';
        this.name = 'Madge Circular Dependency Detector';
        this.version = '1.0.0';
        this.capabilities = [
            {
     ...
     */
    analyzeImportsInChangedFiles(files) {
     ...
     */
    detectPotentialCircularPatterns(imports) {
        const findings = [];
        ...
        filePaths.forEach(fileA => {
            ...
            filePaths.forEach(fileB => {
                ...
                const aImportsB = Array.from(fileAImports).some(imp => fileB.includes(imp.replace(/\.\//, '').replace(/\.\.\//, '')));
                const bImportsA = Array.from(fileBImports).some(imp => fileA.includes(imp.replace(/\.\//, '').replace(/\.\.\//, '')));
     ...
     */
    calculateImportComplexity(imports) {
        let totalImports = 0;
     ...
     */
    calculateLimitedArchitectureScore(findings) {
        let score = 10;
     ...
     */
    async runMadgeAnalysis(workingDir, isTypeScript) {
        try {
            // Build madge command
     ...
     */
    async checkCircular(workingDir, isTypeScript) {
        try {
     ...
     */
    generateCircularFindings(circularDeps, directory) {
        const findings = [];
     ...
     */
    analyzeArchitectureMetrics(result) {
        ...
        const orphaned = modules.filter(mod => tree[mod].length === 0 && !imported.has(mod));
     ...
     */
    calculateMaxDepth(tree, maxIterations = 20) {
        let maxDepth = 0;
        const visited = new Set();
        const calculateDepth = (module, currentDepth) => {
            ...
            const childDepths = deps.map(dep => calculateDepth(dep, currentDepth + 1));
     ...
     */
    generateMetricFindings(metrics, directory) {
        const findings = [];
     ...
     */
    getCircularSeverity(chainLength) {
        if (chainLength <= 2)
            return 'medium';
        if (chainLength <= 4)
            return 'high';
     ...
     */
    formatMultipleCircularDoc(circularDeps) {
     ...
     */
    calculateOverallMetrics(circularDeps, totalModules, findings) {
        const architectureScore = this.calculateArchitectureScore(circularDeps.length, totalModules, findings);
        return {
            totalModules,
     ...
     */
    calculateArchitectureScore(circularCount, totalModules, findings) {
        if (totalModules === 0)
            return 10;
        let score = 10;
...
exports.MadgeDirectAdapter = MadgeDirectAdapter;
// Export singleton instance
exports.madgeDirectAdapter = new MadgeDirectAdapter();
//# sourceMappingURL=madge-direct.js.map