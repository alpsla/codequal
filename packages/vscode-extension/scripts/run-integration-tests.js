...
const test_cleanup_1 = require("./test-cleanup");
 ...
 */
async function testVSCodeExtension(config) {
 ...
 */
async function testGitHubActions(config) {
    const start = Date.now();
    try {
 ...
 */
async function testGitLabCI(config) {
    const start = Date.now();
    try {
 ...
 */
async function testBitbucketPipelines(config) {
    const start = Date.now();
    try {
 ...
 */
async function testCLIIntegration(config) {
    const start = Date.now();
    try {
 ...
 * Main test runner
 */
async function runAllTests(config = defaultConfig) {
    ...
    for (const scenario of scenarios) {
        ...
        if (config.cleanupMode === 'lsp-targeted' && fs.existsSync(config.lspPath)) {
            const cleanup = (0, test_cleanup_1.cleanupLSPAffectedFiles)(config.repoPath, config.lspPath);
            if (config.verbose) {
...
if (require.main === module) {
    ...
    const cleanupMode = args.find(a => a.startsWith('--cleanup='))?.split('=')[1] || 'git-stash';
    runAllTests({
        ...defaultConfig,
        verbose,
        cleanupMode,
