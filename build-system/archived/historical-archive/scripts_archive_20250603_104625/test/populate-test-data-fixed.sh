...
# Populate Test Data for Grafana Security Dashboard - Fixed for Supabase
...
# Function to generate test data - Fixed version
generate_test_data() {
    echo -e "${BLUE}📊 Generating test security events...${NC}"
    
    psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" << 'EOF'
-- First, create test users in auth.users table (Supabase requirement)
-- Note: We'll create users without passwords as they're just for testing
DO $$
BEGIN
...
BEGIN
    ...
    FOR i IN 1..100 LOOP
        ...
        ELSIF event_type = 'AUTH_FAILURE' THEN
            severity := 'medium'::security_severity;
        ELSE
