...
# Step 2: Create fix for OpenRouter client
echo -e "${BLUE}Step 2: Creating OpenRouter client fix script...${NC}"
...
# Add ensure_model_prefix method before convert_inputs_to_api_kwargs
print("Applying OpenRouter client patch...")
...
# Replace the model handling in convert_inputs_to_api_kwargs
...
# Step 3: Create fix for Google model initialization
echo -e "${BLUE}Step 3: Creating Google model initialization fix script...${NC}"
...
# Check if patch is already applied
if "def extract_base_model_name" in content:
    print("Patch already applied, skipping")
    sys.exit(0)

print("Applying Google model initialization patch...")
...
patched_content = content.replace(
    ...
    "from api.config import get_model_config" + helper_func
)
...
echo -e "${YELLOW}2. Test with a small repository: curl -X POST http://localhost:8001/chat/completions/stream -H 'Content-Type: application/json' -d '{\"model\":\"anthropic/claude-3-7-sonnet\",\"repo_url\":\"https://github.com/jpadilla/pyjwt\",\"messages\":[{\"role\":\"system\",\"content\":\"You are a helpful assistant.\"},{\"role\":\"user\",\"content\":\"Analyze this repository briefly.\"}],\"max_tokens\":100,\"stream\":false}'${NC}"