#!/bin/bash
# Enhanced scoring script with model fallback mechanism
# This script maintains OpenRouter as the provider but implements fallback to other models
...
EOF
    ...
    if [ -f "$raw_response" ]; then
      ...
      if grep -q "error\|Error\|API_KEY\|cannot access\|free variable" "$raw_response"; then
      ...
      # Check if the content looks valid (not just error JSON)
      if grep -q "## \|# " "$temp_output" || ! grep -q "\"error\"\|\"detail\"" "$temp_output"; then
...
echo "Starting specialized analyses of $REPO_NAME repository with fallback capability..."
...
for analysis_type in "${CATEGORIES[@]}"; do
    ...
    if [ -f "$analysis_file" ]; then
        ...
        # Look for the score line - try multiple patterns
        score=""
        score_line=$(grep -i "score.*[0-9]/10\|score.*[0-9] out of 10\|score.*[0-9]$" "$analysis_file" | head -n 1)
        ...
        # If not found with first pattern, try just looking for numbers
        if [ -z "$score" ]; then
            score_line=$(grep -i "score" "$analysis_file" | grep -o -E '[0-9]+' | head -n 1)
            score=$score_line
        fi
        ...
        if [ -z "$score" ]; then
            ...
            echo "| $category | $score (default) |" >> "$SCORING_FILE"
        else
        ...
        echo "| $category | 5 (analysis failed) |" >> "$SCORING_FILE"
    fi
done
...
if [ $CATEGORY_COUNT -gt 0 ]; then
    ...
    echo "" >> "$SCORING_FILE"
    echo "## Overall Repository Score: $OVERALL_SCORE" >> "$SCORING_FILE"
else
    echo "" >> "$SCORING_FILE"
    echo "## Overall Repository Score: 5.0 (default)" >> "$SCORING_FILE"
fi
...
for analysis_type in "${CATEGORIES[@]}"; do
    ...
    if [ -f "$ANALYSIS_FILE" ]; then
        ...
        # Add a note about which model was used if it wasn't the primary
        for fallback_model in "${FALLBACK_MODELS[@]}"; do
            temp_file="${OUTPUT_DIR}/${analysis_type}_${fallback_model//\//_}_temp.md"
            if [ -f "$temp_file" ] && cmp -s "$ANALYSIS_FILE" "$temp_file"; then
                echo "> Note: This analysis was performed with fallback model: $fallback_model" >> "$COMBINED_FILE"
                echo "" >> "$COMBINED_FILE"
                break
            fi
        done
...
echo ""
echo "====================================================="
echo "Enhanced scoring with fallback capability complete!"
