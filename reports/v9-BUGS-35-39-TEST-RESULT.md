...
**Target Branch:** trunk  
**Analysis Date:** October 20, 2025 at 09:29 PM GMT  
...
**Total Issues**: 522,640 (67 unique types)
...
**By Category**:
- 🆕 NEW: 150353 (introduced in this PR)
- ⚠️  EXISTING_MODIFIED: 382 (pre-existing in modified files)
- ✅ RESOLVED: 768 (fixed by this PR)
- 📝 EXISTING_REST: 371137 (pre-existing in unchanged files)
...
- 📊 **Most Common**: Com Puppycrawl Tools Checkstyle Checks Indentation IndentationCheck appears 370922 times
- 🔒 **Security Alert**: 2 critical security vulnerabilities found
- 🔧 **Auto-Fix Available**: 516187 issues can be fixed automatically (see IDE integration files)
...
3. **Code Review Process**: High issue count (150353 new) suggests need for more thorough pre-commit review
...
public class KRaftMetadataRequestBenchmark_testTopicIdInfo_jmhTest {
    ...
    private ConcurrentHashMap<String, String> topicIdMap;

    @Setup(Level.Iteration)
    public void setup() {
        // Directly initialize ConcurrentHashMap without dead local store
        topicIdMap = new ConcurrentHashMap<>(1024); // Fixed-size initialization for predictable memory allocation
        for (int i = 0; i < 1024; i++) {
            topicIdMap.put("topic-" + i, "id-" + i); // O(1) average case for ConcurrentHashMap put
        }
    }

    @Benchmark
    public String testTopicIdInfo() {
        // Use pre-initialized map directly
        return topicIdMap.get("topic-123"); // O(1) average case for ConcurrentHashMap get
    }
}
```
...
```java
return versions.clone(); // O(n) time
    // Replace mutable array list with HashMap for O(1) key-based access
    private final Map<String, Metadata> versionMap = new HashMap<>();
     * Avoids linear scans by using HashMap's constant-time put().
    public void addMetadata(String key, Metadata metadata) {
        versionMap.put(key, metadata); // O(1) average case
    }
     * Retrieves metadata by key in O(1) time.
     * Replaces inefficient list iteration or contains() checks.
```
...
import org.apache.kafka.common.security.oauthbearer.internals.handler.OAuthBearerLoginCallbackHandlerAbstract;

public class OAuthBearerLoginCallbackHandler extends OAuthBearerLoginCallbackHandlerAbstract {
    // Use ConcurrentHashMap for thread-safe O(1) lookups instead of linear search
    private final ConcurrentHashMap<String, OAuthBearerToken> tokenCache = new ConcurrentHashMap<>();

    @Override
    public void handle(Callback[] callbacks) throws UnsupportedCallbackException {
        for (Callback callback : callbacks) {
            if (callback instanceof OAuthBearerToken) {
                OAuthBearerToken token = (OAuthBearerToken) callback;
                // Compute token if absent using supplier (avoids redundant computation)
                tokenCache.computeIfAbsent(token.name(), this::fetchTokenFromSource);
                // Directly retrieve cached token (O(1) instead of O(n))
                Principal principal = tokenCache.get(token.name());
                // ... rest of implementation using principal ...
            }
        }
    }

    // Simulated token fetching (replace with actual source)
    private OAuthBearerToken fetchTokenFromSource(String tokenName) {
        // ... implementation ...
        return new OAuthBearerToken(tokenName, "token-value".getBytes(), System.currentTimeMillis(), 3600L);
    }
}
```
...
So, the fix is to rename the variable to comply with the naming conventi
```
...
public class AddRaftVoterRequestDataJsonConverter {
    ...
    void parseJson(String json) {
        try {
            // Hypothetical JSON parsing logic
        } catch (JsonProcessingException e) {
            throw new KafkaException("Failed to parse AddRaftVoterRequestData JSON: " + e.getMessage(), e);
        } catch (Exception e) {
            throw new KafkaException("Unexpected error during JSON conversion", e);
        }
    }
...
**Severity**: MEDIUM | **Tool**: checkstyle | **Found in**: 5225 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-whitespace-operatorwrapcheck-medium-checkstyle-cursor-fix.json)
...
This issue appears in **5225 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 5225 occurrences with one click!
...
**Severity**: MEDIUM | **Tool**: checkstyle | **Found in**: 4437 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-missingjavadocmethodcheck-medium-checkstyle-cursor-fix.json)
...
/**
 * Represents a description of a consumer group, including its members and their states.
 * This class provides detailed information about the consumer group's current state,
 * such as the group ID, members, and the topics they are subscribed to.
 */
public class ConsumerGroupDescription {
    ...
    private final Map<String, List<MemberDescription>> membersByTopic;
    private final Optional<String> state;

    public ConsumerGroupDescription(String groupId,
                                    boolean isSimpleConsumerGroup,
                                    Optional<String> partitionAssignor,
                                    List<MemberDescription> members,
                                    Map<String, List<MemberDescription>> membersByTopic,
                                    Optional<String> state) {
        this.groupId = groupId;
        this.isSimpleConsumerGroup = isSimpleConsumerGroup;
        this.partitionAssignor = partitionAssignor;
        this.members = members;
        this.membersByTopic = membersByTopic;
        this.state = state;
    }
    ...
    public Optional<String> partitionAssignor() {
    ...
    public Map<String, List<MemberDescription>> membersByTopic() {
...
This issue appears in **4437 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 4437 occurrences with one click!
...
**Severity**: MEDIUM | **Tool**: checkstyle | **Found in**: 3162 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-javadocparagraphcheck-medium-checkstyle-cursor-fix.json)
 ...
 *
 * <p>
 * It encapsulates the configuration key, the value to set, and the operation type (set or delete).
 */
...
This issue appears in **3162 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 3162 occurrences with one click!
...
**Severity**: MEDIUM | **Tool**: checkstyle | **Found in**: 3069 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-missingjavadoctypecheck-medium-checkstyle-cursor-fix.json)
...
/**
 * Represents an operation to alter the configuration of a specific resource.
 * This class is used to specify the configuration changes to be applied to a Kafka resource such as a topic or broker.
 */
@InterfaceStability.Evolving
public class AlterConfigOp {
...
This issue appears in **3069 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 3069 occurrences with one click!
...
```java
/**
 * Returns the SCRAM credentials for the specified user.
 * <p>
 * This method provides detailed information about the user's SCRAM credentials,
 * including authentication status and configuration metadata.
 *
 * @param userId the unique identifier for the user
 * @return a map of SCRAM mechanism types to their credential details
 * @throws InterruptedException if the operation is interrupted
 */
public Map<String, ScramCredentialMetadata> userScramCredentials(String userId) throws InterruptedException {
...
class ShareConsumeRequestManager {
    void manageRequests() {
        ...
        long nextRequestTimestamp = System.currentTimeMillis() + requestTimeout;
        ShareGroupJoinRequest.GroupInstanceId groupInstanceId = new ShareGroupJoinRequest.GroupInstanceId("instance-1");
...
First, I need to recall how logging works in Java. Typically
```
...
/**
 * Specifies the offset to be used for resetting the consumer group offsets.
 */
public class OffsetSpec {
    /**
     * Creates an offset specification for the earliest offset.
     *
     * @return an offset specification for the earliest offset
     */
    public static OffsetSpec earliest() {
        return new OffsetSpec(OffsetSpecType.EARLIEST);
    }

    /**
     * Creates an offset specification for the latest offset.
     *
     * @return an offset specification for the latest offset
     */
    ...
    /**
     * Creates an offset specification for a specific offset.
     *
     * @param offset the specific offset to use
     * @return an offset specification for the specified offset
     */
    public static OffsetSpec at(long offset) {
        return new OffsetSpec(offset);
    }
    ...
    private OffsetSpec(long offset) {
        this.type = OffsetSpecType.AT;
        this.offset = offset;
    }

    /**
     * Returns the type of offset specification.
     *
     * @return the type of offset specification
     */
    public OffsetSpecType type() {
        return type;
    }

    /**
     * Returns the specific offset if the type is AT, otherwise throws an exception.
     *
     * @return the specific offset
     * @throws IllegalStateException if the type is not AT
     */
    public long offset() {
        if (type != OffsetSpecType.AT) {
            throw new IllegalStateException("Offset is not specified for type " + type);
        }
        return offset;
    }

    /**
     * Enumerates the types of offset specifications.
     */
    public enum OffsetSpecType {
        EARLIEST,
        LATEST,
        AT
    }
...
```java
   616 |      *       we may need to modify the ConsumerPartitionAssignor API to better support this case.
   617 |      *
   618 |      * @param assignorName          the selected assignor name
>  619 |      * @param assignments           the assignments after assignor assigned
   620 |      * @param allSubscribedTopics   all consumers' subscribed topics
   621 |      */
   622 |     private void maybeUpdateGroupSubscription(String assignorName,
```
...
**Location**: `/workspace/jmh-benchmarks/build/generated/sources/annotationProcessor/java/main/org/apache/kafka/jmh/producer/jmh_generated/RecordAccumulatorFlushBenchmark_jmhType_B1.java` (Line 1)

**Code**:

```java
>    1 | package org.apache.kafka.jmh.producer.jmh_generated;
     2 | import org.apache.kafka.jmh.producer.RecordAccumulatorFlushBenchmark;
     3 | public class RecordAccumulatorFlushBenchmark_jmhType_B1 extends org.apache.kafka.jmh.producer.RecordAccumulatorFlushBenchmark {
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 370922 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-indentation-indentationcheck-low-checkstyle-cursor-fix.json)
...
public class AlterConfigOp {
    ...
    public boolean equals(Object o) {
        ...
        return name.equals(that.name) && opType == that.opType;
    }
...
This issue appears in **370922 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 370922 occurrences with one click!
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 43700 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-sizes-linelengthcheck-low-checkstyle-cursor-fix.json)
...
public class AlterConfigOp {
    ...
    public void validate() {
        if (opType == OpType.DELETE && value != null) {
            throw new IllegalArgumentException(
                "Delete operation cannot specify a value. " +
                "Key: " + key + ", Value: " + value
            );
        }
    }
...
This issue appears in **43700 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 43700 occurrences with one click!
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 14176 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-imports-customimportordercheck-low-checkstyle-cursor-fix.json)
...
public class AlterConfigOp {
    ...
    public AlterConfigOp(Map<String, String> configEntries, OpType opType, ConfigResource resource) {
        this.configEntries = Collections.unmodifiableMap(new HashMap<>(configEntries));
        this.opType = opType;
        this.resource = resource;
    }

    public Map<String, String> configEntries() {
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass()) return false;
        AlterConfigOp that = (AlterConfigOp) o;
        return Objects.equals(configEntries, that.configEntries) &&
               opType == that.opType &&
    ...
    @Override
    public int hashCode() {
        return Objects.hash(configEntries, opType, resource);
    }
}
```
...
This issue appears in **14176 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 14176 occurrences with one click!
...
public class JmxReporter {
    ...
    private void registerMBean(String m_bean_name, ObjectName objectName) {
        // Method implementation using m_bean_name
        if (m_bean_name == null || m_bean_name.isEmpty()) {
            throw new IllegalArgumentException("MBean name cannot be null or empty");
        }
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 7630 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-blocks-needbracescheck-low-checkstyle-cursor-fix.json)
...
### 1. Improvement Description
Wrap the `if` statement in braces to improve readability and prevent potential errors from adding more statements later.
...
This issue appears in **7630 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 7630 occurrences with one click!
...
public class Admin {
    ...
    public KafkaFuture<Map<String, TopicDescription>> describeTopics(Collection<String> topics) {
        // Method implementation
        return null;
    }
}
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 446 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-javadoctagcontinuationindentationcheck-low-checkstyle-cursor-fix.json)
...
This issue appears in **446 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 446 occurrences with one click!
...
**Location**: `/workspace/jmh-benchmarks/build/generated/sources/annotationProcessor/java/main/org/apache/kafka/jmh/producer/jmh_generated/RecordAccumulatorFlushBenchmark_jmhType_B2.java` (Line 6)

**Code**:

```java
     3 | public class RecordAccumulatorFlushBenchmark_jmhType_B2 extends RecordAccumulatorFlushBenchmark_jmhType_B1 {
     4 |     public volatile int setupTrialMutex;
     5 |     public volatile int tearTrialMutex;
>    6 |     public final static AtomicIntegerFieldUpdater<RecordAccumulatorFlushBenchmark_jmhType_B2> setupTrialMutexUpdater = AtomicIntegerFieldUpdater.newUpdater(RecordAccumulatorFlushBenchmark_jmhType_B2.c...
     7 |     public final static AtomicIntegerFieldUpdater<RecordAccumulatorFlushBenchmark_jmhType_B2> tearTrialMutexUpdater = AtomicIntegerFieldUpdater.newUpdater(RecordAccumulatorFlushBenchmark_jmhType_B2.cl...
     8 | 
...
**Location**: `/workspace/jmh-benchmarks/build/generated/sources/annotationProcessor/java/main/org/apache/kafka/jmh/producer/jmh_generated/RecordAccumulatorFlushBenchmark_jmhType_B1.java` (Line 3)
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 53 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-naming-methodtypeparameternamecheck-low-checkstyle-cursor-fix.json)
...
This issue appears in **53 files** across your codebase.
...
> 💡 **Tip**: Download the IDE fix file to resolve all 53 occurrences with one click!
...
**Severity**: LOW | **Tool**: checkstyle | **Found in**: 35 files | **Category**: NEW | **Auto-fix**: ✅ [Available](attachments/group-com-puppycrawl-tools-checkstyle-checks-naming-classtypeparameternamecheck-low-checkstyle-cursor-fix.json)
...
**Good news! All 513,679 CheckStyle issues can be fixed automatically!**
...
-- **Future Risk:** 🟡 Medium
  - Technical debt will compound if 522520 backlog issues are not addressed
...
| **Code Quality** | 0 | 522520 | 522520 | 🟡 Medium |
...
3. **Planning:** Schedule time for 58036 medium-severity issues in upcoming sprints
4. **Continuous Improvement:** Track and reduce 464484 low-severity issues over time
...
🥇 **checkstyle**: 542817 issues in 117.0s (4640.29/s) ⚡ Fast
🥈 **pmd**: 8586 issues in 82.5s (104.05/s) ⚡ Fast
🥉 **spotbugs**: 117 issues in 406.4s (0.29/s) ⚠️ Slow
4. **semgrep**: 11 issues in 113.7s (0.10/s) 🐌 Very Slow
5. **dependency-check**: 0 issues in 5.9s (0.00/s) 🐌 Very Slow
...
### Summary
- **Total Issues:** 522640 (67 unique types)
- **Blocking Issues:** 10 ⛔
- **Resolved Issues:** 768 🎉
- **Analysis Time:** 1451.0s
...
13. [Fix Group 12](attachments/group-com-puppycrawl-tools-checkstyle-checks-whitespace-operatorwrapcheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.whitespace.OperatorWrapCheck (5225 occurrences)
14. [Fix Group 13](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-missingjavadocmethodcheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.MissingJavadocMethodCheck (4437 occurrences)
15. [Fix Group 14](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-javadocparagraphcheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.JavadocParagraphCheck (3162 occurrences)
16. [Fix Group 15](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-missingjavadoctypecheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.MissingJavadocTypeCheck (3069 occurrences)
17. [Fix Group 16](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-summaryjavadoccheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.SummaryJavadocCheck (1997 occurrences)
...
26. [Fix Group 25](attachments/group-com-puppycrawl-tools-checkstyle-checks-naming-abbreviationaswordinnamecheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.naming.AbbreviationAsWordInNameCheck (241 occurrences)
...
29. [Fix Group 28](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-nonemptyatclausedescriptioncheck-medium-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.NonEmptyAtclauseDescriptionCheck (123 occurrences)
...
36. [Fix Group 35](attachments/group-avoidthrowingnullpointerexception-medium-pmd-fix.json) - AvoidThrowingNullPointerException (13 occurrences)
...
47. [Fix Group 46](attachments/group-com-puppycrawl-tools-checkstyle-checks-indentation-indentationcheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.indentation.IndentationCheck (370922 occurrences)
48. [Fix Group 47](attachments/group-com-puppycrawl-tools-checkstyle-checks-sizes-linelengthcheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.sizes.LineLengthCheck (43700 occurrences)
49. [Fix Group 48](attachments/group-com-puppycrawl-tools-checkstyle-checks-imports-customimportordercheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.imports.CustomImportOrderCheck (14176 occurrences)
...
52. [Fix Group 51](attachments/group-com-puppycrawl-tools-checkstyle-checks-blocks-needbracescheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.blocks.NeedBracesCheck (7630 occurrences)
...
54. [Fix Group 53](attachments/group-com-puppycrawl-tools-checkstyle-checks-javadoc-javadoctagcontinuationindentationcheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.javadoc.JavadocTagContinuationIndentationCheck (446 occurrences)
...
59. [Fix Group 58](attachments/group-com-puppycrawl-tools-checkstyle-checks-whitespace-whitespacearoundcheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.whitespace.WhitespaceAroundCheck (110 occurrences)
60. [Fix Group 59](attachments/group-com-puppycrawl-tools-checkstyle-checks-naming-methodtypeparameternamecheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.naming.MethodTypeParameterNameCheck (53 occurrences)
61. [Fix Group 60](attachments/group-com-puppycrawl-tools-checkstyle-checks-naming-classtypeparameternamecheck-low-checkstyle-fix.json) - com.puppycrawl.tools.checkstyle.checks.naming.ClassTypeParameterNameCheck (35 occurrences)
...
**Total auto-fixable issues**: 522,640
