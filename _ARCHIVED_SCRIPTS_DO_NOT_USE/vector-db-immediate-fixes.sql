-- Immediate fixes for Vector DB embedding issues
-- Run these in Supabase SQL Editor to address critical problems
...
-- 2. Create embedding configuration table
...
-- 3. Insert default embedding configurations
...
-- 4. Add metadata column to vector storage if missing
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    -- Add embedding_metadata column if it doesn't exist
    IF NOT EXISTS (
...
-- 5. Create vector operation logs table
...
-- Embedding configs - public read
CREATE POLICY "Public read embedding configs" ON embedding_configurations
  FOR SELECT USING (is_active = true);

CREATE POLICY "Service role manage configs" ON embedding_configurations
  FOR ALL USING (auth.role() = 'service_role');
...
-- 10. Create summary view for monitoring
...
SELECT * FROM embedding_configurations ORDER BY created_at DESC;